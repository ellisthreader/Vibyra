...
#[path = "abandoned_journals.rs"]
pub(super) mod abandoned_journals;
...
impl Vault {
    ...
    pub fn choose(&self, path: PathBuf) -> Result<Value, String> {
        ...
        let engine = Arc::new(Engine::new_read_only(
            directory.clone(),
            name,
            path.clone(),
        )?);
        abandoned_journals::sweep(
            self.state_dir.clone(),
            directory,
            |name| abandoned_journals::is_named(name, "vault-", 16),
