...
test("opening a wide panel stacks terminals instead of squeezing them into unreadable columns", () => {
  const columns = (panes, width) =>
    terminalGridLayout(panes, { width, height: 900, cellWidth: 7.8, cellHeight: 17.33, fontSize: 13 })
      .columns;
  assert.equal(columns(2, 1080), 2);
  assert.equal(columns(2, 500), 1);
  assert.equal(columns(6, 800), 2);
  assert.equal(columns(6, 1400), 3);
});
