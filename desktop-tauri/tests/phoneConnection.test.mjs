...
test("remote access follows the one phone switch: on when signed in, off with it", async () => {
  const pane = await read("src/components/settings/SettingsPhonePane.tsx");
  assert.match(pane, /label="Remote phone control"/, "one switch is the whole decision");
  assert.match(pane, /remote\.signedIn && !remote\.enabled/, "the cloud leg only opens for a signed-in Mac");
  assert.match(pane, /await setRemote\(next\)/, "and it is turned off with the switch");
  assert.doesNotMatch(pane, /<input\b/, "nothing is typed: the account decides who may connect");
  const ipc = await read("src/ipc/phone.ts");
