...
export function seedParityFixture() {
  ...
  const names = ['Review the navigation','Improve sign-in','Development server','Documentation'];
  useTerminalStore.setState({focusedId:1,panes:names.map((title,i) => ({id:i+1,projectId:'studio',agentId:i===2?'shell':'codex',title,
