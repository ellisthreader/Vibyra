    ...
    <div className="welcome-film">
      <div className="welcome-film__desktop" style={phone ? {
          ...
          <div className="welcome-terminal-stage">
            {[0,1,2,3].map(index => {
              ...
              return <div className="welcome-opening-pane" data-terminal={index} key={index}
                ...
                <WelcomeTerminal index={index} time={demoTime - index * .65} />
              </div>;
            })}
          </div>
        </div>
        <div className="welcome-film__view" data-active={step === 2}>
          ...
          <WelcomeAgentActivity time={demoTime} />
          <WelcomeDemoCursor time={demoTime} />
        </div>
        <div className="welcome-film__view welcome-film__settings" data-active={phone}><WelcomeScreen name="phone" /></div>
      </div>
      <div className="welcome-film__phone" style={{opacity:phone ? 1 : 0, transform:`translateX(${phone ? (1 - connected) * -116.6667 : 30}%)`}}>
        <WelcomePhoneFilm time={phone ? demoTime : 0} playing={phone && playing} reduced={reduced} />
      </div>
    </div>
  </div>;
}
