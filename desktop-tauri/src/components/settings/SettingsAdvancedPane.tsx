...
type Group = Extract<SettingsPanelId, "terminal" | "files" | "graphics" | "runtimes" | "vibyraFeatures">;
...
export function SettingsAdvancedPane({ settings, update }: SettingsPaneProps) {
  ...
  const [open, setOpen] = useState<Record<Group, boolean>>({ terminal: false, files: false, graphics: false, runtimes: false, vibyraFeatures: false });

  useEffect(() => {
    if (panel && panel in open) setOpen((prev) => ({ ...prev, [panel]: true }));
    if (panel === "usage") setOpen((prev) => ({ ...prev, vibyraFeatures: true }));
  ...
  return (
    <>
      ...
      <Disclosure title="Chat and voice typing" summary={settings.openaiKeyConfigured ? "OpenAI key installed" : "Needs an OpenAI key"} open={open.vibyraFeatures} onToggle={toggle("vibyraFeatures")} panel="vibyraFeatures">
        <div className="disclosure__stack"><VibyraFeaturesRow settings={settings} update={update} /></div>
      </Disclosure>
