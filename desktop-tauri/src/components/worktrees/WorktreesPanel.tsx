...
import { WorktreesSafeMode } from './WorktreesSafeMode';
...
export function WorktreesPanel(props: Props) {
  ...
  const safe = useProjectLaunchSettings(project).safeMode;
  // Safe Mode needs a repository to branch from; without one it is not running
  // however the preference was left, so this page says so rather than listing
  // worktrees that cannot exist.
  const entry = useProjects().find(candidate => candidate.id === project) ?? null;
  const root = entry?.root ?? null;
  const support = useSafeWorkspaceSupport(root);
  const available = support.supported !== false;
  if (!safe || !available) {
    return <WorktreesSafeMode projectId={project} root={root} name={entry?.name ?? ''}
      available={available} onSetUp={support.recheck} />;
  }
