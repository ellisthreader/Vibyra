...
export function Thread({ agent, identity, active, enabled, onDetails }: { agent: Teammate; identity: string; active: boolean; enabled: boolean; onDetails(): void }) {
  ...
  return <section className="teammate-thread" hidden={!active} aria-label={`Conversation with ${agent.name}`}>
    ...
    <form className="teammate-composer" onSubmit={e => { e.preventDefault(); void chat.estimate(); }}>
      <div className="teammate-composer-attachments">{chat.attachments.map(file => <button type="button" disabled={Boolean(chat.pending) || chat.busy} key={file.id} onClick={() => chat.removeAttachment(file.id)} aria-label={`Remove ${file.name}`}>{file.name} ×</button>)}</div>
      ...
      <div className="teammate-composer-toolbar"><label className="teammate-attach">＋<input type="file" aria-label="Attach file" disabled={chat.busy || !chat.consented || Boolean(chat.pending) || !enabled} onChange={e => { const file = e.target.files?.[0]; if (file) void chat.attach(file); e.target.value = ''; }} /></label><select aria-label="AI model" value={chat.model} onChange={e => chat.setModel(e.target.value)}><option value="auto">{agent.model && agent.model !== 'auto' ? `Default · ${chat.models.find(m => m.id === agent.model)?.name ?? agent.model.split('/').pop()}` : 'Auto'}</option>{chat.models.filter(m => m.available && m.id !== 'auto').map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select>
        ...
        <button className="teammate-mic" type="button" aria-label={voicePhase === 'listening' ? 'Finish dictation' : 'Dictate message'} disabled={chat.busy || Boolean(chat.pending) || !enabled} onClick={() => useVoiceStore.getState().toggle()}>{voicePhase === 'listening' ? 'Finish' : <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true"><rect x="9" y="3" width="6" height="12" rx="3" /><path d="M5 10v2a7 7 0 0 0 14 0v-2M12 19v3m-4 0h8" /></svg>}</button>
        {running ? <button type="button" disabled={chat.busy || !enabled} onClick={() => void chat.stop(tail.id)}>Stop</button> : <button className="teammate-send" aria-label="Prepare message" disabled={!chat.draft.trim() || !chat.ready || chat.busy || !chat.consented || Boolean(chat.pending) || !enabled || agent.archived}>↑</button>}
      </div>
    </form>
  </section>;
}
