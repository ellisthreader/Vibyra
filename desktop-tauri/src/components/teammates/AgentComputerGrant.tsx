...
export function AgentComputerGrant({ agentId, disabled, onChanged }: {
  ...
  return <section className="teammate-computer-grant" aria-label="Agent Computer">
    ...
    <p>Choose one folder this teammate may inspect. Its contents can be sent to the AI model. Keep this Mac open for computer tasks.</p>
    ...
    {grant && <p>{grant.revoked ? 'Local access is off. Cloud removal needs a retry.' : grant.needsReselection ? 'Choose this folder again to restore edit access under the current Mac safety rules.' : <><strong>{grant.canWrite ? 'Edit source:' : 'Read-only folder:'}</strong> {grant.path}{grant.worktreePath && <> <strong>Agent worktree:</strong> {grant.worktreePath} Review edits there before applying them to your project. Removing access leaves this folder on your Mac.</>}</>}</p>}
