...
export function TitleBar() {
  ...
  return <>
    <header className="chrome" data-tauri-drag-region>
      ...
      <div className="chrome__drag" data-tauri-drag-region><div className="product-mode-switch" role="tablist" aria-label="Workspace mode">{(['work', 'agent'] as const).map(value => <button key={value} role="tab" aria-selected={mode === value} onClick={() => choose(value)}>{value === 'work' ? 'Code' : 'Agents'}</button>)}</div></div>
