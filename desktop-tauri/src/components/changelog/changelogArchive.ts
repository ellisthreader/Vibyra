...
// Releases nobody is updating *from* any more, kept because the notes are a
// record. Split from `changelogContent`, which holds the current set, so
// adding a release never means editing a file that is already at its limit.

export const archivedReleases: Record<string, DesktopChangelog> = {
  "0.4.0": {
    version: "0.4.0",
