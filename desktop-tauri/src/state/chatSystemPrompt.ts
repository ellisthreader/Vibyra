...
import { workspaceState } from "../lib/vibyraWorkspaceState";
import { useSettingsStore } from "./settingsStore";

/** The brief costs a native call, so the placeholder is already on screen by
 * the time this is awaited. A brief that cannot be read is not faked. The
 * workspace state is read fresh every pass, so a follow-up after an action
 * sees the terminals that action opened. */
export async function buildPrompt(projectId: string, query: string, spoken: boolean): Promise<string> {
  ...
  if (!project) return systemPrompt(null, null, spoken, workspace);
  const brief =
    settings?.sendProjectContext === false
      ? CONTEXT_OFF
      : await projectBrief(project, query).catch(() => null);
  return systemPrompt(brief, project, spoken, workspace);
}
