...
- For iOS credit/economy plans, verify the maintained mobile purchase integration
  ...
  Implemented economy ownership is in `Vibyra/_ai/App/AI Credits.md`; use the
  versioned Vibes wallet rather than legacy credit refresh. Verify duplicate
  submit/settlement/purchase, overlapping plan-change grants, uncertain costs,
  disabled-feature read access and account-bound restore. Use
  `mobile/scripts/verify-vibes-ui.mjs` for labelled UI fixtures, and keep project
  approvals in the scrolling transcript. Require a native build and Apple sandbox
  purchase acceptance before enabling sales; Expo Go/export, autolinking and
  standalone StoreKit typechecking establish only their respective layers.
