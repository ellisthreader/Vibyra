...
export async function readSecrets(adapter: SecretStorageAdapter): Promise<PersistedSecrets> {
  try {
...
export async function readSecretsStrict(adapter: SecretStorageAdapter): Promise<PersistedSecrets> {
  const raw = await adapter.read();
  return raw ? normalizeSecrets(JSON.parse(raw)) : EMPTY_SECRETS;
}
export async function writeVerifiedSecrets(adapter: SecretStorageAdapter, secrets: PersistedSecrets) {
  ...
  if (current) {
    try { if (secretsEqual(JSON.parse(current), secrets)) return; } catch { /* Repair malformed stored JSON. */ }
  }
...
export function mergeSecretStates(legacy: PersistedSecrets, stored: PersistedSecrets) {
  return normalizeSecrets({
...
export function hasSecrets(value: PersistedSecrets) {
