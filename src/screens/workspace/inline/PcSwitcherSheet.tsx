...
import { getDesktopStatusLabel, getDesktopStatusStyle } from "./TopBarControls";
            ...
            {(["auto", "manual"] as const).map((mode) => {
              ...
              return (
                <Pressable disabled={modeDisabled} key={mode} onPress={() => setConnectMode(mode)} style={[styles.pcConnectTab, active ? styles.pcConnectTabActive : null, modeDisabled ? styles.pcControlDisabled : null]}>
                  <Ionicons name={mode === "auto" ? "wifi-outline" : "keypad-outline"} color={active ? colors.text : "#A6ADBA"} size={15} />
              ...
              {visibleCandidates.map((desktop) => {
                ...
                return (
                  <Pressable disabled={disabled} key={`${desktop.url}-${desktop.pairCode}`} style={[styles.pcCandidateRow, disabled ? styles.pcControlDisabled : null]} onPress={() => onConnectCandidate(desktop)}>
                    <View style={[styles.pcCandidateIcon, desktop.status === "current" ? styles.pcCandidateIconCurrent : null]}><Ionicons name="desktop-outline" color="#91A7FF" size={21} /></View>
                    ...
                    <Ionicons name="chevron-forward" color="#A6ADBA" size={21} />
                  </Pressable>
                );
              })}
            </View>
