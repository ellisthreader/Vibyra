...
export function ProjectCard({
  ...
  const publishedAccent = useThemedColor("#5B7CFA");
  const archivedAccent = useThemedColor("#A6ADBA");
  const draftAccent = useThemedColor("#DAD6F6");
  const activeDot = useThemedColor("#3CD783");
  const publishedDot = useThemedColor("#5B7CFA");
  const archivedDot = useThemedColor("#747C8A");
  const draftDot = useThemedColor("#8F8AA3");
  const sourceIconColor = useThemedColor("#91A7FF");
  const placeholderColor = useThemedColor("#747C8A");
  const moreIconColor = useThemedColor(menuOpen ? "#F5F7FA" : "#747C8A");
  const footerIconColor = useThemedColor("#A6ADBA");
  const renameCancelIconColor = useThemedColor("#BEB9D4");
  const openGradient = prefs.effectiveScheme === "light" ? ["#315BD8", "#2449B8"] as const : ["#4667E8", "#3D5ACF"] as const;
