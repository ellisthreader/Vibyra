...
export async function discoverProjectsFromRoots(roots, selectedRoots = []) {
  const projects = [];
  const seen = new Set();
  ...
  for (const root of roots) {
    ...
    await scanChildren(root, seen, projects, state, 0);
  }
  return projects;
}

async function scanChildren(root, seen, projects, state, depth) {
