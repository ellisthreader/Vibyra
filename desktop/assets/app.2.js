...
function messageRow(message, index) { const assistant = message.role === "assistant"; const image = normalizeChatImage(message.image); const app = normalizeChatApp(message.app); return `<div class="message ${assistant ? "assistant" : "user"}"><div class="message-card"><div class="message-author">${assistant ? "Vibyra" : "You"}</div><div class="message-body">${escapeHtml(message.text)}</div>${image ? `<figure class="generated-image-card"><img src="${escapeAttribute(image.url)}" alt="${escapeAttribute(image.title)}" /><figcaption><strong>${escapeHtml(image.title)}</strong>${image.provider ? `<span>${escapeHtml(image.provider)}</span>` : ""}</figcaption></figure>` : ""}${app ? `<figure class="generated-app-card"><figcaption><span>${icon("code")}</span><strong>${escapeHtml(app.title)}</strong><button type="button" data-open-app="${index}">${icon("play")}Open</button></figcaption></figure>` : ""}</div></div>`; }
