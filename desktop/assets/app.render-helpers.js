...
function homeStatusItem(iconName, label, value, target, detail, state = "") {
  ...
  const pending = target === "phone" && currentState.pendingPair?.status === "pending";
  const tone = state === "working" ? "is-working" : pending ? "is-attention" : connected ? "is-connected" : target === "phone" ? "is-disconnected" : "";
  return `<button class="desktop-home-context-item ${tone}" type="button" ${action} title="${escapeAttribute(detail)}"><span class="desktop-home-context-icon">${icon(iconName)}</span><span class="desktop-home-context-copy"><small>${escapeHtml(label)}</small><strong>${escapeHtml(value)}</strong></span></button>`;
}
...
function homeEmptyTerminals() {
  return `<div class="desktop-home-empty"><span>${icon("terminal")}</span><div><strong>No workspaces open</strong><p>Launch a focused AI workspace for your next build.</p></div><button class="desktop-home-text-button" type="button" data-jump="terminals">Open workspace ${icon("arrow")}</button></div>`;
}
function homeActiveWorkRow(item) {
  return `<button class="desktop-home-active-work" type="button" data-jump="chat"><span class="desktop-home-active-pulse"></span><span><small>In progress</small><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.subtitle)}</p></span><time>${escapeHtml(buildTimeLabel(item))}</time>${icon("arrow")}</button>`;
}
