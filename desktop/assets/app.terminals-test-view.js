function terminalTestTopbarHtml() {
  const presets = terminalTestPresetOptionsHtml();
  return `<header class="terminal-test-toolbar terminal-test-toolbar--sidebar" data-terminal-test-toolbar aria-label="Live preview controls">
    <div class="terminal-test-toolbar-center">
      <div class="terminal-test-device-control">${icon("desktop")}<select data-terminal-test-preset aria-label="Preview resolution">${presets}</select><small data-terminal-test-auto hidden>Auto</small></div>
      <div class="terminal-test-app-control" data-terminal-test-app-control hidden>${icon("grid")}<select data-terminal-test-target aria-label="Detected app"></select></div>
...
function terminalTestWorkspaceHtml() {
  return `<section class="terminal-test-workspace" data-terminal-test-workspace aria-label="Live preview">
    ...
    <div class="terminal-test-canvas" data-terminal-test-canvas>
      <div class="terminal-test-frame" data-terminal-test-frame><iframe data-terminal-test-frame-content title="Project preview" referrerpolicy="no-referrer"></iframe>${terminalTestRunnerHtml()}</div>
...
function terminalTestPresetOptionsHtml() {
  return Array.from(new Set(terminalTestPresets.map((preset) => preset.group))).map((group) => {
    const options = terminalTestPresets.filter((preset) => preset.group === group)
      .map((preset) => `<option value="${preset.key}">${escapeHtml(preset.label)} · ${preset.width} × ${preset.height}</option>`).join("");
    return `<optgroup label="${escapeAttribute(group)}">${options}</optgroup>`;
  }).join("");
}
