...
function handleLocalSlashCommand(terminal, command, args, profile) {
  ...
  if (command === "/voice") { if (typeof openTerminalCompanionPanel === "function") openTerminalCompanionPanel("voice", "terminal"); appendTerminalMessages(terminal, [makeTerminalMessage("system", "Opened Vibyra Voice in the terminal side panel.", { type: "tool", provider: profile.key, meta: { command } })], { clearDraft: true }); return { handled: true }; }
  if (["/memory", "/memories"].includes(command)) { if (typeof openTerminalCompanionPanel === "function") openTerminalCompanionPanel("memory", "terminal"); appendTerminalMessages(terminal, [makeTerminalMessage("system", "Opened Memory in the terminal side panel.", { type: "tool", provider: profile.key, meta: { command } })], { clearDraft: true }); return { handled: true }; }
  if (command === "/phone") { if (typeof openTerminalPhonePanel === "function") openTerminalPhonePanel("terminal"); appendTerminalMessages(terminal, [makeTerminalMessage("system", "Opened Phone Preview in the terminal side panel.", { type: "tool", provider: profile.key, meta: { command } })], { clearDraft: true }); return { handled: true }; }
  ...
  if (["/mcp", "/ide", "/extensions", "/hooks", "/plugin", "/plugins", "/reload-plugins", "/skills", "/apps", "/agent", "/agents", "/tasks", "/background", "/ps", "/tools", "/init", "/terminal-setup", "/setup-github"].includes(command)) { appendTerminalMessages(terminal, [makeTerminalMessage("system", terminalIntegrationText(command, profile), { type: "tool", provider: profile.key, meta: { command } })], { clearDraft: true }); return { handled: true }; }
