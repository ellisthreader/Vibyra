...
Prepared 5 September 2026. Status: proposal for review; application implementation has not started.
...
The shared lesson is clarity: input, current context, visible progress and useful output. Reproduce that usability quality with Vibyra's own visual identity and interaction design.
...
Use one navigation drawer containing Work, Projects, Computers and Settings. Place account access at the drawer footer. A small attention indicator opens pending decisions. The default Work destination restores the most recent task when appropriate; otherwise it shows active work, tasks needing attention and recent results. Keep the information architecture stable while adapting presentation for iPad later.

| Screen | Contents and primary action | Required states |
|---|---|---|
| First launch | Short explanation, sign in, connect a computer, optional clearly labeled demo | New user, returning user, expired login, recovery |
...
| Work | Current tasks, blocked tasks, useful recent outputs; New task | Loading, empty, stale cache, disconnected, failed retrieval |
...
| New task | Prompt plus explicit project/host; provider selection in a sheet | Draft, attachments transferring, ready, sending, acknowledged, failed |
