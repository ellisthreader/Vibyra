...
Chat tool-plan state is guarded in `ChatComposer.tsx`: while a plan card is pending or loading, the composer send button stays locked and async planner completions are invalidated on cancel, edit, clear-tool, start, or unmount. Preserve this request-id guard so cancelled Deep Research/web/image plans cannot reappear or auto-start after the user moves on.
