...
Shared loading surface (2026-05-19): `src/components/LoadingScreen.tsx` owns the reusable animated Vibyra loading page. Use it for full-page or framed load states that would otherwise expose rough spinners; current callers include initial app boot in `App.tsx`, preview loading in `src/components/AppWebView.*.tsx`, and Explore feed loading in `src/screens/workspace/inline/chunk12.tsx`.
