...
After a successful publish response, the publish modal should show the lightweight `ProjectPublishCompletion.tsx` result animation, then close back to Projects. `chunk8.tsx` reuses `publishResult` as the temporary top notice state, and `ProjectPublishNotice.tsx` shows the outcome near the top of Projects before auto-dismissing. Keep under-review copy explicit: the project is currently under review and will appear in Explore after approval.

Owned publish review status comes from authenticated `GET /api/projects/publish-status`. `useProjectPublishStatuses.ts` fetches that map for Projects cards, `ProjectPublishResult.ts` maps review states to card labels/modal copy, and `ProjectPublishModal.tsx` uses `ProjectPublishStatusPanel.tsx` to show a locked status page for pending/under-review projects instead of allowing repeat publishes. The backend also returns `409` from `POST /api/projects/publish` when the same source project is already pending or under review.
