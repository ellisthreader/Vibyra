...
`TypingIndicator` shows only while message text is exactly `Working on it...`. Pending assistant rows carry `ChatMessage.runStatus`; while pending, `AgentRunProgressText` renders elapsed time, safe step labels, and a compact active-file card for code-generation prompts. Tool runs may also set `runStatus.tool` (`image`, `research`, `web`, `analyze`), in which case `ChatToolActivityCard.tsx` renders the visible running card. This progress is runtime-only and disappears as soon as the first stream delta arrives or the generated image card replaces it.
