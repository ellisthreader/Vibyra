...
Signup/login use foreground `appApiRequest` calls. A "Could not reach Vibyra ... failed to fetch" auth error usually means Laravel is not listening on `EXPO_PUBLIC_API_URL`/`:8000`; verify with `curl -I http://127.0.0.1:8000/api/skills` and start `npm run backend` or `npm run dev`. Relevant files: `src/utils/appApi.ts`, `src/context/AppContext.tsx`, `backend/routes/web.php`.
