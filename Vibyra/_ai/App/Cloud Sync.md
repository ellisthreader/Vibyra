...
Signup/login and AI chat use foreground backend calls. `agentErrors.ts` preserves detailed `Could not reach Vibyra at <url>` messages from `appApiMessages.ts` so chat failures expose the configured API URL instead of only showing generic iPhone Local Network copy. A "Could not reach Vibyra ... failed to fetch" auth or chat error usually means Laravel is not listening on `EXPO_PUBLIC_API_URL`/`:8000`; verify with `curl -I http://127.0.0.1:8000/api/skills` and start the default combined workflow with `npm start` (`npm run dev`) or backend-only with `npm run backend`. `npm start` uses `scripts/start-dev.sh`, which refreshes `.env` from the current LAN IP and waits for `/api/skills` before starting Expo. Relevant files: `src/utils/appApi.ts`, `src/context/AppContext.tsx`, `backend/routes/web.php`.
