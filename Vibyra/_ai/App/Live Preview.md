...
`PreviewErrorPanel` should match the phone error overlay pattern: a compact dark bottom card with a purple border, smaller red warning emblem, `Preview crashed` title, one-line message, and three clear actions (`Fix with AI`, `Details`, dismiss) without clipping on narrow screens. When the mini chat is open, measure the chat surface and attach the crash panel directly above it with shared square inner corners, no gap, and no doubled shadow so both existing surfaces read as one clean stack. Desktop-injected runtime/module/HTTP error screens in `desktop/lib/preview.mjs` use the same compact dark crash surface: centered card, purple border, red eyebrow row, smaller `Preview crashed` heading, divider, message, and a collapsed technical-details control.
