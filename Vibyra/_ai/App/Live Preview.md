...
Play-icon and slash-command preview launch are unified in `workspacePreviewLauncher.ts`. Both first call `runtime.openRunnablePreview()` for existing inline/static/verified previews; when none exists and the PC is connected, they create/update the standard `previewServer` chat card, call `app.startPreviewServer()`, attach the verified returned preview, and immediately open `AppPreviewModal`. The chat top-bar play icon should be visible for connected project chats even when no preview is already attached, because pressing it can start the website.
...
`WorkspaceScreen` refreshes an already-open `AppPreviewModal` by comparing the latest preview fingerprint (`id`, `projectId`, `source`, `title`, `url`, and `html`) instead of only `id`; stable project preview ids can carry newer HTML or URLs after mini-chat edits. Relevant file: `src/screens/WorkspaceScreen.tsx`.

`AppPreviewModal` owns the runnable-preview overlay UI. `AppPreviewMiniChat` should stay preview-first: a compact dark/purple command surface, small bottom-right chat icon, short `Preview AI` header, and one clear composer row. Preview edit submits must call `useWorkspaceActions.submitPreviewEdit`, which chooses `runtime.activeProjectTarget()`, opens that project chat, and then calls `app.startAgent(target, prompt)`; do not route mini-preview edits through the generic `onStartChat` path. After send, the mini composer closes immediately and `AppPreviewEditStatus` shows a compact model-aware status pill over the preview. The preview modal tracks native keyboard height and adds it to mini chat and diagnostics bottom offsets so phone keyboards do not cover prompt boxes or error panels.

Preview opening should feel like a full page entering from the top. `AppPreviewModal.tsx` uses a custom `Animated.Value` with `Modal animationType="none"` and translates the whole preview surface down from above the viewport; keep the top chrome compact and let `AppWebView` own the rest of the page.

`PreviewErrorPanel` should match the phone error overlay pattern: a compact dark bottom card with a purple border, smaller red warning emblem, `Preview crashed` title, one-line message, and three clear actions (`Fix with AI`, `Details`, dismiss) without clipping on narrow screens. Desktop-injected runtime/module/HTTP error screens in `desktop/lib/preview.mjs` use the same compact dark crash surface: centered card, purple border, red eyebrow row, smaller `Preview crashed` heading, divider, message, and a collapsed technical-details control.
