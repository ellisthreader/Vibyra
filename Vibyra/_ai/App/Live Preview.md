...
When project chat preview intent finds no runnable preview but has a desktop connection, `workspacePreviewActions.ts` asks for explicit approval to start a phone-viewable desktop web preview without naming a specific framework. `workspacePromptActions.ts` keeps a pending preview-server approval separate from generic terminal approvals; on yes it must immediately add the user's approval and a "starting preview server" notice before calling `app.startPreviewServer()` in the background, then attach the returned verified URL as a Live Preview card. On no it cancels without running a desktop command.
