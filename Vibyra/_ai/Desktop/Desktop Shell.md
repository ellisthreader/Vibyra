...
The Profile page (`activePage === "profile"`, renderer `renderProfile()` in `desktop/assets/app.profile.js`) mirrors the mobile `src/screens/workspace/inline/profile/ProfilePage.tsx` structure. It is registered in the `pages` array in `desktop/assets/app.1.js` with `hidden: true` so it does not appear in the nav rail; it is only reachable through the account dropdown. `renderNav()` filters out `page.hidden`.

Layout: hero (72px avatar, name, optional email, plan pill on the right, credits line + thin purple bar), then three grouped row lists (Account, Preferences, Support), then a footer row group (Clear local cache, Log out — both `.profile-row--danger`), then a one-line muted footer note. Group titles are uppercase muted captions. Rows are 48px tall buttons with icon + label + chevron (or a `Phone` pill on phone-managed rows).

Row actions are handled by `handleProfileRow(action, key)`:
- `open-plans` → `openTokenModal("plans")` for billing
- `open-usage` → `openTokenModal("profile")` for credits/usage
- `open-help` → `openTokenModal("help")`
- `mailto` → `mailto:support@vibyra.app` in a new tab
- `clear-cache` → `confirmClearDesktopCache()` (purges `vibyra.desktop.*` localStorage keys except `vibyra.desktop.auth`, then re-renders)
- `logout` → handled by the global `data-setting="Log out"` delegated handler in `desktop/assets/app.auth.js`
- `phone` → shows a transient `Open Vibyra on your phone to change this.` notice for ~2.4s under the row; do NOT add fake toggles to make these rows "work" on desktop

The Settings dropdown entry passes `setProfileFocus("preferences")` so the Preferences group gets a brief `.profile-group--focused` ring and the page auto-scrolls to it via `requestAnimationFrame` + `scrollIntoView`. The focus state resets immediately so the ring is one-shot.

Honest desktop-side limitations encoded in the rows: Profile information / Refer & earn / Appearance / Language / Notifications / Privacy & security / Terms of service are all `action: "phone"` because the desktop shell has no preference store, account-edit API, or referrals UI yet. When those land, swap `action: "phone"` for a real handler and remove the corresponding `Phone` pill.
...
`#token-modal` is a multi-view sheet driven by `tokenModalView` (in `desktop/assets/app.1.js`), valid values `profile | plans | personalization | help`. Title text in `<h2 id="token-title">` is mutated per view via `tokenModalTitles`. `openTokenModal(view)` accepts the target view; `setTokenModalView(view)` re-renders.
...
- `personalization` (narrow): `.account-note` — short copy directing the user to the phone, because the desktop shell has no real preference store; do NOT fake toggles here.
...
The desktop launcher prefers a frameless Electron window and falls back to Chrome app mode. In Electron, `desktop/app.html` renders one Focus-style `desktop-chrome` titlebar with a draggable left/center region and real minimize/maximize/close controls on the right; the sidebar owns the Vibyra label, and `renderTopbar()` must not inject app/page/account controls while `body.desktop-authenticated` is absent so the auth welcome titlebar stays clean and clickable. Chrome fallback hides `desktop-chrome` and keeps the normal topbar/native frame to avoid duplicate controls. Relevant files: `desktop/electron-main.cjs`, `desktop/electron-preload.cjs`, `desktop/lib/window.mjs`, repo-root `Vibyra Desktop`, `desktop/app.html`, `desktop/assets/app.1.js`, `desktop/assets/app.7.css`.
...
Desktop multi AI terminals are exposed as a primary rail page. `desktop/assets/app.terminals.js` owns the 12-terminal workspace, per-terminal draft/history/model/effort/project state in `localStorage["vibyra.desktop.aiTerminals"]`, and concurrent sends through the desktop chat client; `desktop/assets/app.terminals.css` owns the terminal styling. The UI defaults to a focus-mode active terminal; terminal open/close/reorder controls live in the existing topbar through compact numbered tabs, visible tab close buttons, and drag-to-reorder tabs. Because those tabs render inside the Electron draggable titlebar, terminal controls must use `-webkit-app-region: no-drag`, close clicks should stop propagation before tab drag handling, and the terminal tab strip should stay centered in the desktop chrome when terminal chats are open. If no terminals exist, the page shows a simple setup panel asking how many terminals to open, supports custom counts up to 12 with a small layout preview, and uses one compact model selector that opens a searchable vertical model list; do not display every model as cards in the setup panel or use provider filter boxes in the dropdown. Keep model-list hover neutral and reserve purple for the active/selected model row. Optional grid mode uses `localStorage["vibyra.desktop.terminalsLayout"]`; model/effort/project settings stay hidden in per-terminal settings. Backend plumbing lives in `desktop/lib/aiTerminals.mjs`, with `desktop/lib/routes.mjs` delegating loopback-only `/desktop/terminals` routes after `authorizeDesktopUi`. The in-memory API supports up to 12 sessions, sends prompts through `sendDesktopChat()` instead of duplicating backend chat logic, and lets backend 429/reset metadata propagate through the route error handler.

A spawned terminal opens with a Claude-Code-style boot banner rendered by `terminalBanner()` in `desktop/assets/app.terminals.js`: a "Vibyra Desktop v<version>" title, a small ASCII logo block (provider-tinted: green for OpenAI, orange for Claude, blue for Gemini, purple for Auto), and three meta lines showing the model + reasoning effort (`<Model> with <effort>`), the plan label (`Vibyra <tier>`, derived from `currentPlanTier()` in `app.2.js`), and a fake `~/<slug>` cwd built from the bound project name (or `~/workspace` when none). The banner persists at the top of `.terminal-lines` and scrolls off as messages accumulate, mimicking a real shell session. Messages use a `>` prompt prefix for the user line and a prefix-free monospace block for assistant output. A blinking purple caret (`terminalCaret()`, `terminal-caret` keyframes) sits below the latest message when the terminal is idle, and the composer ships a leading `>` glyph aligned with line prompts so the input feels like part of the same scroll. Do not regress the banner to the old single-line "modelLabel / project" empty state—it must show the boot block even before the first message so the page feels like a real CLI.
