...
## Starting A Project (2026-09-01)

The `＋` on the project strip no longer opens a folder picker. It opens
`components/projects/new/NewProjectDialog.tsx`, which forks into *start
something new* and *open a folder I have* — the second calls the unchanged
`projectStore.pickAndCreate`. The new branch asks kind → stack → options →
name, then runs the stack's own scaffolder. Every question is skippable and
every skip lands on naming, so all paths still end in a folder
(`lib/projectCreateFlow.ts` owns that contract and is unit-tested).

The catalog is data in `lib/projectTemplates*.ts`, in the spirit of
`agents/catalog.rs`: a stack is one entry with `requires`, `steps` and
optional `seeds`. A step declares its working directory (`parent` for
scaffolders that create the folder themselves, `project` for ones that run
inside it) and its phase (`create` always runs, `install` follows the
dependencies toggle). Options are deliberately uniform — install, git,
open a terminal — because a per-template TypeScript or package-manager switch
turns every entry into a special case and is how a wrong flag ships.

The stack list ends in an **`Other…`** row that swaps the kind's shortlist for
the whole catalog, searchable through `paletteScore` — the command palette's
matcher, not a second one — with each row chipped with the kind it is filed
under. Browsing is a view of the stack step, not a fifth step, so the title and
rail do not move. `kindForTemplate` keeps the chosen kind when the template
covers it and falls back to the template's primary kind when it does not, so
the review screen can never read "Making: Game / With: Next.js".

Rust owns everything that touches disk: `vibyra-core/src/scaffold/`.
Four rules that must not be relaxed:

- **No TTY.** Steps are spawned with `stdin` null, so a scaffolder that stops
  to ask a question would hang. Every catalogued npm/npx generator carries a
  non-interactive flag (asserted by `tests/projectTemplates.test.mjs`) and
  `run.rs` has a 90-second stall guard that kills the step and offers the user
  a real terminal instead.
- **No shell.** Steps are argv, so a project name can never become a command.
- **`sanitize_command` on every child.** Without it an AppImage launch hands
  `npx` and `python3` the bundle's loader paths and they fail in ways that read
  as a broken template.
- **Nothing is overwritten.** `plan.rs` refuses a destination that already has
  files, and refuses a seed path with a `..` component.

`{{name}}`/`{{Name}}` are substituted in TypeScript; `{{dir}}` and `{{venv}}`
in Rust, because only that side knows a venv puts binaries in `bin` on Unix and
`Scripts` on Windows. Node CLIs resolve to `npm.cmd`/`npx.cmd` on Windows, in a
`#[cfg(windows)]` helper whose imports stay inside the function.

`ProjectSpec` gained optional `templateId` and `createdMs` (both sides,
`#[serde(default)]` so old settings load). Nothing reads them yet.

Catalog commands were each run for real once. `rails`, `go-module` and
`flutter` are unverified — those toolchains are absent on this machine, and
preflight disables their rows. See `docs/desktop-new-project-plan.md` Part 9.
