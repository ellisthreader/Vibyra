...
Owner listing management, updated 2026-06-09: `GET /api/projects/publish-status` returns owner metadata plus backend-owned lifecycle fields: `listingState`, `isDiscoverable`, `isOpenable`, `currentReleaseState`, `candidateReleaseState`, `currentPublicUrl`, `candidateError`, and `allowedActions`. Authenticated `GET /api/community/projects` marks the viewer's own public listings with `viewerCanManage` and `sourceProjectId`; unauthenticated/public calls return `viewerCanManage=false`. `PATCH /api/projects/{slug}/listing` updates listing metadata without review or deployment changes. `PATCH /api/projects/{slug}/publish` only changes visibility, and `DELETE /api/projects/{slug}/publish` deletes the listing without deleting local project files. Public discovery requires an approved, public, openable current release; a newer building or failed candidate must not hide the preserved current release.
...
Temporary testing flag (updated 2026-06-09):
`moderation.publish_force_approve_under_review` /
`PUBLISH_REVIEW_FORCE_APPROVE_UNDER_REVIEW=true` can explicitly force-approve
publish requests in local, testing, or production environments after
deterministic safety and local text moderation. It skips remote moderation/AI
review so end-to-end publishing can be tested quickly; hard deterministic or
local moderation `denied` decisions still block. It only applies when a publish
request runs and does not mass-approve existing review rows. Production
currently has this flag enabled temporarily. Disable it after testing with
`railway variable set PUBLISH_REVIEW_FORCE_APPROVE_UNDER_REVIEW=false --service Vibyra --environment production`.
Focused tests that exercise real pending or denied review behavior explicitly
set `moderation.publish_force_approve_under_review=false`; do not disable the
running testing flag globally just to make those tests deterministic.
