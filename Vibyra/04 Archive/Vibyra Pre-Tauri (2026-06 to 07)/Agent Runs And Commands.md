---
title: Desktop - Agent Runs And Commands
tags:
  - ai/desktop
  - agents
  - commands
...
- Desktop authority for legacy paired-phone agent runs, apply/discard gates, run artifacts, vault lookup, safe command allowlist, and PTY route basics.
- Pair with [[AI Terminals]] for current PTY/provider terminals and [[Projects And Preview]] for project context/discovery feeding agent prompts.
- Decisions: no local template fallback may pretend to satisfy prompts; generated edits stay pending until permission gate applies them unless `apply: true` is explicitly allowed.
- Watchpoints: constrain generated paths under the approved project root and keep command execution on the small explicit allowlist.
