---
title: Desktop - Voice And Project Memory
tags:
  - ai/desktop
  - voice
  - memory
...
- Desktop authority for AI Talk, terminal dictation, companion panel memory, local prompt transcripts, microphone capture, speech playback, and editable project memory.
- Pair with [[AI Terminals]] for companion-panel lifecycle and [[Auth And Cloud Sync]] for backend project-memory vault routes and merge behavior.
- Decisions: Voice is a Vibyra AI conversation, not terminal dictation; `F8` terminal dictation is separate and sends bracketed paste to the originally captured terminal.
- Watchpoints: stop microphone/playback tracks on every cancellation/context-change path, never write raw audio/base64 to transcripts, and flush dirty Memory before terminal context switches.
