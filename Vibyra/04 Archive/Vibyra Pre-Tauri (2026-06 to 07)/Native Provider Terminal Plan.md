---
title: Native Provider Terminal Plan
tags:
  - ai/desktop
  - terminals
  - native-providers
...
- Implementation-ready but gated rollout plan for Qwen Code, Kimi Code, Mistral Vibe, and Grok Build native CLIs.
- Pair with [[AI Terminals]] for current provider runtime rules and [[Chat And Cost Controls]] for managed-credit billing/gateway enforcement.
- Decisions: each provider must run its genuine foreground TUI; Vibyra Agent is only for model families without an applicable official coding CLI.
- Watchpoints: recheck version pins immediately before implementation, capture real native traffic before designing endpoints, and keep adapters disabled until sandbox, credential, billing, and PTY evidence passes.
