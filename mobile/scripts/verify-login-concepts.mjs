...
try {
  for (const concept of (process.env.VIBYRA_CONCEPT ? [process.env.VIBYRA_CONCEPT] : ['pocket','everywhere','connected'])) for (const theme of ['light','dark']) {
    ...
    const campaignTone = await page.locator(`.${concept}-campaign`).evaluate(el => getComputedStyle(el).backgroundColor);
    if (concept === 'pocket') {
      assert.equal(campaignTone, 'rgba(0, 0, 0, 0)', 'simplified campaign has no panel background');
      assert.equal(await page.locator('.pocket-showcase').evaluate(el => getComputedStyle(el).backgroundColor), 'rgba(0, 0, 0, 0)');
    } else assert.notEqual(campaignTone, 'rgba(0, 0, 0, 0)', 'campaign surface is painted');
    if (concept === 'pocket') {
      assert.match(await page.locator('.pocket-campaign h2').innerText(), /Download the new\s+Vibyra iOS app\./);
      assert.match(await page.locator('.pocket-showcase-title').innerText(), /Build from\s+your pocket\./);
      ...
      assert.ok(Math.abs(login.x + login.width / 2 - 640) < 1, 'sign-in centred in full window');
      const showcase = await page.locator('.pocket-showcase').boundingBox();
      assert.ok(showcase.x >= login.x + login.width, 'phone showcase stays right of sign-in');
    ...
    if (concept === 'pocket') {
      ...
      assert.ok(Math.abs(box.x + box.width / 2 - 480) < 1, 'signup stays centred in a smaller Mac window');
    }
  ...
  console.log(`PASS: ${process.env.VIBYRA_CONCEPT || '3 concepts'} × 2 themes, provider/email/signup/recovery previews, cleared demo fields, narrow/short windows, reduced motion, gallery switching and compare.`);
