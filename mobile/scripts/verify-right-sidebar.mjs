...
try {
 for(const theme of ['dark','light']) {
  ...
  assert.equal(await page.getByRole('tab',{name:'Worktrees',exact:true}).count(),0);
  await page.getByRole('textbox',{name:'Message Vibyra'}).fill('Keep this draft');
  await page.evaluate(()=>{window.dispatchEvent(new Event('fixture-checkout'));window.dispatchEvent(new CustomEvent('fixture-safe',{detail:true}));});
