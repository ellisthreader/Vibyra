import { notificationFixtureApi } from './notificationFixtureApi';
...
function Fixture() {
  ...
  const workspace = useMemo<WorkspaceModel>(() => {
    ...
    return {
      notifications: notificationFixtureApi, demo: sample, status: host ? 'connected' : 'offline', error: null, host, projects: [], sessions: [], devices: [], approvals: [],
    ...
    <SafeAreaProvider><SafeAreaInsetsContext.Provider value={insets}>
      ...
      <IntegrationsProvider api={integrations} identity="ellis@example.com">
        <View testID="fixture-ground" style={{ flex: 1, backgroundColor: colors.background, paddingTop: insets.top }}>
          ...
          <SettingsSheet initialPage="notifications" visible={visible} workspace={workspace} onClose={close}
