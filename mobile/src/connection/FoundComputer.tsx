...
export const FoundComputer = memo(function FoundComputer({ computer, compact, stageRef, labelOpacity }: {
  computer: NearbyComputer; compact: boolean; stageRef?: Ref<View>; labelOpacity?: Animated.Value;
}) {
  ...
  return <View style={s.found}>
    <View ref={stageRef} collapsable={false} pointerEvents="none" aria-hidden accessibilityElementsHidden importantForAccessibility="no-hide-descendants"
      style={[s.stage, { width, height: width * 0.84 }]}>
      <FoundMachine width={width} motion={motion} logo={logo} />
    </View>
    <Animated.View accessible accessibilityLabel={`${computer.name}${family}, ready to connect`}
      style={[s.plate, { opacity: labelOpacity ? Animated.multiply(motion.settle, labelOpacity) : motion.settle,
        transform: [{ translateY: motion.settle.interpolate({ inputRange: [0, 1], outputRange: [14, 0] }) }] }]}>
      <View style={s.title}>
        {logo && <Icon name={logo} size={20} color={colors.muted} />}
        <Text numberOfLines={2} style={[s.name, { color: colors.text }]}>{computer.name}</Text>
      </View>
      <View style={s.status}>
        <View style={s.well}>
...
const s = StyleSheet.create({
  ...
  title: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, maxWidth: '100%' },
  name: { fontSize: 24, lineHeight: 29, fontWeight: '600', letterSpacing: -0.5, textAlign: 'center', flexShrink: 1 },
