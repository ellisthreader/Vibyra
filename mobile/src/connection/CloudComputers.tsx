...
export function CloudComputers({ workspace, onSelect }: { workspace: WorkspaceModel; onSelect: (computer: CloudComputer) => void }) {
  const { colors } = useTheme();
  const list = workspace.actions.listComputers;
  const signedIn = Boolean(workspace.account) && Boolean(list);
  const [listing, setListing] = useState<Listing>();
  useEffect(() => {
    if (!signedIn || !list) { setListing(undefined); return; }
    let live = true;
    const refresh = () => { list().then(next => { if (live) setListing(next); }, () => {}); };
    refresh();
    const timer = setInterval(() => { if (AppState.currentState === 'active') refresh(); }, REFRESH);
    return () => { live = false; clearInterval(timer); };
  }, [signedIn, list]);
  if (!listing?.live || !listing.computers.length) return null;
  return <View style={s.block}>
    <Text accessibilityRole="header" style={[s.title, { color: colors.text }]}>Your computers</Text>
    <Text style={[s.detail, { color: colors.muted }]}>
      {listing.entitled ? 'Signed in on these. Reach one from any network.'
        : 'Connecting from anywhere is part of Pro. On the same Wi-Fi, find your computer below.'}
    </Text>
    <View style={[s.rows, { borderTopColor: colors.border }]}>
      {listing.computers.map(computer => <Row key={computer.id} computer={computer} onPress={() => onSelect(computer)} />)}
    </View>
  </View>;
}
...
function Row({ computer, onPress }: { computer: CloudComputer; onPress: () => void }) {
  const { colors } = useTheme();
  const platform = describePlatform(computer.platform ?? undefined);
  const state = computer.online ? (computer.activeSessions ? 'Online · a phone is connected' : 'Online')
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityHint={computer.online ? undefined
    : 'Open Vibyra on that computer and keep it awake.'} accessibilityState={{ disabled: !computer.online }}
    disabled={!computer.online} onPress={onPress}
    style={({ pressed }) => [s.row, { borderBottomColor: colors.border, opacity: pressed ? 0.6 : 1 }]}>
    <Icon name={platform.icon} size={20} color={computer.online ? colors.text : colors.muted} />
    <View style={s.text}>
      <Text numberOfLines={1} style={[s.name, { color: computer.online ? colors.text : colors.muted }]}>{computer.name}</Text>
      <View style={s.stateRow}>
        <View style={[s.dot, { backgroundColor: computer.online ? colors.success : colors.border }]} />
        <Text numberOfLines={1} style={[s.state, { color: colors.muted }]}>{state}</Text>
      </View>
    </View>
    {computer.online && <Icon name="chevron-forward" size={18} color={colors.muted} />}
  </Pressable>;
}
...
const s = StyleSheet.create({
  ...
  rows: { borderTopWidth: StyleSheet.hairlineWidth, marginTop: 8 },
  row: { minHeight: 62, flexDirection: 'row', alignItems: 'center', gap: 14, borderBottomWidth: StyleSheet.hairlineWidth },
  text: { flex: 1, gap: 3 },
