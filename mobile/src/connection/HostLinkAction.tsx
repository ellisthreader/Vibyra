...
export function HostLinkAction({ workspace }: { workspace: WorkspaceModel }) {
  ...
  if (state === 'sent') {
    return <View accessibilityLiveRegion="polite" style={s.stack}>
  ...
  if (state === 'typing' || (state === 'error' && !known)) {
    return <View style={s.form}>
      <View style={[s.field, { backgroundColor: colors.background, borderColor: colors.border }]}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Send the link"
          ...
          style={({ pressed }) => [s.send, { backgroundColor: ready ? colors.action : colors.border, opacity: pressed ? 0.7 : 1 }]}>
          <Icon name="arrow-forward" size={19} color={ready ? colors.onAction : colors.muted} />
        </Pressable>
      </View>
      <Text accessibilityLiveRegion="polite" style={[s.detail, { color: state === 'error' ? colors.error : colors.muted }]}>
        {state === 'error' ? message : 'Open the email on your computer to install.'}
      </Text>
    </View>;
  }
  return <View style={s.stack}>
    <Pressable accessibilityRole="button" accessibilityLabel="Email me the download link"
      accessibilityHint={known ? `Send the Vibyra Desktop download link to ${known}` : 'Enter your email to get Vibyra Desktop on your computer'}
    ...
    {state === 'error' && <Text accessibilityLiveRegion="polite" style={[s.detail, { color: colors.error }]}>{message}</Text>}
  </View>;
}
const s = StyleSheet.create({
  stack: { gap: 8 },
  linkRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 },
  link: { fontSize: 13, lineHeight: 18, fontWeight: '500', flexShrink: 1, textAlign: 'center' },
  detail: { fontSize: 12, lineHeight: 17, textAlign: 'center' },
  form: { paddingTop: 8, gap: 8 },
  field: { minHeight: 52, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth,
    flexDirection: 'row', alignItems: 'center', paddingLeft: 12, paddingRight: 4, gap: 8 },
  input: { flex: 1, minWidth: 0, minHeight: 48, fontSize: 15, outlineWidth: 0 },
  send: { width: 44, height: 44, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
});
