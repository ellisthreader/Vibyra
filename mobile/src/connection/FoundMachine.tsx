...
export const FoundMachine = memo(function FoundMachine({ width, motion, logo, screen, screenOpacity }: {
  width: number; motion: Arrival; logo?: IconName; screen?: ReactNode; screenOpacity?: Animated.AnimatedInterpolation<number>;
}) {
  ...
  const shadow = {
    shadowColor: '#080B12', shadowOpacity: dark ? 0.55 : 0.16, shadowRadius: 18 * k, shadowOffset: { width: 0, height: 12 * k } };
  ...
  return <View pointerEvents="none" style={[s.machine, { width }]}>
    <View style={[s.light, { width: lightWidth, height: lightHeight, left: (width - lightWidth) / 2,
      top: lidHeight + deckHeight - lightHeight / 2 }]}>
      <Svg width={lightWidth} height={lightHeight}>
        <Defs><RadialGradient id="found-floor" cx="50%" cy="50%" rx="50%" ry="50%">
        ...
        <Ellipse cx={lightWidth / 2} cy={lightHeight / 2} rx={lightWidth / 2} ry={lightHeight / 2} fill="url(#found-floor)" />
      </Svg>
    </View>
    <Animated.View needsOffscreenAlphaCompositing style={{
      ...
      transform: [{ scale: rise.interpolate({ inputRange: [0, 1], outputRange: [0.88, 1], extrapolate: 'clamp' }) }] }}>
      {/* Composite the glass and bezel before rotating; the deck stays in front of the hinge. */}
      <Animated.View shouldRasterizeIOS renderToHardwareTextureAndroid style={[s.lid, shell, { height: lidHeight, marginHorizontal: 14 * k, padding: 7 * k,
        borderTopLeftRadius: 15 * k, borderTopRightRadius: 15 * k, transformOrigin: '50% 100%',
        transform: [{ perspective: 800 }, { rotateX: open }] }]}>
        {dark && <View style={[s.gloss, { left: 20 * k, right: 20 * k }]} />}
        <View style={[s.display, { borderRadius: 9 * k, backgroundColor: dark ? colors.background : colors.elevated }]}>
          {(!screen || screenOpacity) && <>
          <Animated.View style={[StyleSheet.absoluteFill, s.mark, { opacity: awake }]}>
            {logo ? <Icon name={logo} size={58 * k} color={colors.text} /> : <BrandMark size={52 * k} />}
          </Animated.View>
          <Animated.View style={[StyleSheet.absoluteFill, { backgroundColor: colors.accent, opacity: screenLight }]} />
          </>}
          {screen && <Animated.View style={[StyleSheet.absoluteFill, { opacity: screenOpacity ?? 1 }]}>{screen}</Animated.View>}
        </View>
      </Animated.View>
      <View style={[s.deck, shell, shadow, { height: deckHeight, borderBottomLeftRadius: 8 * k, borderBottomRightRadius: 8 * k }]}>
...
const s = StyleSheet.create({
  ...
  deck: { zIndex: 1, borderWidth: 1, borderTopWidth: 0, alignItems: 'center', justifyContent: 'center' },
  lip: { borderRadius: 2 },
});
