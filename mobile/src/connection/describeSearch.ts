...
export function describeSearch({
  ...
  if (denied) {
    return {
      ...
      detail:
        'iOS is blocking Vibyra from seeing your Wi-Fi. Turn on Local Network for Vibyra in Settings, then look again.',
    };
  }
  if (only) {
    return {
      title: 'Is this your computer?',
  ...
  if (computers.length === 1) {
    return {
      title: `Found ${computers[0].name}`,
      detail:
        status === 'finished' || status === 'failed'
          ? 'This computer did not become available. Check that iPhone connection is on, then search again.'
          : 'Getting it ready…',
    };
  }
  if (status === 'finished') {
    return {
      title: 'No computer found',
      detail:
        'Open Vibyra on your computer and turn on iPhone connection, then look again. Both devices need the same Wi-Fi.',
    };
  }
  if (status === 'failed') {
    return {
      title: 'Could not look for computers',
      detail: 'Check your Wi-Fi connection, then try again.',
    };
  }
  return {
    title: 'Looking for your computer',
    detail:
      elapsed > 10
