...
export interface Arrival { rise: Animated.Value; lid: Animated.Value; settle: Animated.Value }
...
export function useArrival(still: boolean): Arrival {
  ...
  useEffect(() => {
    if (!cued.current) { cued.current = true; arrived(); }
    if (still) { [rise, lid, settle].forEach(value => value.setValue(1)); return; }
    ...
    const sequence = Animated.parallel([
      Animated.timing(rise, { toValue: 1, duration: 280, easing: Easing.out(Easing.cubic), ...native }),
      Animated.sequence([Animated.delay(LID_START),
        Animated.spring(lid, { toValue: 1, damping: 11, stiffness: 60, mass: 1, ...native })]),
      Animated.sequence([Animated.delay(LID_HITS - 80),
        Animated.timing(settle, { toValue: 1, duration: 560, easing: Easing.out(Easing.cubic), ...native })]),
    ]);
    sequence.start();
    const knock = setTimeout(thud, LID_HITS);
    return () => { sequence.stop(); clearTimeout(knock); };
