...
export function PairingForm({
  ...
  const connect = async (value: string) => {
    setLink(value);
    if (await run(() => workspace.actions.connect(value.trim()))) {
      setLink('');
      onClose();
    }
  };
  return (
    <>
      <Text style={[s.intro, { color: colors.muted }]}>
        {computerName
          ? `Open the pairing code on ${computerName}.`
          : 'Open iPhone connection in Vibyra Desktop Settings, or the pairing code in Vibyra Host.'}
      </Text>
      <View style={[s.trust, { borderColor: colors.border }]}>
        ...
        <Text style={[s.trustText, { color: colors.muted }]}>
          Approve access on your computer. Desktop connections are view-only; standalone Host
          connections also allow commands.
        </Text>
      </View>
      <Pressable
      ...
      >
        ...
        <View style={s.scanText}>
          <Text style={[s.scanTitle, { color: colors.text }]}>Scan QR code</Text>
          <Text style={[s.scanDetail, { color: colors.muted }]}>Shown on your computer</Text>
        </View>
      ...
      <TextInput
        accessibilityLabel="Computer pairing link"
        value={link}
      ...
      />
      {pending && (
        <View accessibilityLiveRegion="polite" style={s.pending}>
          <ActivityIndicator color={colors.accent} />
          <Text style={[s.pendingText, { color: colors.text }]}>
            {workspace.status === 'pairing'
              ? 'Approve this phone on your computer…'
              : 'Connecting…'}
          </Text>
        </View>
      )}
      ...
      <Text style={[s.note, { color: colors.muted }]}>
        Keep your computer awake and Vibyra Desktop or Host running.
      </Text>
      <ScannerSheet
        visible={scanner}
...
const s = StyleSheet.create({
  ...
  scanText: { flex: 1, gap: 5 },
  scanTitle: { fontSize: 16, fontWeight: '500' },
  scanDetail: { fontSize: 12 },
  ...
  pending: { flexDirection: 'row', alignItems: 'center', gap: 11, paddingVertical: 5 },
  pendingText: { flex: 1, fontSize: 14, lineHeight: 21 },
