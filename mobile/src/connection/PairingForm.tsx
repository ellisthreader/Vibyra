...
export function PairingForm({ workspace, onClose, computerName }: {
  workspace: WorkspaceModel; onClose: () => void; computerName?: string;
}) {
  ...
  return <>
    <Text style={[s.intro, { color: colors.muted }]}>{computerName ? `Open the pairing code on ${computerName}.` : 'Open iPhone connection in Vibyra Desktop Settings, or the pairing code in Vibyra Host.'}</Text>
    <View style={[s.trust, { borderColor: colors.border }]}>
      <Icon name="shield-checkmark-outline" size={18} color={colors.muted} />
      <Text style={[s.trustText, { color: colors.muted }]}>Approve access on your computer. Desktop connections are view-only; standalone Host connections also allow commands.</Text>
    </View>
    ...
    <Text style={[s.note, { color: colors.muted }]}>Keep your computer awake and Vibyra Desktop or Host running.</Text>
