...
const Handoff = createContext<
  { report: (frame: MachineFrame, stage: ConnectionStage) => void } | undefined
>(undefined);
...
export function ComputerHandoff({
  computer,
  origin,
  children,
}: {
  ...
  const motion = useRef({
    rise: new Animated.Value(1),
    lid: new Animated.Value(1),
    settle: new Animated.Value(1),
  }).current;
  ...
  const report = useRef((frame: MachineFrame, next: ConnectionStage) => {
    ...
    if (
      previous &&
      (Math.abs(previous.x - frame.x) > 1 ||
        Math.abs(previous.y - frame.y) > 1 ||
        Math.abs(previous.width - frame.width) > 1)
    ) {
      progress.setValue(1);
      setDone(true);
    }
    measured.current = frame;
    setTarget((previous) => previous ?? frame);
    setStage(next);
  }).current;
  useEffect(() => {
    if (done) return;
    ...
    () => [
      {
        translateX: progress.interpolate({
          inputRange: [0, 1],
          outputRange: [0, target && from ? target.x - from.x : 0],
        }),
      },
      {
        translateY: progress.interpolate({
          inputRange: [0, 1],
          outputRange: [0, target && from ? target.y - from.y : 0],
        }),
      },
      {
        scale: progress.interpolate({
          inputRange: [0, 1],
          outputRange: [1, target && from ? target.width / from.width : 1],
        }),
      },
    ],
