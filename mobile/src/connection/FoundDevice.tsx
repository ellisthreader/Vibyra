 ...
 *  change freely. */
export function FoundDevice({ size, angle, index, still, state, live }: {
  size: number; angle: number; index: number; still: boolean; state: Readiness; live: boolean;
}) {
  const { colors, dark } = useTheme();
  const ready = state === 'ready';
  const working = state === 'pending' && live;
  const tone = ready ? colors.success : state === 'older' ? colors.warning : working ? colors.accent : colors.muted;
  ...
  const shell = { backgroundColor: dark ? '#2A2E38' : colors.surface,
    ...
    shadowColor: '#080B12', shadowOpacity: dark ? 0.5 : 0.14, shadowRadius: 9 * k,
    shadowOffset: { width: 0, height: 5 * k } };
  return <View pointerEvents="none" style={[s.box, { width: reach, height: reach },
    ringPoint(size, angle, size * 0.36, reach)]}>
    <Svg width={reach} height={reach} style={StyleSheet.absoluteFill}>
      {ready && <AnimatedCircle cx={reach / 2} cy={reach / 2} fill="none" stroke={tone}
        strokeWidth={reply.interpolate({ inputRange: [0, 1], outputRange: [2.2, 0.8] })}
        r={reply.interpolate({ inputRange: [0, 1], outputRange: [16 * k, reach / 2 - 2],
    ...
    <Animated.View style={[s.glow, { width: 84 * k, height: 84 * k,
      opacity: halo.interpolate({ inputRange: [0, 1], outputRange: ready || working ? [0.55, 1] : [0.4, 0.4] }),
      transform: [{ scale: halo.interpolate({ inputRange: [0, 1], outputRange: [0.88, 1] }) }] }]}>
    ...
    <Animated.View style={[s.laptop, { width: 52 * k, opacity: appear,
      transform: [{ scale: appear.interpolate({ inputRange: [0, 1], outputRange: [0.55, 1] }) }] }]}>
      <View style={[s.lid, shell, { marginHorizontal: 4 * k, height: 30 * k, padding: 2.5 * k,
        borderTopLeftRadius: 5 * k, borderTopRightRadius: 5 * k }]}>
        {dark && <View style={[s.gloss, { left: 6 * k, right: 6 * k }]} />}
        <View style={[s.screen, { borderRadius: 2.5 * k, backgroundColor: dark ? colors.background : colors.elevated }]}>
          <View style={[StyleSheet.absoluteFill, { backgroundColor: tone, opacity: ready || working ? 0.14 : 0.05 }]} />
          <View style={{ width: 7 * k, height: 7 * k, borderRadius: 3.5 * k, backgroundColor: tone,
            shadowColor: tone, shadowOpacity: 0.9, shadowRadius: 5 * k, shadowOffset: { width: 0, height: 0 } }} />
        </View>
      </View>
      <View style={[s.deck, shell, { height: 5 * k, borderBottomLeftRadius: 3.5 * k,
        borderBottomRightRadius: 3.5 * k, shadowOpacity: 0 }]}>
        <View style={[s.lip, { width: 14 * k, height: 1.5 * k, backgroundColor: colors.border }]} />
      </View>
    </Animated.View>
  </View>;
}
