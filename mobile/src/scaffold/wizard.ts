  ...
  switch (action.type) {
      ...
      return go(
        {
          ...
          browsing: false,
          extraIds: [],
          templateId: action.kind === 'empty' ? 'empty' : null,
        },
    ...
    case 'chooseTemplate': {
      ...
      const next = {
        ...
        extraIds: state.extraIds.filter((id) => id !== action.templateId),
    ...
    case 'toggleExtra': {
      ...
      return {
        ...state,
        extraIds: on
          ? state.extraIds.filter((id) => id !== action.templateId)
          : [...state.extraIds, action.templateId],
      };
    }
    case 'continue':
      return go({ ...state, browsing: false }, stepAfterStack());
    case 'browseAll':
      return { ...state, browsing: action.on };
    case 'setOptions':
    ...
    case 'preflight': {
      ...
      const name =
        state.name === 'untitled' ? suggestedName(action.projectPaths, parent) : state.name;
    ...
    case 'reset':
      return initialWizard(action.runId);
  }
}
