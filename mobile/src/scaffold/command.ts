...
export function buildScaffoldRequest(
  ...
  const wanted = chosen.flatMap((pick) =>
    pick.steps.filter((step) => step.phase === 'create' || options.install),
  );
  const steps = quicken(
    wanted.map((step) => ({
      label: step.label,
  ...
  return {
    ...
    createDir,
    seeds: chosen
      .flatMap((pick) => pick.seeds)
      .map((seed) => ({ path: seed.path, body: fill(seed.body, slug) })),
    steps,
    gitInit: options.git,
  };
}
