import { parentOf, pascalCase, slugify } from './destination';
import type { ProjectTemplate, TemplateOptions } from './types';
...
export function buildScaffoldRequest(entry: ProjectTemplate, dir: string, options: TemplateOptions): ScaffoldRequest {
  ...
  return { dir, createDir, seeds: entry.seeds.map(seed => ({ path: seed.path, body: fill(seed.body, slug) })),
