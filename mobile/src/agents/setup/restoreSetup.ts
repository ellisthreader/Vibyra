...
function validFields(value: unknown): value is TeammateFields {
  ...
  return typeof f.name === 'string' && typeof f.brief === 'string' && typeof f.memory === 'string'
    && typeof f.avatar === 'string' && typeof f.budget === 'number' && Number.isFinite(f.budget) && strings(f.integrations);
}
...
export function restoreSetup(raw: string | null, legacy: string | null): SetupDraft {
  ...
  if (!value || ![1, 2].includes(value.version) || !['job', 'memory', 'tools', 'routine', 'review', 'identity', 'budget'].includes(value.step)
    || !validFields(value.fields) || !strings(value.routines) || value.routines.length > 2
    || !['text', 'job', 'routineSuggestion'].every(key => typeof value[key] === 'string')
    || value.requestedTools !== undefined && typeof value.requestedTools !== 'string') throw new Error('Invalid setup');
  if (value.pending !== undefined && !validPending(value.pending)) throw new Error('Invalid pending request');
  if (!value.pending && legacy) {
    const previous = JSON.parse(legacy); const pending = { ...previous, routines: [] };
    if (!validPending(pending)) throw new Error('Invalid legacy request');
    value.pending = pending;
  }
  if (value.pending) { value.fields = value.pending.fields; value.step = 'review'; }
  else if (value.version === 1) {
    if (value.step === 'memory') value.text = [value.fields.memory, value.text].filter(Boolean).join('\n');
    if (value.step === 'routine' && value.text.trim()) { value.routines = [value.text]; value.text = ''; }
  }
