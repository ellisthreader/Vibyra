...
export function SetupWelcome({
  ...
  disabled: boolean;
  onChoose(job: string): void;
}) {
  const { colors } = useTheme();
  const [ideas, setIdeas] = useState(false);
  return (
    <View style={s.body}>
      <Text style={[s.section, { color: colors.muted }]}>Start from a role</Text>
      <View style={s.list}>
        {starters.map((item) => (
          <Pressable
            key={item.label}
          ...
          >
            <TeammateAvatar avatar={item.avatar} size={44} />
            <View style={s.grow}>
      ...
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={ideas ? 'Fewer ideas' : 'More job ideas'}
      ...
      >
        <Text style={{ color: colors.accent, fontSize: 14 }}>
          {' '}
          {ideas ? 'Fewer ideas' : 'More job ideas'}
        </Text>
        <Icon name={ideas ? 'chevron-up' : 'chevron-down'} size={14} color={colors.accent} />
      </Pressable>
      {ideas && (
        <View style={s.ideas}>
          {setupIdeas.map(([label, job]) => (
            <Pressable
              key={label}
              accessibilityRole="button"
              accessibilityLabel={label}
              disabled={disabled}
              onPress={() => onChoose(job)}
              style={[s.idea, { backgroundColor: colors.elevated }]}
            >
...
const s = StyleSheet.create({
  ...
  grow: { flex: 1 },
  label: { ...font.row, fontSize: 16 },
  detail: { ...font.footnote, marginTop: 2 },
  more: {
    minHeight: 44,
    flexDirection: 'row',
    alignItems: 'center',
