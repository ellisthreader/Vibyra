...
export function RoutineDrafts({ identity, id }: { identity: string; id: string }) {
  ...
  useEffect(() => {
    let active = true;
    void readFlag(routinePlanKey(identity, id))
      .then((raw) => {
        const value = raw ? (JSON.parse(raw) as RoutinePlan) : null;
        if (value && (value.status !== 'draft' || !Array.isArray(value.routines)))
