...
export function useRoster(api: AgentsApi, signedIn: boolean, active = true) {
  ...
  const refresh = useCallback(async () => {
    ...
    try { const result = await api.list(); if (alive.current) { setRoster(result); setError(null); } }
    catch (e) { if (alive.current) setError(e instanceof Error ? e.message : 'Could not load teammates.'); }
    finally { pending.current = false; if (alive.current) setLoading(false); }
  }, [api, signedIn]);
  useEffect(() => {
    alive.current = true; if (!active) return () => { alive.current = false; }; void refresh();
  ...
  const adopt = (teammate: Teammate) => {
    setRoster(current => current && ({ ...current, teammates: [teammate, ...current.teammates.filter(a => a.id !== teammate.id)] }));
    void refresh();
  };
