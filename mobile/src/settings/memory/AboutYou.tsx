...
export function AboutYou({
  ...
  const drafts = {
    name: useTextDraft(store, state, 'name'),
    occupation: useTextDraft(store, state, 'occupation'),
  ...
  const reveal = (part: Part) => {
    onTouch();
    setTimeout(() => onReveal(top.current + (at.current[part] ?? 0)), 280);
  };
  const field = (part: Part) => ({
    on: preferences[`${part}Enabled`],
    disabled: !memoryOn,
    onSwitch: (on: boolean) => {
      onTouch();
      void store.setSwitch(`${part}Enabled`, on);
    },
    onLayout: (event: { nativeEvent: { layout: { y: number } } }) => {
      at.current[part] = event.nativeEvent.layout.y;
    },
  });
  return (
    <>
      ...
      >
        <Group>
          <ProfileField
            ...
            {...field('name')}
            draft={drafts.name}
            max={PROFILE_MAX.name}
            placeholder="What should Vibyra call you?"
            onFocus={() => reveal('name')}
          />
          <ProfileField
            title="Occupation"
            {...field('occupation')}
            draft={drafts.occupation}
            max={PROFILE_MAX.occupation}
            placeholder="What do you do?"
            onFocus={() => reveal('occupation')}
          />
          <ProfileField
            title="More about you"
            {...field('about')}
            draft={drafts.about}
            max={PROFILE_MAX.about}
            countFrom={1300}
            multiline
            placeholder="Interests, values or preferences to keep in mind"
            onFocus={() => reveal('about')}
          />
