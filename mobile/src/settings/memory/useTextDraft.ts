 ...
 */
export function useTextDraft(store: Personalization | null, state: PersonalizationState, field: TextField) {
  ...
  useEffect(() => () => { if (pending.current !== null) void store?.saveText(field, pending.current); }, [store, field]);
  return {
    value: draft ?? state.preferences?.[field] ?? '',
    focused,
    saving: state.busy === field,
    saved: saved && !focused,
    change: (text: string) => { setDraft(text); setSaved(false); store?.clearError(); },
