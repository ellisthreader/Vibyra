...
export function SavedMemories({
  ...
  const clear = () => {
    ...
    confirmAction(
      'Clear all memories?',
      'Vibyra will forget everything on this list. This can’t be undone.',
      'Clear all',
      () => void store.clearMemories(),
    );
  };
  return (
    <>
        ...
        {memories.map((memory) => (
          <View key={memory.id} style={s.memory}>
            <View style={s.words}>
              ...
              {memory.source === 'chat' && (
                <Text style={[s.source, { color: colors.muted }]}>From a chat</Text>
              )}
            </View>
            <Pressable
              accessibilityRole="button"
              accessibilityLabel={`Remove memory: ${memory.text}`}
              hitSlop={10}
              onPress={() => {
                onTouch();
                void store.removeMemory(memory.id);
              }}
              style={({ pressed }) => [s.remove, { opacity: pressed ? 0.5 : 1 }]}
            >
        ...
        {adding ? (
          <View style={s.addRow}>
            <TextInput
              ...
              returnKeyType="done"
              placeholder="Something Vibyra should remember"
              ...
              onFocus={() => setTimeout(onFocusEnd, 280)}
              style={[s.addInput, { color: colors.text }]}
            />
            {text.length >= COUNT_FROM && (
              <Text style={[s.count, { color: colors.muted }]}>{MEMORY_MAX - text.length}</Text>
            )}
      ...
      {memories.length > 0 && (
        <Pressable
          ...
          onPress={clear}
          disabled={state.busy === 'clear'}
          style={({ pressed }) => [s.clear, { opacity: pressed ? 0.6 : 1 }]}
        >
