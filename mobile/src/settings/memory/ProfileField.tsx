...
export function ProfileField({
  ...
  return (
    <View style={[s.item, disabled ? s.dim : null]} onLayout={onLayout}>
      <View style={s.head}>
        <Text style={[s.title, { color: colors.text }]}>{title}</Text>
        {status === 'saving' ? (
          <ActivityIndicator size="small" color={colors.muted} />
        ) : status === 'saved' ? (
          <Text accessibilityLiveRegion="polite" style={[s.note, { color: colors.muted }]}>
            Saved
          </Text>
        ) : null}
        <Switch
          accessibilityLabel={`Use ${title.toLowerCase()}`}
          value={on}
          disabled={disabled}
          onValueChange={onSwitch}
          trackColor={{ true: colors.action, false: colors.border }}
          thumbColor={on ? colors.onAction : '#FFFFFF'}
      ...
      {count ? (
        <Text style={[s.count, { color: colors.muted }]}>
          {draft.value.length.toLocaleString('en-GB')} / {max.toLocaleString('en-GB')}
        </Text>
      ) : null}
    </View>
  );
