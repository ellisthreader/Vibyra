...
export function SummaryPage({ workspace, nav }: SettingsPageProps) {
  ...
  return <View style={[s.page, { paddingBottom: Math.max(bottom, 12) + 8 }]}>
    <View style={[s.card, { backgroundColor: colors.surface, borderColor: draft.focused ? colors.accent : colors.border }]}>
      <TextInput ref={input} value={draft.value} multiline maxLength={PROFILE_MAX.summary} textAlignVertical="top"
        accessibilityLabel="Memory summary" autoFocus={!draft.value} placeholderTextColor={colors.muted}
        placeholder={'Everything Vibyra should know about you, in your own words: who you are, what you’re working on, '
          + 'the tools you use, how you like to learn, and the people and dates worth remembering.'}
        onChangeText={draft.change} onFocus={draft.focus} onBlur={() => void draft.commit()}
        style={[s.input, { color: colors.text }]} />
      <View style={s.foot}>
        <Text style={[s.count, { color: colors.muted }]}>
          {draft.value.length.toLocaleString('en-GB')} / {PROFILE_MAX.summary.toLocaleString('en-GB')}</Text>
        <View style={s.status}>
          {draft.saving ? <ActivityIndicator size="small" color={colors.muted} />
            : draft.saved ? <Text accessibilityLiveRegion="polite" style={[s.saved, { color: colors.muted }]}>Saved</Text> : null}
          {draft.focused && <Pressable accessibilityRole="button" accessibilityLabel="Done" hitSlop={8} onPress={() => input.current?.blur()}>
    ...
    {!draft.focused && <Footnote>{!memoryEnabled ? 'Memory is off, so Vibyra isn’t reading this. Turn it on in Memory.'
      : !summaryEnabled ? 'This summary is switched off, so Vibyra isn’t reading it. Switch it on in Memory.'
        : workspace.demo ? sampleNote(workspace) : 'Vibyra reads this in every phone chat.'}</Footnote>}
  </View>;
}
const s = StyleSheet.create({
  page: { flex: 1, paddingHorizontal: 20, paddingTop: 4 },
  card: { flex: 1, minHeight: 180, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 10 },
