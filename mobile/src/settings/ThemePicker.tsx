          ...
          return (
            ...
            >
              ...
              <View style={s.caption}>
                ...
                <Text style={[s.captionText, { color: on ? colors.text : colors.muted }]}>
                  {item.title}
                </Text>
              </View>
            </Pressable>
...
function Preview({ mode, accent }: { mode: ThemePreference; accent: AccentId }) {
  ...
  const line =
    mode === 'light' ? light.border : mode === 'dark' ? dark.border : 'rgba(128,136,150,0.55)';
  const pill = accents[accent][mode === 'light' ? 'light' : 'dark'].accent;
  return (
    <View
      style={[
        s.preview,
        { backgroundColor: ground, borderColor: mode === 'light' ? light.border : dark.border },
      ]}
    >
      {mode === 'system' && (
...
const s = StyleSheet.create({
  tiles: { flexDirection: 'row', gap: 12, padding: 16, paddingBottom: 14 },
  tile: { flex: 1, alignItems: 'center', gap: 8 },
  ring: { alignSelf: 'stretch', borderRadius: 14, borderWidth: 2, padding: 3 },
  preview: {
    height: 74,
    borderRadius: 9,
  ...
  captionText: { fontSize: 13, fontWeight: '500', letterSpacing: -0.1 },
