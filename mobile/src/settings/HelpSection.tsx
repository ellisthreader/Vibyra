...
export function HelpSection({ workspace }: { workspace: WorkspaceModel }) {
  ...
  const confirmLogOut = () => confirmAction('Log out of Vibyra?', 'Your chats stay with your account. Sign in again to pick them back up.',
    'Log out', () => void run(() => workspace.actions.logOut!()));
  return <>
    <Label>Help</Label>
    <Group>
    ...
    {mailFailed && <View style={s.notice}><Hint>{`No mail app opened. Write to ${SUPPORT_EMAIL}.`}</Hint></View>}
    {logOut && <Group style={s.logOut}>
      <Row title={busy ? 'Logging out…' : 'Log out'} danger busy={busy} onPress={confirmLogOut} trailing="none" />
    </Group>}
    {error && <View style={s.notice}><Hint error>{error}</Hint></View>}
    <View style={s.footer}>
      <Text style={[s.version, { color: colors.muted }]}>{`Vibyra ${appVersion()}`.trim()}</Text>
      <View style={s.legal}>
        <Text accessibilityRole="link" onPress={() => open(links.terms)} style={[s.version, { color: colors.accent }]}>Terms</Text>
        <Text accessibilityRole="link" onPress={() => open(links.privacy)} style={[s.version, { color: colors.accent }]}>Privacy</Text>
      </View>
    </View>
  </>;
}
