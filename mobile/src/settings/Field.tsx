 ...
 */
export function Field({ label, status, ...input }: TextInputProps & {
  /** Spoken name of the box; the visible label is the card's heading. */
  label: string; status?: 'saving' | 'saved' | null;
}) {
  const { colors } = useTheme();
  return <View style={s.row}>
    <TextInput accessibilityLabel={label} placeholderTextColor={colors.muted} {...input}
    ...
    {status === 'saving' ? <ActivityIndicator size="small" color={colors.muted} />
      : status === 'saved' ? <Text accessibilityLiveRegion="polite" style={[s.saved, { color: colors.muted }]}>Saved</Text> : null}
  </View>;
}
