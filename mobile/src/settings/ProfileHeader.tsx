 ...
 */
export function ProfileHeader({ workspace, onOpenProfile }: { workspace: WorkspaceModel; onOpenProfile?: () => void }) {
  ...
  const title = shown ? shown.name.trim() || shown.email : workspace.demo ? 'Sample account' : 'Not signed in';
  const subtitle = workspace.demo && !workspace.account ? 'Nothing here is saved' : shown?.name.trim() ? shown.email : null;
  // The plan the wallet knows, else the account's; the sample's stand-in plan is Free.
  const id = wallet?.plan ?? shown?.plan ?? null;
  const plan = !id ? null : id === 'free' || id === 'sample' ? { name: 'Free', paid: false } : { name: titled(id), paid: true };
  const label = ['Profile', title, subtitle, plan?.name].filter(Boolean).join(', ');
  return <ProfilePhoto workspace={workspace} plan={plan} label={label} onOpen={onOpenProfile}>
    <View style={s.words}>
      <Text accessibilityRole="header" numberOfLines={1} style={[s.name, { color: colors.text }]}>{title}</Text>
      {subtitle && <Text numberOfLines={1} style={[s.email, { color: colors.muted }]}>{subtitle}</Text>}
      {test && <Text numberOfLines={1} style={[s.tag, { color: colors.muted }]}>Test account · nothing here is saved</Text>}
    </View>
  </ProfilePhoto>;
}
const s = StyleSheet.create({
  ...
  email: { fontSize: 14, lineHeight: 19 },
