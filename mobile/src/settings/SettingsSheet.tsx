 ...
 */
export function SettingsSheet({ visible, workspace, onClose, routes, initialPage }: {
  visible: boolean; workspace: WorkspaceModel; onClose: () => void; routes: SettingsRoutes;
  ...
  const [stack, setStack] = useState<Layer[]>(() => (visible && initialPage && settingsPages[initialPage] ? ['home', initialPage] : ['home']));
  ...
  const current = useRef(stack); current.current = stack;
  const latest = useRef({ onClose, reduced }); latest.current = { onClose, reduced };
  const nav = useMemo<SettingsNav>(() => {
    const run = (to: number, done?: () => void) => Animated.timing(motion, { toValue: to,
      duration: latest.current.reduced ? 150 : 260, useNativeDriver: Platform.OS !== 'web',
    ...
    return {
      push: page => {
        ...
        setPushing(true); setLeaving(null); motion.setValue(0);
        setStack([...current.current, page]); run(1);
      },
      back: () => {
        const stackNow = current.current;
        if (stackNow.length < 2) return;
        setPushing(false); setLeaving(stackNow[stackNow.length - 1]!); motion.setValue(1);
        setStack(stackNow.slice(0, -1)); run(0, () => setLeaving(null));
      },
      close: then => { latest.current.onClose(); then?.(); },
  ...
  useEffect(() => {
    ...
    motion.stopAnimation(); motion.setValue(1); setLeaving(null); setPushing(true);
  ...
  const reset = () => { motion.stopAnimation(); motion.setValue(1); setLeaving(null); setPushing(true); setStack(['home']); };
  return <OverlaySheet visible={visible} title={title} label="Settings" testID="settings-sheet" onClose={onClose}
    onBack={stack.length > 1 ? nav.back : undefined} onClosed={reset}>
    <View style={s.stage} onLayout={event => setWidth(event.nativeEvent.layout.width)}>
      {layers.map(id => {
        ...
        const style = !shown ? { opacity: 0 }
          : isMoving ? (reduced ? { opacity: motion }
            : { transform: [{ translateX: motion.interpolate({ inputRange: [0, 1], outputRange: [width, 0] }) }] })
            : isUnder ? (reduced ? { opacity: motion.interpolate({ inputRange: [0, 1], outputRange: [1, 0] }) }
              : { opacity: motion.interpolate({ inputRange: [0, 1], outputRange: [1, 0.6] }),
                transform: [{ translateX: motion.interpolate({ inputRange: [0, 1], outputRange: [0, -width * 0.3] }) }] })
              : null;
        const Page = id === 'home' ? null : settingsPages[id]?.Page;
        return <Animated.View key={id} pointerEvents={active ? 'auto' : 'none'} aria-hidden={!active}
          ...
          {id === 'home' ? <SettingsHome workspace={workspace} nav={nav} routes={routes} />
            : Page ? <Page workspace={workspace} nav={nav} routes={routes} /> : null}
        </Animated.View>;
      })}
    </View>
    <AccountSheet visible={accountForm !== null} mode={accountForm ?? 'login'} workspace={workspace} onClose={() => setAccountForm(null)} />
  </OverlaySheet>;
}
