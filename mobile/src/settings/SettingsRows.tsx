import { styles as s } from './SettingsRowsStyles';
...
/**
 * The parts every Settings page is built from. Text first: a quiet muted heading,
 * aligned with the rows' text, names each card; rows carry no icon column, and the value on the right
...
export function Label({ children, first }: { children: ReactNode; first?: boolean }) {
  ...
  return (
    <Text accessibilityRole="header" style={[s.label, first && s.first, { color: colors.muted }]}>
      {children}
    </Text>
  );
}
...
export function Group({
  ...
  return (
    <View style={[s.group, { backgroundColor: colors.surface, borderColor: colors.border }, style]}>
      {rows.map((row, index) => (
        <Fragment key={row.key ?? index}>
          {index > 0 && (
            <View style={[s.rule, { marginLeft: inset, backgroundColor: colors.border }]} />
          )}
          {row}
        </Fragment>
      ))}
...
export function Row({
  ...
  const content = (
    <>
      ...
      <View style={[s.text, danger && s.center]}>
        <Text
          numberOfLines={2}
          style={[s.title, { color: danger ? colors.error : colors.text }, danger && s.dangerTitle]}
        >
          {title}
        </Text>
      ...
      {value ? (
        <View style={s.valueRow}>
          ...
          <Text numberOfLines={1} style={[s.value, { color: colors.muted }]}>
            {value}
          </Text>
        </View>
      ) : null}
  ...
  if (!onPress)
    return (
      <View testID={testID} style={[s.row, detail && s.tall]}>
        {content}
      </View>
    );
  return (
    <Pressable
      ...
      accessibilityLabel={label ?? (value ? `${title}, ${value}` : title)}
    ...
    <Row
      ...
      right={
        <Switch
          ...
          thumbColor={value ? colors.onAction : '#FFFFFF'}
