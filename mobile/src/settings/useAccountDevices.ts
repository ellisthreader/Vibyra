...
interface ListState { devices: AccountDevice[] | null; error: string | null; loading: boolean }
interface Shared { identity: string; state: ListState; listeners: Set<() => void> }
...
function listFor(load: Loader, identity: string) {
  ...
  if (!held || held.identity !== identity) lists.set(load, held = { identity, state: empty, listeners: new Set() });
  return held;
}
...
export function useAccountDevices(workspace: WorkspaceModel) {
  ...
  const reload = useCallback(async () => {
    ...
    try { const list = await load(); set(shared, { devices: list, loading: false }); }
    catch (reason) { set(shared, { loading: false, error: reason instanceof Error ? reason.message : 'Your devices could not be loaded.' }); }
  }, [shared, load]);
  useEffect(() => { void reload(); }, [reload]);
  return { ...state, reload, available: Boolean(shared),
    drop: (id: string) => { if (shared) set(shared, { devices: shared.state.devices?.filter(item => item.id !== id) ?? null }); } };
}
