...
const valid = (iso: string | null | undefined) => (iso && !Number.isNaN(Date.parse(iso)) ? iso : null);
const long = (iso: string) => new Date(iso).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
 ...
 */
export function subscriptionState(account: Account | null,
  wallet: { plan: string; paidUntil: string | null } | null): SubscriptionView {
  const id = wallet?.plan ?? account?.plan ?? 'free';
  const paid = id !== 'free' && id !== 'sample';
  if (!paid) return { plan: 'Free', paid, state: null, billing: null, cycle: null, manage: null, row: 'Free' };
  const stripe = account?.billingProvider === 'stripe';
  const ends = account?.membershipCancelAtPeriodEnd ? valid(account.membershipEndsAt) ?? valid(account.planRenewsAt) : null;
  ...
  return {
    plan, paid,
    state: ends ? `Ends on ${long(ends)}` : renews ? `Renews on ${long(renews)}` : until ? `Paid through ${long(until)}` : null,
    ...
    row: ends ? `Ends ${short(ends)}` : renews ? `Renews ${short(renews)}` : until ? `Until ${short(until)}` : plan,
  };
}
