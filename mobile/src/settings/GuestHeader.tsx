 ...
 */
export function GuestHeader({ onSignUp, onSignIn }: { onSignUp?: () => void; onSignIn?: () => void }) {
  const { colors, dark } = useTheme();
  return <View style={s.header}>
    <Light strength={dark ? 1 : 0.45} ground={colors.rail} />
    <BrandMark size={52} />
    <Text accessibilityRole="header" style={[s.title, { color: colors.text }]}>Make Vibyra yours</Text>
    <Text style={[s.subtitle, { color: colors.muted }]}>Sign up free with Apple, Google or email.</Text>
    <View style={s.perks}>
      {perks.map(perk => <View key={perk.text} style={s.perk}>
        <Icon name={perk.icon} size={19} color={colors.accent} />
        <Text style={[s.perkText, { color: colors.text }]}>{perk.text}</Text>
      </View>)}
    </View>
    {onSignUp && <View style={[s.lift, { shadowColor: colors.accent }]}>
      <Button title="Create free account" onPress={onSignUp} />
    </View>}
    {onSignIn && <View style={s.signIn}>
      <Text style={[s.signInLead, { color: colors.muted }]}>Already have an account?</Text>
      <Pressable accessibilityRole="button" accessibilityLabel="Sign in" hitSlop={12} onPress={onSignIn}
...
function Light({ strength, ground }: { strength: number; ground: string }) {
  return <View pointerEvents="none" aria-hidden accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={s.light}>
    <Image accessible={false} source={require('../../assets/glow-cobalt.png')} style={[s.cobalt, { opacity: 0.34 * strength }]} />
    <Image accessible={false} source={require('../../assets/glow-sky.png')} style={[s.sky, { opacity: 0.2 * strength }]} />
    <Svg style={s.fade} width="100%" height="100%" preserveAspectRatio="none">
      <Defs><LinearGradient id="guest-light-fade" x1="0" y1="0" x2="0" y2="1">
...
const s = StyleSheet.create({
  ...
  cobalt: { position: 'absolute', width: 480, height: 480, top: -200, left: '50%', marginLeft: -330 },
  ...
  title: { marginTop: 18, fontSize: 26, lineHeight: 31, fontWeight: '600', letterSpacing: -0.7, textAlign: 'center' },
  ...
  perkText: { fontSize: 15.5, letterSpacing: -0.2 },
  lift: { alignSelf: 'stretch', marginTop: 26, borderRadius: 18,
    shadowOpacity: 0.35, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 5 },
  signIn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap', gap: 5, minHeight: 44, marginTop: 6 },
