 ...
 */
export function AccountSection({ workspace, nav }: { workspace: WorkspaceModel; nav: SettingsNav }) {
  ...
  const deletable = Boolean(account && (isTestAccount(workspace) || (!workspace.demo && workspace.actions.deleteAccount)));
  ...
  return <>
    ...
    <Group>
      ...
      {account && <Row title="Subscription" value={subscriptionState(account, wallet).row} onPress={() => nav.push('subscription')} />}
      {manage && <Row title="Security" value={devices ? `${devices.length} device${devices.length === 1 ? '' : 's'}` : null}
        onPress={() => nav.push('security')} />}
      {canCreate && <Row title="Create account" onPress={() => nav.signIn('signup')} />}
    </Group>
    {deletable && <Group style={s.delete}>
