 ...
 */
export function CodePrompt({ title, detail, action, danger, busy, error, onSubmit, onCancel }: {
  title: string; detail: string; action: string; danger?: boolean; busy: boolean; error: string | null;
  ...
  return <View>
    ...
    {recovery ? <Group>
      <Field label="Recovery code" value={code} onChangeText={setCode} placeholder="abcde-12345"
        autoCapitalize="none" autoCorrect={false} autoFocus editable={!busy}
        onSubmitEditing={() => onSubmit(code)} returnKeyType="done" />
    </Group> : <CodeInput value={code} onChange={setCode} onComplete={onSubmit} busy={busy} autoFocus />}
    <View style={s.status} accessibilityLiveRegion="polite">
      {busy ? <ActivityIndicator size="small" color={colors.muted} accessibilityLabel="Checking your code" />
        : error ? <Hint error>{error}</Hint> : <Text style={[s.detail, { color: colors.muted }]}>{detail}</Text>}
    </View>
    <Pressable accessibilityRole="button" accessibilityLabel={recovery ? 'Use your authenticator app instead' : 'Use a recovery code instead'}
      onPress={swap} disabled={busy} style={s.swap}>
      <Text style={[s.swapText, { color: colors.accent }]}>
        {recovery ? 'Use your authenticator app instead' : 'Use a recovery code instead'}</Text>
    </Pressable>
    <View style={s.actions}>
      <Button title={action} danger={danger} busy={busy} disabled={busy || !code.trim()} onPress={() => onSubmit(code)} />
