    ...
    <View>
      <View style={[s.hero, { backgroundColor: colors.accentSoft }]}>
        ...
        <Text style={[s.heroLine, { color: colors.muted }]}>
          With this on, your password alone can’t open your account. Signing in on a new device also
          asks for a six-digit code from an app on your phone — one that changes every thirty
          seconds and never travels over email or SMS.
        </Text>
      </View>
      <View style={s.points}>
        {[
          [
            'flash-outline',
            'Set up in one tap',
            'Vibyra opens your authenticator app with this account ready to add.',
          ],
          [
            'wifi-outline',
            'Works without signal',
            'The codes are worked out on your phone, so a plane or a dead zone changes nothing.',
          ],
          [
            'key-outline',
            'Ten recovery codes',
            'Kept for the day the phone with the app isn’t in your hand.',
          ],
        ].map(([icon, title, line]) => (
...
export function TwoFactorOn({
  detail,
  onRenew,
  onDisable,
}: {
  detail: TwoFactorState;
  onRenew: () => void;
  onDisable: () => void;
}) {
  const { colors } = useTheme();
    ...
    <View>
      ...
      <Group>
        ...
        <Row
          ...
          detail={
            left === 0
              ? 'None left. Get a new set before you need one.'
              : left <= 3
                ? 'Running low. A new set retires the old ones.'
                : undefined
          }
        />
      </Group>
  ...
  return (
    <View style={s.elsewhere}>
      <Text style={[s.heroTitle, { color: colors.text }]}>
        Your security settings couldn’t be loaded
      </Text>
...
export function TwoFactorElsewhere({ provider }: { provider: string }) {
  return (
    <TwoFactorNote
      title={`Managed by ${provider}`}
      line={
        `You sign in to Vibyra with ${provider}, so ${provider} is where a second step belongs.` +
 ...
 *  `note` is the quiet line under it (the sample's), centred with the rest. */
export function TwoFactorNote({
  title,
  line,
  note,
}: {
  ...
  return (
    <View style={s.elsewhere}>
      <View style={[s.tile, { backgroundColor: colors.elevated, borderColor: colors.border }]}>
        <Icon name="shield-checkmark-outline" size={24} color={colors.text} />
      </View>
      <Text style={[s.heroTitle, s.noteTitle, { color: colors.text }]}>{title}</Text>
      <Text style={[s.heroLine, s.noteLine, { color: colors.muted }]}>{line}</Text>
      {note && <Text style={[s.pointLine, s.noteFoot, { color: colors.muted }]}>{note}</Text>}
    </View>
  );
}
const s = StyleSheet.create({
  ...
  notice: { marginBottom: 12, marginHorizontal: 16 },
  actions: { marginTop: 20 },
  danger: { marginTop: 24 },
  elsewhere: { alignItems: 'center', gap: 8, paddingTop: 48, paddingHorizontal: 12 },
  tile: {
    width: 56,
    height: 56,
    borderRadius: 16,
  ...
  noteTitle: { fontSize: 21, lineHeight: 27, letterSpacing: -0.4 },
  noteLine: { fontSize: 15, lineHeight: 22, letterSpacing: -0.2, maxWidth: 330 },
  noteFoot: { textAlign: 'center', marginTop: 10 },
