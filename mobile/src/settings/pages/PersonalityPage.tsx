import { styles as s } from './PersonalityPageStyles';
...
const descriptions: Record<PersonalityStyle, string> = {
  balanced: 'Clear answers with a short why.',
  concise: 'Just the answer and the code.',
  detailed: 'Step by step, with the trade-offs.',
  friendly: 'Warm and encouraging, in plain words.',
};
...
export function PersonalityPage({ workspace, nav }: SettingsPageProps) {
  ...
  useEffect(
    () => () => {
      if (pending.current !== null) void store?.saveInstructions(pending.current);
    },
    [store],
  );
  ...
  return (
    ...
    >
        ...
        {personalityStyles.map((style) => {
          ...
          return (
            <Pressable
              ...
              accessibilityLabel={styleNames[style]}
              accessibilityHint={descriptions[style]}
              aria-checked={on}
      ...
      >
        ...
        />
        {(focused || saved || state.busy === 'instructions' || text.length >= COUNT_FROM) && (
          <View style={s.foot}>
            {text.length >= COUNT_FROM ? (
              <Text style={[s.count, { color: colors.muted }]}>
                {text.length.toLocaleString()} / {INSTRUCTIONS_MAX.toLocaleString()}
              </Text>
            ) : (
              <View />
            )}
            <View style={s.status}>
              {state.busy === 'instructions' ? (
      ...
      <Footnote>
        Used in chats on this phone. Computer agents like Claude Code and Codex keep their own
        settings.
      </Footnote>
