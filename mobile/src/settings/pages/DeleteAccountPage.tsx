...
export function DeleteAccountPage({ workspace, nav }: SettingsPageProps) {
  ...
  if (isTestAccount(workspace))
    return <TestAccount bottom={bottom} onLeave={() => nav.close(workspace.actions.exitDemo)} />;
  ...
  const run = () => {
    ...
    setBusy(true);
    setError(null);
    setWaiting(Boolean(reauth));
    void remove(provider ? { provider } : { password }, controller.signal)
      .then(
        (deleted) => {
  ...
  const confirm = () =>
    confirmAction(
      'Delete your account?',
      'Your chats, Vibes, plugin connections and photo are deleted for good. This can’t be undone.',
      'Delete account',
      run,
    );
  return (
    ...
    >
      <Text style={[s.lead, { color: colors.text }]}>
        Deleting your account removes your chats, your Vibes and plan balance, your plugin
        connections and your profile photo. It can’t be undone.
      </Text>
      <Group style={s.first}>
        <Row
          title="Manage subscriptions"
          trailing="external"
          onPress={() => void Linking.openURL(SUBSCRIPTIONS)}
        />
      </Group>
      <Footnote>
        Deleting your account doesn’t cancel an App Store subscription. Cancel it there first.
      </Footnote>
      {reauth ? (
        <>
          ...
          <Group>
            <Row
              title={
                waiting
                  ? `Finish in the ${reauth === 'apple' ? 'Apple' : 'browser'} window`
                  : `Continue with ${providerNames[reauth]}`
              }
              icon={reauth === 'apple' ? 'logo-apple' : 'logo-google'}
              busy={busy}
              onPress={confirm}
              trailing="none"
            />
            {waiting ? (
              <Row title="Cancel" onPress={() => attempt.current?.abort()} trailing="none" />
            ) : null}
          </Group>
        </>
      ) : provider ? null : (
...
function TestAccount({ bottom, onLeave }: { bottom: number; onLeave?: () => void }) {
  ...
  return (
    ...
    >
      <Text style={[s.lead, { color: colors.text }]}>
        This is the test account, so it can’t be deleted. Nothing in it is saved, and leaving the
        sample workspace clears it.
      </Text>
      {onLeave && (
        <Group style={s.first}>
          <Row title="Leave the sample workspace" onPress={onLeave} trailing="none" />
        </Group>
      )}
...
const s = StyleSheet.create({
  ...
  lead: { fontSize: 15, lineHeight: 22, marginTop: 8, marginHorizontal: 16 },
  first: { marginTop: 20 },
