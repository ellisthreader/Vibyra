...
const said = (error: unknown) => error instanceof Error ? error.message : 'That could not be saved. Try again.';
const month = (iso?: string) => (iso && !Number.isNaN(Date.parse(iso))
  ? new Date(iso).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' }) : null);
...
export function ProfilePage({ workspace, nav }: SettingsPageProps) {
  ...
  const [status, setStatus] = useState<{ field: 'name' | 'email'; state: 'saving' | 'saved' } | null>(null);
  ...
  const save = async (field: 'name' | 'email') => {
    ...
    if (!updateProfile || value === current || (field === 'email' && value.toLowerCase() === current)) return;
    if (sending.current === `${field}:${value}`) return;
    sending.current = `${field}:${value}`;
    setError(null); setNote(null); setStatus({ field, state: 'saving' });
    try {
      ...
      setStatus(null); setError(said(reason));
      if (field === 'name') setName(account.name); else setEmail(account.email);
    } finally { sending.current = null; }
  };
  const resend = async () => {
    if (!resendVerification) return;
    setError(null);
    try { setNote(await resendVerification()); } catch (reason) { setError(said(reason)); }
  };
  return <ScrollView contentContainerStyle={[s.content, { paddingBottom: bottom + 24 }]} keyboardShouldPersistTaps="handled"
    showsVerticalScrollIndicator={false}>
    <Label first>Name</Label>
    <Group>
      <Field label="Name" value={name} onChangeText={setName} editable={editable} placeholder="Your name" autoCapitalize="words"
        autoComplete="name" textContentType="name" returnKeyType="done" maxLength={80}
        onSubmitEditing={() => void save('name')} onBlur={() => void save('name')}
        status={status?.field === 'name' ? status.state : null} />
    </Group>
    <Label>Email</Label>
    <Group>
      {provider ? <Row title={account.email} detail={`Signed in with ${provider}`} />
        : <Field label="Email" value={email} onChangeText={setEmail} placeholder="you@example.com"
          ...
          returnKeyType="done" onSubmitEditing={() => void save('email')} status={status?.field === 'email' ? status.state : null}
          editable={emailEditable} />}
      {!provider && account.emailVerified === false && resendVerification
        ? <Row title="Resend confirmation email" onPress={() => void resend()} trailing="none" /> : null}
    </Group>
    <Footnote>{provider ? `${provider} manages this address. Change it in your ${provider} account.`
      : workspace.demo ? 'The sample’s email can’t be changed.'
      : account.emailVerified === false ? 'This address isn’t confirmed yet. Check your inbox for the link.'
        : 'Change it and we’ll email a link to confirm the new address.'}</Footnote>
    {(error || note) && <View style={s.notice}><Hint error={Boolean(error)}>{error ?? note}</Hint></View>}
    {month(account.createdAt) && <>
      <Label>Account</Label>
      <Group><Row title="Member since" value={month(account.createdAt)} /></Group>
    </>}
    ...
    {(isTestAccount(workspace) || (!workspace.demo && workspace.actions.deleteAccount)) && <Group style={s.danger}>
