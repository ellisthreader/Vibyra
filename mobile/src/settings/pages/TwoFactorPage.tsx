...
import { TwoFactorElsewhere, TwoFactorNote, TwoFactorOff, TwoFactorOn, TwoFactorUnknown } from '../twoFactor/TwoFactorStatus';
 ...
 */
export function TwoFactorPage({ workspace, nav }: SettingsPageProps) {
  ...
  const { startTwoFactor, confirmTwoFactor, newRecoveryCodes, disableTwoFactor } = workspace.actions;
  const back = () => { clearError(); setStep({ at: 'status' }); };
  const begin = () => void run(async () => {
    const setup = await startTwoFactor!();
    setStep({ at: 'setup', setup });
  });
  const confirm = (code: string) => void run(async () => {
    const codes = await confirmTwoFactor!(code);
    settle({ enabled: true, confirmedAt: new Date().toISOString(), recoveryCodesLeft: codes.length });
    setStep({ at: 'codes', codes, fresh: true });
  });
  const renew = (code: string) => void run(async () => {
    const codes = await newRecoveryCodes!(code);
    settle({ recoveryCodesLeft: codes.length });
    setStep({ at: 'codes', codes, fresh: false });
  });
  const disable = (code: string) => void run(async () => {
    await disableTwoFactor!(code);
    settle({ enabled: false, confirmedAt: null, recoveryCodesLeft: 0 });
    setStep({ at: 'status' });
  });
  const body = () => {
    if (step.at === 'setup') return <TwoFactorSetup setup={step.setup} busy={busy} error={error} onConfirm={confirm} />;
    if (step.at === 'codes') return <RecoveryCodes codes={step.codes} account={account?.email ?? ''} onDone={back}
      doneLabel={step.fresh ? 'Done' : 'Back to security'} />;
    if (step.at === 'renew') return <CodePrompt title="Get new recovery codes" action="Get new codes" busy={busy} error={error}
      detail="Enter a code from your authenticator app. The codes you have now stop working."
      onSubmit={renew} onCancel={back} />;
    if (step.at === 'off') return <CodePrompt title="Turn off two-factor" action="Turn off" danger busy={busy} error={error}
      detail="Enter a code to confirm it’s you. Your password alone will open your account again."
      onSubmit={disable} onCancel={back} />;
    if (status.state === 'loading') return <View style={s.centre}><ActivityIndicator color={colors.muted}
      accessibilityLabel="Loading two-factor authentication" /></View>;
    if (status.state === 'unknown') return <TwoFactorUnknown problem={status.problem} onRetry={() => void reload()} />;
    if (!status.detail.available) return <TwoFactorElsewhere provider={names[account?.provider as 'apple'] ?? 'your provider'} />;
    if (status.detail.enabled) return <TwoFactorOn detail={status.detail}
      onRenew={() => { clearError(); setStep({ at: 'renew' }); }} onDisable={() => { clearError(); setStep({ at: 'off' }); }} />;
   ...
   */
  if (!startTwoFactor) return <ScrollView contentContainerStyle={[s.content, { paddingBottom: bottom + 24 }]}>
    <TwoFactorNote title="Not in the sample"
      line={'On your own account, this asks for a six-digit code from an app on your phone as well as '
        + 'your password, so a stolen password isn’t enough on its own.'} />
    <Footnote>{workspace.demo ? sampleNote(workspace) : 'Two-factor authentication isn’t available on this phone yet.'}</Footnote>
  </ScrollView>;
