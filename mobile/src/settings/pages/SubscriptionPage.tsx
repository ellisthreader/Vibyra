...
const said = (error: unknown) => error instanceof Error ? error.message : 'Billing couldn’t be opened. Try again.';
...
export function SubscriptionPage({ workspace, nav, routes }: SettingsPageProps) {
  ...
  const manage = async () => {
    setError(null);
    if (view.manage !== 'stripe' || !portal) { void Linking.openURL(APP_STORE_SUBSCRIPTIONS); return; }
    setOpening(true);
    try { void Linking.openURL(await portal()); } catch (reason) { setError(said(reason)); } finally { setOpening(false); }
  };
  return <ScrollView contentContainerStyle={[s.content, { paddingBottom: bottom + 24 }]} showsVerticalScrollIndicator={false}>
    ...
    {!view.paid && <Footnote>You’re on the free plan. Paid plans add more Vibes each month.</Footnote>}
    <Group style={s.actions}>
      {view.manage && live && <Row title="Manage subscription" busy={opening} onPress={() => void manage()} trailing="external" />}
      {live && buy.bridge && <Row title="Restore purchases" busy={buy.busy} onPress={buy.restore} trailing="none" />}
      <Row title="See plans" onPress={() => nav.close(() => routes.wallet('subscription'))} />
    </Group>
    {(error || buy.notice) && <View style={s.notice}>
      <Hint error={Boolean(error) || buy.notice?.ok === false}>{error ?? buy.notice?.text}</Hint>
    </View>}
    {workspace.demo && <Footnote>{sampleNote(workspace)} Nothing is billed.</Footnote>}
    {live && view.manage === 'appstore' && <Footnote>App Store subscriptions are changed or cancelled in the App Store.</Footnote>}
  </ScrollView>;
}
