 ...
 */
export function PageState({ feature, state, onRetry, onSignIn }: {
  feature: string; state: PersonalizationState; onRetry: () => void; onSignIn?: () => void;
}) {
  const { colors } = useTheme();
  if (state.status === 'idle' || state.status === 'loading') return <View style={s.box}>
  ...
  return <View style={s.box} accessibilityLiveRegion="polite">
    <Text style={[s.line, { color: colors.text }]}>{line}</Text>
    {state.problem && <Text style={[s.detail, { color: colors.muted }]}>{state.problem}</Text>}
    {signedOut ? onSignIn && <Pressable accessibilityRole="button" accessibilityLabel="Sign in" onPress={onSignIn}
      style={({ pressed }) => [s.signIn, { backgroundColor: colors.action, opacity: pressed ? 0.85 : 1 }]}>
      <Text style={[s.signInText, { color: colors.onAction }]}>Sign in</Text>
    </Pressable> : <Pressable accessibilityRole="button" accessibilityLabel="Try again" onPress={onRetry}
...
const s = StyleSheet.create({
  ...
  signIn: { marginTop: 12, minHeight: 40, borderRadius: 20, paddingHorizontal: 22, alignItems: 'center', justifyContent: 'center' },
