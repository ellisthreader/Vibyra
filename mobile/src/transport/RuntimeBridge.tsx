...
export const RuntimeBridge = forwardRef<BridgeHandle, BridgeProps>(({ onMessage }, ref) => {
  ...
  useImperativeHandle(
    ref,
    () => ({ post: (message) => webview.current?.postMessage(JSON.stringify(message)) }),
    [],
  );
  return (
    <View
      // WKWebView throttles timers in an opacity: 0 view to roughly one tick
      // per second, which stalls the encrypted Preview send queue under load.
    ...
    >
      <WebView
        ref={webview}
        source={{ html: runtimeHtml, baseUrl: origin }}
        originWhitelist={['*']}
        javaScriptEnabled
        domStorageEnabled={false}
        onShouldStartLoadWithRequest={(request) =>
          request.url === 'about:blank' || request.url === `${origin}/`
        }
        onMessage={(event) => {
          try {
        ...
        onContentProcessDidTerminate={() =>
          onMessage({
            type: 'error',
            message: 'iOS restarted the secure connection. Reconnect to your computer.',
          })
        }
        onError={() =>
          onMessage({ type: 'error', message: 'Secure connection support could not start.' })
        }
