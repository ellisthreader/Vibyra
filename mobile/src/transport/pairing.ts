...
export function parsePairing(link: string, now = Date.now()): Pairing {
  ...
  try {
    ...
    throw new Error('Scan or paste a Vibyra pairing code from your computer.');
  }
  ...
  if (value.route !== undefined && !['direct', 'relay'].includes(value.route))
    throw new Error('Unsupported route.');
  if (value.network !== undefined && value.network !== 'lan')
    throw new Error('Unsupported network scope.');
  ...
  if (
    ...
    (value.nearby !== true || value.route !== 'direct' || value.network !== 'lan')
  ) {
    throw new Error('This connection cannot use nearby pairing.');
  }
  const local =
    /^(localhost|127\.\d+\.\d+\.\d+|10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|172\.(1[6-9]|2\d|3[01])\.\d+\.\d+|169\.254\.\d+\.\d+|100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.\d+\.\d+|\[::1\])$/.test(
      url.hostname,
    );
  ...
  if (
    value.relayToken !== undefined &&
    (typeof value.relayToken !== 'string' || value.relayToken.length > 4096)
  ) {
  ...
  if (value.invite !== undefined) {
    ...
    const expires =
      typeof value.expiresAt === 'number' ? value.expiresAt : Date.parse(value.expiresAt ?? '');
    if (!Number.isFinite(expires) || expires <= now)
      throw new Error('This pairing code expired. Create a new one on your computer.');
  }
  return value;
}
