...
export class ConversationLedger {
  ...
  snapshot(value: ConversationSnapshot) {
    ...
    if (this.value?.generation === value.generation && this.value.cursor > value.cursor)
      return this.value;
    const authoritative = new Set(
      [...value.items, ...(value.pending ?? [])].map((item) => item.id),
    );
    const retained =
      this.value?.generation === value.generation
        ? this.value.items.map((item) =>
            !authoritative.has(item.id) && ['pending', 'responding'].includes(item.status)
              ? { ...item, status: item.status === 'pending' ? 'expired' : 'unknown' }
              : item,
          )
        : [];
    this.value = {
      ...value,
      items: mergeItems(mergeItems(retained, value.items), value.pending ?? []),
    };
  ...
  prepend(value: ConversationSnapshot) {
    ...
    this.value = {
      ...
      items: mergeItems(value.items, this.value.items),
    };
  }
  push(event: ConversationEvent): boolean {
    ...
    if (!this.value) {
      ...
      this.queued = this.queued.slice(-256);
      return false;
    }
    if (event.generation !== this.value.generation)
      throw new Error('The conversation restarted. Reconnect to catch up.');
    if (event.cursor <= this.value.cursor) return false;
    if (event.cursor !== this.value.cursor + 1) {
      this.queued.push(event);
      this.queued = this.queued.slice(-256);
      throw new Error('Catching up with your conversation…');
    }
    this.value = {
      ...this.value,
      ...event,
      items: event.item ? mergeItems(this.value.items, [event.item]) : this.value.items,
    };
    return true;
  }
