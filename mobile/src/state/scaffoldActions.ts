...
export function scaffoldActions(store: WorkspaceStore): { scaffold: ScaffoldActions } {
  const request = async <T>(method: string, params: object): Promise<T> => {
    if (store.state.status !== 'connected')
      throw new Error('Connect your computer to start a project on it.');
    if (!store.state.scaffoldAvailable)
      throw new Error('Update Vibyra Host on your computer to start projects from here.');
  ...
  return {
    scaffold: {
      ...
      start: async (runId, plan) => {
        await request('scaffold.start', { runId, plan });
      },
      cancel: async (runId) => {
        await request('scaffold.cancel', { runId });
      },
...
export function parseScaffoldEvent(
  event: unknown,
  ...
  if (event === 'scaffold.done') {
    const project =
      data.project && typeof data.project === 'object' ? (data.project as Project) : null;
    return {
      type: 'done',
      runId,
      ok: data.ok === true,
