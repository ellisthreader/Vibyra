...
export function rememberConversation(store: WorkspaceStore, id: string | null) {
  ...
  const work = (persistence.get(store) ?? Promise.resolve())
    .then(() => (id ? store.deps.flags.write(key, id) : store.deps.flags.delete(key)))
    .catch(() => {
      if (store.current(epoch))
        store.report(new Error('This chat could not be remembered on your phone.'));
    });
...
export async function restoreConversation(store: WorkspaceStore) {
  ...
  if (store.state.sessions.some((item) => item.id === id && item.runner === 'conversation'))
    await selectSession(store, id);
}
