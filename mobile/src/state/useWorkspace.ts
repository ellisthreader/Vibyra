...
export function useWorkspace() {
  ...
  const [store] = useState(() => new WorkspaceStore({
    ...
    remote: createRemoteApi(String(Constants.expoConfig?.extra?.apiUrl ?? 'https://vibyra-production.up.railway.app'),
      () => self.current?.token ?? null, deviceLabel()),
  }));
  self.current = store;
  ...
  const [integrationsApi] = useState(() => createIntegrationsApi(apiUrl, async prepare => {
  ...
  const [preferences] = useState(() => createPreferencesApi(apiUrl,
  ...
  useEffect(() => {
    ...
    const lifecycle = AppState.addEventListener('change', status => {
      ...
      else if (status === 'active') { store.resume(); void refreshAccount(store); }
    });
    ...
    return () => { mounted = false; links.remove(); lifecycle.remove(); network.remove(); };
  }, [store]);
  return { refreshMembership: () => { void refreshAccount(store); }, agentsApi, vibesApi, integrationsApi, bridge, onMessage: store.deps.rpc.receive, workspace: { ...state, preferences, actions: store.actions } };
}
const deviceLabel = () => Platform.OS === 'ios' ? 'iPhone' : Platform.OS === 'android' ? 'Android phone' : 'Vibyra web';
function accountApi() {
  ...
  return { ...api, socialLogin: (provider: 'apple' | 'google', signal: AbortSignal) => signInWithProvider(api, provider, signal),
    providerDeletion: (provider: 'apple' | 'google', token: string, signal: AbortSignal) => deleteWithProvider(api, provider, token, signal) };
}
