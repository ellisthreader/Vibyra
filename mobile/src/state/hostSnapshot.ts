...
export async function hostSnapshot(store: WorkspaceStore, epoch: number): Promise<HostState> {
  ...
  while (cursor) {
    if (seen.has(cursor) || seen.size >= 200) throw new Error('The computer returned too much session history.');
    seen.add(cursor);
    const page = await store.deps.rpc.request<Pick<HostState, 'sessions' | 'nextCursor'>>('session.list', { cursor, limit: 100 });
    store.assertCurrent(epoch);
