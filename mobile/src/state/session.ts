...
export async function claimControl(store: WorkspaceStore) {
  if (store.state.sessions.find(item => item.id === store.state.selectedSessionId)?.readOnly) {
    throw new Error('This desktop connection is view-only. Control this terminal on your Mac.');
  }
