...
const leaseTaken = (error: unknown) =>
  error instanceof Error && error.message.startsWith('Another phone took this terminal');
...
> {
  ...
  const inputs = new InputQueue(async (data) => {
    ...
    try {
      ...
      if (leaseTaken(error) && store.lease?.lease === lease.lease) {
        store.lease = null;
