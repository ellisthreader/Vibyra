...
export async function loadConversation(store: WorkspaceStore, sessionId: string) {
  ...
  if (!session || session.runner !== 'conversation' || store.state.selectedSessionId !== sessionId)
    return;
  const epoch = store.epoch;
  const selected = store.selectionEpoch;
  const current = () => store.current(epoch) && selected === store.selectionEpoch;
  const ledger =
    store.conversationLedger?.sessionId === sessionId
  ...
  const snapshot = await store.deps.rpc.request<ConversationSnapshot>('conversation.snapshot', {
    sessionId,
  });
  if (!current() || snapshotRequests.get(ledger) !== request) return;
  ledger.snapshot(snapshot);
  store.update({
    conversation: ledger.value,
    error: null,
    sessions: store.state.sessions.map((item) =>
      item.id === sessionId ? { ...item, status: ledger.value!.processState } : item,
    ),
  });
}
export function receiveConversation(store: WorkspaceStore, event: ConversationEvent) {
  ...
  try {
    if (ledger.push(event))
      store.update({
        conversation: ledger.value,
        sessions: store.state.sessions.map((item) =>
          item.id === event.sessionId ? { ...item, status: event.processState } : item,
        ),
      });
    ...
    const ended =
      ['completed', 'interrupted', 'failed'].includes(event.turnState) &&
      previous?.turnState !== event.turnState;
    if (ended || (previous && previous.processState !== event.processState))
      refreshConversation(store, ledger, true);
  } catch (error) {
    store.lease = null;
    store.update({ control: 'readonly' });
    store.report(error);
...
function refreshConversation(store: WorkspaceStore, ledger: ConversationLedger, force = false) {
  ...
  const work = loadConversation(store, ledger.sessionId)
    .catch((cause) => {
      if (store.current(epoch) && store.conversationLedger === ledger) store.report(cause);
    })
    .finally(() => {
      if (store.conversationLoading === work) store.conversationLoading = null;
    });
  store.conversationLoading = work;
}
export function conversationGeneration(store: WorkspaceStore) {
  return store.state.sessions.find((item) => item.id === store.state.selectedSessionId)?.runner ===
    'conversation'
    ? store.conversationLedger?.value?.generation
    : store.ledger?.generation;
}
