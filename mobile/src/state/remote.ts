...
export function relayPairing(grant: CloudGrant): Pairing {
  return {
    ...
    name: grant.host.name,
    publicKey: grant.host.id,
    url: grant.relayUrl,
    route: 'relay',
...
export async function refreshRelay(store: WorkspaceStore, pairing: Pairing): Promise<Pairing> {
  ...
  if (grant.host.id !== pairing.publicKey)
    throw new Error('Vibyra Cloud named a different computer. Pair it again.');
  return {
    ...pairing,
    name: grant.host.name || pairing.name,
    url: grant.relayUrl,
    relayToken: grant.token,
  };
}
...
export async function viaCloud(
