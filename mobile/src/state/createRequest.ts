...
export class CreateRequests {
  ...
  key(host: string, project: string, kind: SessionKind, title: string) {
    return JSON.stringify([host, project, kind, title]);
  }
  begin(key: string) {
    if (!this.ids.has(key) && this.ids.size >= 32)
      throw new Error('Resolve pending session requests before starting more work.');
    const id = this.ids.get(key) ?? this.uuid();
    this.ids.set(key, id);
    return id;
  }
  success(key: string) {
