...
export function restoreAccent(store: WorkspaceStore) {
  void store.deps.storage.read('accent').then(value => {
    if (isAccentId(value) && store.state.accent === initialState.accent) store.update({ accent: value });
  }, () => {});
}
...
export function restoreTerminalFontSize(store: WorkspaceStore) {
  void store.deps.storage.read('terminalFontSize').then(value => {
    const size = Number(value);
    if (!value || !Number.isFinite(size) || store.state.terminalFontSize !== initialState.terminalFontSize) return;
