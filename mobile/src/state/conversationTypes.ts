export interface AgentItem {
  ...
  category?: string; durationMs?: number | null; command?: string; cwd?: string;
  startedAt?: string; updatedAt?: string; provenance?: string; exitCode?: number | null;
  actions?: { type: string; path?: string; query?: string }[];
  artifact?: { id: string; hash: string; bytes: number; truncated: boolean }; hasDetail?: boolean;
  changes?: { path: string; kind: { type: 'add' | 'delete' | 'update'; move_path?: string | null }; added: number; removed: number }[];
  allowLabel?: string; choices?: string[]; persistentAvailable?: boolean; decisionScope?: string;
...
export interface ConversationSnapshot {
  ...
  settings?: { provider?: string; model: string; effort: string | null; approvalPolicy: unknown; sandbox?: unknown; revision: number; appliesTo: string };
