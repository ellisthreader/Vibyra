...
function valid(frame: OutputFrame) {
  if (
    ...
    frame.offset < byteLength(frame.output)
  ) {
...
export class OutputLedger {
  ...
  push(frame: OutputFrame): void {
    ...
    if (frame.generation !== this.generation)
      throw new Error('This terminal restarted. Refresh to load the new session.');
    if (frame.offset <= this.offset) return;
    const start = frame.offset - byteLength(frame.output);
    if (start > this.offset)
      throw new Error('Terminal output was interrupted. Refresh to catch up.');
  ...
  snapshot(frame: Snapshot): void {
    if (frame.sessionId !== this.sessionId)
      throw new Error('The computer returned a different terminal session.');
    valid(frame);
