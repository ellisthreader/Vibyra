import {
  ...
  type Preferences,
  type PreferencesApi,
} from '../vibes/preferencesApi';

export const sampleInstructions =
  'I build with Expo and TypeScript. Keep diffs small, and explain backend steps like I’m new to them.';
export const sampleMemoryTexts = [
  'Builds Vibyra, an app for coding from your phone.',
  'Uses Expo SDK 57 with TypeScript.',
  'Writes in British English.',
  'Keeps source files under 200 lines.',
...
export const sampleProfile = {
  ...
  about:
    'I learn best from small working examples. Outside code I run, cook and read science fiction.',
  summary:
    'I’m building Vibyra, an app for coding from your phone, mostly on evenings and weekends.\n\n' +
...
const refuse = (message: string): never => {
  throw new PreferencesError(message, 'refused', 422);
};
const PROFILE_NAMES = {
  name: 'your name',
...
export function createSamplePreferences(): PreferencesApi {
  ...
  let memories: Memory[] = sampleMemoryTexts.map((text, index) => ({
    id: `sample-${++serial}`,
    text,
  ...
  return {
    ...
    savePreferences: async (changes) => {
      if (changes.instructions !== undefined && changes.instructions.length > INSTRUCTIONS_MAX)
        refuse('Keep instructions to 1,000 characters.');
    ...
    addMemory: async (text) => {
      ...
      if (memories.length >= LIMIT)
        refuse('You can keep up to 50 memories. Remove one to add another.');
      memories = [
        { id: `sample-${++serial}`, text: clean, createdAt: new Date().toISOString() },
        ...memories,
      ];
      return list();
    },
    removeMemory: async (id) => {
      if (!memories.some((memory) => memory.id === id))
        throw new PreferencesError('That memory was already removed.', 'gone', 404);
