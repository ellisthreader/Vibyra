...
import { sampleWallet, sampleVibesApi } from './sampleVibes';
...
export function createSampleAgents() {
  ...
  const make = (id: string, fields: TeammateFields): Teammate => ({
    ...fields,
    id,
    chatId: `sample-agent-chat-${id}`,
    revision: 1,
    archived: false,
    status: 'idle',
    lastMessage: 'Ready for your first task.',
    updatedAt: now(),
    lastRunId: null,
  });
  let teammates = [
    make('website', {
      name: 'Website helper',
      avatar: 'site',
      brief: 'Review a website and prepare useful improvements.',
      memory: '',
      integrations: [],
      budget: 5,
    }),
    make('reviewer', {
      name: 'Code reviewer',
      avatar: 'review',
      brief: 'Review changes and explain what to improve.',
      memory: '',
      integrations: [],
      budget: 5,
    }),
  ...
  const chat = (id: string) => {
    const a = owned(id);
    return { id: a.chatId, title: a.name, trial_slot: null, trial_used: 0 };
  };
  ...
  const agentsApi: AgentsApi = {
    models: () => sampleVibesApi.models(),
    skills: async () => copy(skills),
    saveSkill: async (skill) => {
      const saved = { ...skill, revision: skill.revision + 1 };
      skills = [saved, ...skills.filter((s) => s.id !== skill.id)];
      return copy(saved);
    },
    ...
    save: async (fields, target) => {
      ...
      if (existing && target.revision !== existing.revision)
        throw new VibesError('Reload this teammate before editing.', 409);
      const saved = existing
        ? { ...existing, ...fields, revision: existing.revision + 1, updatedAt: now() }
        : make(target.id, fields);
      teammates = [saved, ...teammates.filter((a) => a.id !== saved.id)];
      return copy(saved);
    },
    archive: async (agent, archived) => {
      ...
      teammates = teammates.map((a) => (a.id === saved.id ? saved : a));
      return copy(saved);
    },
    decide: unavailable,
  };
  const chatApi: VibesApi = {
    ...
    models: async () => [],
    consent: async () => {},
    chats: async () => teammates.map((a) => chat(a.id)),
    createChat: unavailable,
    quote: async (chatId, text, model, effort, _integrations, attachments = []) => {
      const agent = owned(chatId);
      if (agent.archived) throw new VibesError('Restore this sample teammate first.', 409);
      const maxCredits = Math.min(agent.budget, 3);
      return {
        quote: JSON.stringify({ chatId, text, model, attachments, maxCredits }),
        model,
        effort,
        maxCredits,
        estimatedCredits: 1,
    ...
    submit: async (id, quote) => {
      ...
      const agent = owned(q.chatId);
      const response =
        `I’d start by clarifying the result, reviewing the relevant material, and preparing a change for you to review.\n\n` +
        `This is a sample conversation with ${agent.name}. No model or connected service was called.`;
      const turn: VibesTurn = {
        id,
        chatId: agent.chatId,
        model: q.model,
        status: 'completed',
    ...
    turns: async (id) => {
      owned(id);
      return copy(turns.filter((t) => t.chatId === id));
    },
    turn: async (id) => {
      const turn = turns.find((t) => t.id === id);
      if (!turn) throw new VibesError('Sample turn not found.', 404);
      return copy(turn);
    },
    cancel: async (id) => {
      const turn = turns.find((t) => t.id === id);
    ...
    upload: async (source) => {
      const attachment: VibesAttachment = {
        id: `sample-agent-file-${++serial}`,
        name: source.name,
        bytes: source.file?.size ?? 0,
        kind: source.mimeType.startsWith('image/')
          ? 'image'
