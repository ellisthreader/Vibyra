...
export const sampleStandIn: Account = {
  email: 'sample@vibyra.app',
  name: 'Sample account',
  plan: 'free',
};
type SampleActions = Pick<
  WorkspaceActions,
  | 'updateProfile'
  | 'setAvatar'
  | 'removeAvatar'
...
export function useSampleAccount(account: Account | null) {
      ...
      loadAccountDevices: async (): Promise<AccountDevice[]> =>
        [
          ...
          },
          {
            ...
            location: 'London, GB',
            current: false,
            lastActive: new Date(Date.now() - 3 * HOUR).toISOString(),
          },
  ...
  const reset = () => {
    ...
    held.avatarUrl = null;
    held.removed.clear();
    setPreferences(createSamplePreferences());
    changed();
  };
  return {
    account: account ? shown : null,
    sampleAccount: account ? undefined : shown,
    preferences,
    actions,
    reset,
  };
