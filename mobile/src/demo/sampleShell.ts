 ...
 */
export interface SampleShell { line: string }
export const samplePrompt = '$ ';
export function sampleShellInput(shell: SampleShell, keys: string): { echo: string; entered: string[] } {
  let echo = '';
  ...
  for (const key of keys) {
    if (key === '\r' || key === '\n') {
      ...
      shell.line = '';
      if (text) { entered.push(text); echo += '\r\nSample only — command not executed.\r\n' + samplePrompt; }
      else echo += '\r\n' + samplePrompt;
    } else if (key === '\u007f' || key === '\b') {
      if (shell.line) { shell.line = shell.line.slice(0, -1); echo += '\b \b'; }
    } else if (key >= ' ') { shell.line += key; echo += key; }
  }
  return { echo, entered };
}
