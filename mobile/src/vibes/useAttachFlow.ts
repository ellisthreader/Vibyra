...
import { attachmentPreviews } from './attachmentMemory';
...
export function useAttachFlow(
  ...
  const files = useAttachments(upload, setNotice, scope, owner);
  ...
  const previews = useRef<Record<string, string>>(attachmentPreviews(owner));
  previews.current = attachmentPreviews(owner);
  ...
  const pick = (picking: () => Promise<Picked>) => {
    ...
    void picking()
      .then((picked) => {
        ...
        if ('error' in picked) setNotice(picked.error);
        else files.add(picked.sources);
      })
      .catch((error) => {
        if (active.current === scope)
          setNotice(error instanceof Error ? error.message : 'That could not be opened.');
      });
  };
  return {
    files,
    ...
    camera: () => pick(takePhoto),
    photos: () => pick(() => choosePhotos(room)),
    documents: () => pick(() => chooseFiles(room)),
    /** The message is on its way: its photos become the transcript's and the tray empties. */
    sent: () => {
      for (const item of files.items)
        if (item.id && item.kind === 'image') previews.current[item.id] = item.uri;
      files.clearSent(files.items.map((item) => item.key));
      setNotice(null);
    },
  };
}
