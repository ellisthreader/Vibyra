...
import { activeTurn, isProjectTool, type VibesTurn } from './types';
      ...
      <View style={s.reply}>{t.response && <ConversationText text={t.response} />}
        {activeTurn(t) && <View accessibilityLiveRegion="polite" style={s.activity}><ActivityIndicator size="small" color={colors.accent} />
          <Text style={[s.status, { color: colors.muted }]}>{t.status === 'queued' ? 'Getting ready…' : t.status === 'waiting'
            ? (t.tools?.some(isProjectTool) ? 'Waiting for project tools…' : 'Asking your integrations…')
        ...
        {t.tools?.map(tool => tool.integration ? <View key={tool.id} style={s.integration}>
