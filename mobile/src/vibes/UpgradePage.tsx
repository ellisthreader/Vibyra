...
export function UpgradePage({
  ...
  const items =
    wallet && offer
      ? benefitsFor(offer, wallet).map((b) => ({
          key: b.amount === undefined ? `${b.id}:${b.label}` : b.id,
          text: b.label,
          amount: b.amount,
          unit: b.unit,
        }))
  ...
  const width = short
    ? __DEV__ && onPreview
      ? 36
      : 64
    : Math.round(Math.min(136, Math.max(92, height * 0.14)));
  const footer = (
    <>
      {buy.notice && !buy.notice.ok ? (
        <Text accessibilityRole="alert" style={[s.notice, { color: colors.error }]}>
          {buy.notice.text}
        </Text>
      ) : null}
      ...
      {__DEV__ && onPreview && (
        <Pressable
          ...
          accessibilityLabel="Test Pro upgrade"
          onPress={onPreview}
          disabled={buy.busy || !offer}
          style={{ minHeight: 32, justifyContent: 'center' }}
        >
          <Text style={[s.terms, { color: colors.muted, textDecorationLine: 'underline' }]}>
            Test Pro upgrade
          </Text>
        </Pressable>
      )}
    ...
    >
      <View style={s.hero}>
        <Animated.Image
          ...
          style={{
            width,
            height: Math.round(width * artRatio),
            opacity: appear,
            transform: [
        ...
        <Text
          ...
          style={[s.title, short && s.titleShort, { color: colors.text }]}
        >
          Get Vibyra {family}
        </Text>
        <Text style={[s.lead, { color: colors.muted }]}>
          All your Pro features. More Vibes to use them.
        </Text>
        {buy.sizes.length > 1 && (
          <View style={[s.tabs, short && s.tabsShort]}>
            <PlanTabs
              sizes={buy.sizes}
              selected={offer?.plan ?? null}
              still={still}
              disabled={buy.busy}
              onSelect={buy.choose}
            />
      ...
      {items && (
        <Bullets still={still} dense={short} title={`What ${family} gives you`} items={items} />
      )}
    </WalletPage>
  );
}
