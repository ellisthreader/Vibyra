...
export const personalityStyles: PersonalityStyle[] = ['balanced', 'concise', 'detailed', 'friendly'];
...
export interface Preferences extends Record<ProfileField, string>, Record<ProfileSwitch, boolean> {
  style: PersonalityStyle; instructions: string; memoryEnabled: boolean;
}
/** `source` is 'chat' for one a phone chat saved; older servers send none. */
export interface Memory { id: string; text: string; createdAt: string; source?: 'user' | 'chat' }
export interface MemoryList { memories: Memory[]; limit: number }
...
export const PROFILE_MAX: Record<ProfileField, number> = { name: 60, occupation: 120, about: 1500, summary: 8000 };
...
export class PreferencesError extends Error {
  constructor(message: string, readonly kind: PreferencesFailure, readonly status: number) { super(message); }
}
...
async function parse(response: Response): Promise<Record<string, unknown>> {
  ...
  try { const value: unknown = text ? JSON.parse(text) : null; return value && typeof value === 'object' ? value as Record<string, unknown> : {}; }
  catch { return {}; }
}
...
function preferences(value: unknown): Preferences {
  ...
  return { style: personalityStyles.includes(p.style as PersonalityStyle) ? p.style as PersonalityStyle : 'balanced',
    instructions: text('instructions'), memoryEnabled: p.memoryEnabled !== false,
    name: text('name'), nameEnabled: p.nameEnabled !== false, occupation: text('occupation'), occupationEnabled: p.occupationEnabled !== false,
    about: text('about'), aboutEnabled: p.aboutEnabled !== false, summary: text('summary'), summaryEnabled: p.summaryEnabled !== false };
}
function memoryList(data: Record<string, unknown>): MemoryList {
  ...
  const memories = (data.memories as Record<string, unknown>[]).filter(item => item && typeof item.id === 'string'
    && typeof item.text === 'string').map(item => ({ id: item.id as string, text: item.text as string,
    createdAt: typeof item.createdAt === 'string' ? item.createdAt : '', source: item.source === 'chat' ? 'chat' as const : 'user' as const }));
  const limit = typeof data.limit === 'number' && data.limit > 0 ? data.limit : 50;
  return { memories, limit };
}

export function createPreferencesApi(baseUrl: string, token: () => string | null | Promise<string | null>,
  fetcher: typeof fetch = (...args) => fetch(...args)): PreferencesApi {
  ...
  const call = async (method: 'GET' | 'POST' | 'DELETE', path: string, body?: object) => {
    ...
    try {
      ...
      try {
        response = await fetcher(root + path, { method, signal: controller.signal, body: body ? JSON.stringify(body) : undefined,
          headers: { Authorization: `Bearer ${identity}`, Accept: 'application/json', ...(body ? { 'Content-Type': 'application/json' } : {}) } });
      } catch { throw new PreferencesError(OFFLINE, 'offline', 0); }
      const data = await parse(response);
      if (identity !== await token()) throw new PreferencesError('Your account changed. Please try again.', 'signedOut', 401);
      if (response.ok && data.ok !== false) {
        if (method !== 'GET') preferencesChanged();
        return data;
      }
      const error = typeof data.error === 'string' && data.error ? data.error : null;
      if ((response.status === 404 || response.status === 405) && !error) throw new PreferencesError(MISSING, 'unavailable', response.status);
      if (response.status === 404) throw new PreferencesError(error!, 'gone', 404);
      if (response.status === 401) throw new PreferencesError(error ?? 'Sign in again to change this.', 'signedOut', 401);
  ...
  return {
    ...
    savePreferences: async changes => preferences((await call('POST', 'preferences', Object.fromEntries(
      Object.entries(changes).filter(([, value]) => value !== undefined)))).preferences),
    listMemories: async () => memoryList(await call('GET', 'memories')),
    addMemory: async text => memoryList(await call('POST', 'memories', { text })),
    removeMemory: async id => memoryList(await call('DELETE', `memories/${encodeURIComponent(id)}`)),
