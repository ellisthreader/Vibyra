...
export interface Notice {
  text: string;
  ok: boolean;
  upgrade?: import('./UpgradeCelebration').UpgradeSuccess;
}
export interface WalletPurchase {
  offers: VibesProduct[];
  sizes: VibesProduct[];
  plan: VibesProduct | null;
  price?: string;
  priceOf(id: string): string | undefined;
  current: VibesProduct | undefined;
  topups: VibesProduct[];
  bridge: boolean;
  canBuy: boolean;
  busy: boolean;
  notice: Notice | null;
  choose(plan: string | null): void;
  buy(id: string): void;
  restore(): void;
  reloadPrices(): void;
}
...
export function useWalletPurchase(active: boolean): WalletPurchase {
  ...
  useEffect(() => {
    ...
    setNotice(null);
    void bridge
      .products(JSON.parse(productIds) as string[])
      .then((p) => {
        if (live) setProducts(p);
      })
      .catch(() => {
        if (live)
          setNotice({ text: 'Apple prices could not be loaded. Please try again.', ok: false });
      });
    return () => {
      live = false;
    };
  }, [active, bridge, store, wallet, productIds, priceAttempt]);
  ...
  const plan =
    selling.find((p) => p.plan === chosen) ??
    selling.find((p) => p.plan === defaultOffer(selling)) ??
    null;
  ...
  const guarded = async (work: () => Promise<string | Notice>) => {
    ...
    try {
      ...
      setNotice(typeof text === 'string' ? (text ? { text, ok: true } : null) : text);
    } catch (e) {
      setNotice({
        text: e instanceof Error ? e.message : 'Purchase could not complete.',
        ok: false,
      });
    } finally {
      lock.current = false;
  ...
  return {
    ...
    sizes,
    plan,
    price: plan ? priceOf(plan.id) : undefined,
    priceOf,
    current: wallet?.products.find((p) => p.kind === 'subscription' && p.plan === wallet.plan),
    topups:
      wallet && wallet.plan !== 'free' ? wallet.products.filter((p) => p.kind === 'topup') : [],
    bridge: Boolean(bridge),
    busy,
    notice,
    choose: setChosen,
    reloadPrices: () => setPriceAttempt((n) => n + 1),
    canBuy: Boolean(ready && bridge && wallet?.purchasesEnabled),
    buy: (id) =>
      void guarded(async () => {
        if (!active || !ready || !wallet || !priceOf(id))
          throw new Error('Apple prices are not ready. Refresh and try again.');
        const next = await buyVibes(store.api, bridge!, id, wallet!);
        if (!next) return '';
        store.update({ wallet: next });
        return {
          text: 'Your Vibes and Pro features are ready.',
          ok: true,
          upgrade:
            wallet.products.find((p) => p.id === id)?.kind === 'subscription' &&
            next.plan !== 'free'
              ? {
                  plan: next.plan,
                  added: Math.max(0, next.total - wallet.total),
