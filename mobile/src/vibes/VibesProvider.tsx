...
import { clearAttachmentMemory } from './attachmentMemory';
import { clearConversationScrollMemory } from './conversationScrollMemory';
...
function accountStore(
  ...
  guest: boolean,
  purchases: PurchaseBridge | null,
) {
  const key = vibesStateKey(identity);
  const prepare = !identity && guest ? () => prepareGuest(api).then(() => {}) : undefined;
  return new VibesStore(
    api,
    randomUUID,
    { read: () => readFlag(key), write: (value) => writeFlag(key, value) },
    purchases,
    prepare,
  );
}
export function VibesProvider({
  ...
  const [account, setAccount] = useState(() => ({
    identity,
    guest,
    store: accountStore(api, identity, guest, purchases),
  }));
  const priorIdentity = useRef(identity);
  useEffect(() => {
    if (priorIdentity.current !== identity) {
      clearAttachmentMemory(`vibes:${priorIdentity.current ?? 'guest'}:`);
      clearConversationScrollMemory(`vibes:${priorIdentity.current ?? 'guest'}:`);
  ...
  useEffect(() => {
    ...
    const recoverPurchases = async () => {
      ...
      try {
        store.update({ wallet: await claimPending(api, purchases, false) });
      } catch (e) {
        store.error(e);
      }
    };
    const start = async () => {
      ...
      if (!identity && guest && store.state.errorStatus === 401 && (await renewGuest(api))) {
        clearAttachmentMemory('vibes:guest:');
        clearConversationScrollMemory('vibes:guest:');
    ...
    void start().catch((error) => {
      if (active) store.error(error);
    });
    const timer = setInterval(() => {
      if (AppState.currentState === 'active' && store.needsPolling) void store.refresh();
    }, 5000);
    const listener = AppState.addEventListener('change', (state) => {
      if (state === 'active') void store.refresh().then(recoverPurchases);
    });
    const transactionListener = purchases?.addListener?.('transactionsChanged', () => {
  ...
  }, [store, identity, guest, api, purchases]);
...
export function useVibesStore() {
  return useContext(Context);
}
const never = () => () => {};
const noChats = { chats: [], selected: null };
...
export function useVibesChats() {
  ...
  return useSyncExternalStore(
    store?.subscribeChats ?? never,
    store?.chatsSnapshot ?? (() => noChats),
    store?.chatsSnapshot ?? (() => noChats),
  );
}
