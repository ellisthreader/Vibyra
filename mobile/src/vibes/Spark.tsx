      ...
      <Animated.View
        style={[
          ...
          breathing && {
            ...
            transform: [
              { scale: breath.interpolate({ inputRange: [0, 1], outputRange: [0.92, 1.08] }) },
            ],
          },
        ]}
      >
...
/** The empty chat's mark: the spark on a quiet accent tile, drawn crisp rather than lit. */
export function SparkTile({ size = 60 }: { size?: number }) {
  const { colors } = useTheme();
  return (
    <View
      accessible={false}
    ...
    >
      <Icon name="sparkles" size={Math.round(size * 0.42)} color={colors.accent} />
    </View>
  );
}
