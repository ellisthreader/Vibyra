 ...
 */
export function DictationButton({ text, onChange, onNote, disabled }: {
  text: string; onChange(value: string): void; onNote(message: string | null): void; disabled?: boolean;
}) {
  ...
  useEffect(() => {
    if (!listening || reduced) { pulse.setValue(0); return; }
  ...
  const press = () => { if (listening) session.current?.stop(); else void start(); };
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={listening ? 'Stop voice input' : 'Speak your message'}
    accessibilityState={{ disabled: disabled && !listening, selected: listening }} disabled={disabled && !listening} hitSlop={{ left: 6, right: 6, top: 6, bottom: 6 }}
    onPress={press} style={({ pressed }) => [s.tool, { opacity: pressed ? 0.6 : disabled && !listening ? 0.35 : 1 }]}>
    {listening && <Animated.View pointerEvents="none" style={[s.ring, { borderColor: colors.accent,
    ...
    <View style={[s.face, listening && { backgroundColor: colors.action }]}>
      <Icon name={listening ? 'stop' : 'mic-outline'} size={listening ? 14 : 21} color={listening ? colors.onAction : colors.text} />
    </View>
  </Pressable>;
}
