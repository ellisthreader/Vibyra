...
export function useChatQuote(input: Input) {
  const { store, chatId, text, model, effort, integrations, attachments, enabled, revision } =
    input;
  const personalRevision = useSyncExternalStore(
    subscribePreferenceChanges,
    preferenceRevision,
    preferenceRevision,
  );
  const identity = JSON.stringify([
    quoteIdentity({ chatId, text, model, effort, integrations, attachments }),
    revision,
    personalRevision,
  ]);
    ...
    const timer = setTimeout(() => {
      void store
        ...
        .then(async (id) => {
          ...
          const value = await store.api.quote(
            id,
            text.trim(),
            model,
            effort,
          ...
          if (version === generation.current && store.state.selected === id) {
            setQuoted({
              identity: JSON.stringify([
                quoteIdentity({ chatId: id, text, model, effort, integrations, attachments }),
                revision,
                personalRevision,
              ]),
              value,
        ...
        .catch((e) => {
          if (version === generation.current)
            setError(
              e instanceof Error ? e.message : 'The estimate could not be loaded. Please refresh.',
            );
        });
    }, 650);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      invalidate,
      Math.max(1000, quoted.value.expiresAt * 1000 - Date.now() - 5000),
    );
    return () => clearTimeout(timer);
  }, [quoted]);
  return {
    quote: enabled && quoted?.identity === identity ? quoted.value : null,
    error,
    invalidate,
    contextRevision: personalRevision,
  };
}
