 ...
 */
export function DrawerBalance({ onPress, style }: { onPress(): void; style?: StyleProp<ViewStyle> }) {
  const { colors } = useTheme();
  const { wallet } = useVibes();
  if (!wallet) return null;
  const count = wallet.available.toLocaleString();
  return <Pressable accessibilityRole="button" testID="drawer-balance"
    accessibilityLabel={`${count} ${wallet.available === 1 ? 'Vibe' : 'Vibes'}`} accessibilityHint="Opens Vibyra tokens"
    onPress={onPress} style={({ pressed }) => [s.balance, style, { opacity: pressed ? 0.55 : 1 }]}>
    <TokenMark size={32} />
    <Text numberOfLines={1} style={[s.count, { color: colors.text }]}>{count}</Text>
  </Pressable>;
}
const s = StyleSheet.create({
  balance: { minHeight: 46, paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center', gap: 9 },
