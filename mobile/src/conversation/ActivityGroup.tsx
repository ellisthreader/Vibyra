import { styles as s } from './ActivityGroupStyles';
  ...
  return (
    <View>
      <Pressable
        ...
        accessibilityLabel={`${label}. ${expanded ? 'Hide' : 'Show'} work details`}
      ...
      >
        {liveLabel ? (
          <View style={s.live}>
            <GenerationIndicator label={label} />
          </View>
        ) : (
      ...
      {liveLabel && !expanded && latestNote?.kind === 'message' && (
        <Text numberOfLines={2} style={[s.preview, { color: colors.muted }]}>
          {latestNote.text}
        </Text>
      )}
      {expanded && (
        <View style={[s.steps, { borderLeftColor: colors.border }]}>
          {items.map((item) =>
            item.kind === 'message' ? (
              <View key={item.id} style={s.note}>
                <Text selectable style={[s.noteText, { color: colors.text }]}>
                  {item.text}
                </Text>
                {item.source?.hasDetail && onInspect && (
                  ...
                  >
                    <Text style={{ color: colors.accent }}>Read full message</Text>
                  </Pressable>
                )}
              </View>
            ) : (
  ...
  return (
    <View>
      <Pressable
        ...
        accessibilityState={inspectable ? { expanded: open } : undefined}
        style={s.step}
      >
        <Icon
          name={
        ...
        {item.source?.durationMs != null && (
          <Text style={{ color: colors.muted, fontSize: 12, fontVariant: ['tabular-nums'] }}>
            {durationLabel(item.source.durationMs)}
          </Text>
        )}
        {inspectable && (
          <Icon name={open ? 'chevron-up' : 'chevron-down'} size={12} color={colors.muted} />
        )}
      </Pressable>
      {open && (
        <View>
          <ScrollView horizontal={!reasoning}>
            <Text
              selectable
              style={[
                s.detail,
                ...
                {
                  backgroundColor: reasoning ? 'transparent' : colors.elevated,
                  color: colors.text,
                },
              ]}
            >
          ...
          {onInspect && item.source && (
            ...
            >
              <Text style={{ color: colors.accent }}>
                Inspect {item.source.category === 'fileChange' ? 'changes' : 'details'}
              </Text>
