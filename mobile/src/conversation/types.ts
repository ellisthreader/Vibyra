...
export type ConversationMessage = ItemIdentity & {
  kind: 'message'; role: 'user' | 'assistant'; text: string; source?: import('../state/conversationTypes').AgentItem;
};
export type ConversationActivity = ItemIdentity & {
  kind: 'activity'; title: string; detail?: string; status: 'running' | 'completed' | 'failed' | 'interrupted' | 'declined';
  source?: import('../state/conversationTypes').AgentItem;
};
export type ConversationPermission = ItemIdentity & {
  kind: 'permission'; title: string; reason?: string; scope?: string; detail?: string; allowLabel?: string; source?: import('../state/conversationTypes').AgentItem; choices?: string[]; status: RequestStatus;
};
...
export type ConversationResult = ItemIdentity & {
  kind: 'result'; source?: import('../state/conversationTypes').AgentItem; text: string; status: 'completed' | 'failed' | 'interrupted'; checks?: string[];
};
...
export type ConversationViewProps = {
  turnId?: string | null;
  memoryKey?: string; items: ConversationItem[]; status: ConversationStatus; connected: boolean; canRespond: boolean;
