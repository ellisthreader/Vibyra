...
export function QuestionCard({
  ...
  item: ConversationQuestion;
  canRespond: boolean;
  onAnswer: ConversationViewProps['onAnswer'];
  draft?: QuestionDraft;
  ...
  const complete =
    ...
    item.questions.every((q) => Boolean(written[q.id]?.trim() || selected[q.id]));
  const submit = async () => {
    ...
    const answers = Object.fromEntries(
      item.questions.map((q) => [q.id, [written[q.id]?.trim() || selected[q.id]]]),
    );
    try {
      await onAnswer(item.id, answers);
    } catch (cause) {
  ...
  return (
    ...
    >
      {item.questions.map((question) => (
        <View key={question.id} style={s.question}>
          ...
          {question.options.map((option) => {
            ...
            return (
              <Pressable
                ...
                onPress={() => {
                  const next = {
                    selected: { ...selected, [question.id]: option.id },
                    written: { ...written, [question.id]: '' },
                  };
              ...
              >
                <View style={s.optionCopy}>
                  <Text style={[s.optionTitle, { color: colors.text }]}>{option.label}</Text>
                  {option.description && (
          ...
          {(question.allowFreeform || question.options.length === 0) && (
            <TextInput
              accessibilityLabel={`Your answer: ${question.prompt}`}
              placeholder="Or write your own answer…"
              placeholderTextColor={colors.muted}
              value={written[question.id] ?? ''}
              editable={!disabled}
              onChangeText={(text) => {
                const next = { ...written, [question.id]: text };
                setWritten(next);
                onDraft?.({ selected, written: next });
              }}
...
const s = StyleSheet.create({
  question: { gap: 8 },
  prompt: { fontSize: 15, lineHeight: 22, letterSpacing: -0.2, marginBottom: 4 },
  option: {
    minHeight: 52,
    borderWidth: 1,
    borderRadius: 12,
  ...
  optionTitle: { fontSize: 15, lineHeight: 20, fontWeight: '600', letterSpacing: -0.25 },
