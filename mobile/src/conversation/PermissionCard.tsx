...
export function PermissionCard({
  ...
  return (
    ...
    >
      ...
      <View style={[s.scope, { backgroundColor: colors.elevated, borderColor: colors.border }]}>
        <Text style={[s.scopeLabel, { color: colors.muted }]}>Applies to</Text>
        <Text selectable style={[s.body, { color: colors.text }]}>
          {item.scope ?? 'This action only'}
        </Text>
      </View>
      {item.detail && (
        <View>
          ...
          {expanded && (
            <Text selectable style={[s.detail, { color: colors.text }]}>
              {item.detail}
            </Text>
          )}
        </View>
...
const s = StyleSheet.create({
  ...
  scopeLabel: { fontSize: 12, fontWeight: '600', letterSpacing: -0.05 },
  detailsButton: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 6 },
  detailsLabel: { fontSize: 13, fontWeight: '500' },
  detail: { fontFamily: 'Menlo', fontSize: 12, lineHeight: 19, paddingBottom: 8 },
