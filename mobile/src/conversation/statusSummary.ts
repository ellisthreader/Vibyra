...
export function statusSummary(value: unknown) {
  const data = record(value),
    ...
    active = record(data.activeSettings),
  ...
  return {
    ...
    rows: [
      ['Provider', 'Codex'],
      ['Account', 'Connected Codex account'],
      ['Model', String(settings.model ?? 'Not reported')],
      ['Reasoning effort', human(settings.effort)],
      ['Working directory', String(data.workingDirectory ?? 'Not reported')],
      ['Permissions', permission],
      [
        'Approval requests',
        settings.approvalPolicy === 'never'
          ? 'Automatically declined'
          : human(settings.approvalPolicy),
      ],
