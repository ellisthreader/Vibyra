...
export function turnSummary(items: AgentItem[], turnId: string) {
  ...
  const changes = turn
    .filter((item) => item.category === 'fileChange' && item.status === 'completed')
  ...
  const duration =
    result?.durationMs ??
    (start.length && Number.isFinite(end) ? Math.max(0, end - Math.min(...start)) : null);
  ...
  return [
    ...
    files ? `${files} ${files === 1 ? 'file' : 'files'} · +${added} −${removed}` : '',
  ]
    .filter(Boolean)
    .join(' · ');
}
...
export function hasLiveActivity(items: AgentItem[]) {
  ...
  return items.some(
    (item) => item.turnId === latestTurn && item.kind === 'activity' && item.status === 'running',
  );
}
