...
export type MarkdownBlock = { kind: 'text' | 'code'; text: string }
...
export function tableCells(line: string): string[] {
  ...
  for (let index = 0; index < line.length; index++) {
    if (line[index] === '\\' && line[index + 1] === '|') { cell += '|'; index++; }
...
export function markdownBlocks(text: string): MarkdownBlock[] {
  ...
  let plain: string[] = [];
  const flush = () => { if (plain.length) blocks.push({ kind: 'text', text: plain.join('\n') }); plain = []; };
  for (let index = 0; index < lines.length; index++) {
    ...
    if (lines[index].includes('|') && headers.length > 0 && headers.length === delimiter.length
      ...
      blocks.push({ kind: 'table', headers, rows, alignments: delimiter.map(cell =>
