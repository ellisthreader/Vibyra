...
export const effortLabel = (value: Effort | null): string =>
  value ? choices[value].label : 'Default';
...
export function normalizeReasoning(raw: unknown): Reasoning {
  ...
  const accepted: string[] =
    supported === null
      ...
      : Array.isArray(supported)
        ? supported.filter((item): item is string => typeof item === 'string')
        : [];
  // A mandatory reasoner cannot be switched off, so "none" is not offered.
  const list = efforts.filter(
  ...
  return {
    efforts: list,
    defaultEffort: list.includes(fallback as Effort) ? (fallback as Effort) : null,
    mandatory,
  };
}

/** The ladder a model offers, or none at all when it cannot be steered. */
export const effortsFor = (model: VibesModel | undefined): Effort[] =>
  model?.reasoning?.efforts ?? [];
export const supportsEffort = (model: VibesModel | undefined): boolean =>
  effortsFor(model).length > 1;
...
export function resolveEffort(
  ...
  return available.reduce((best, effort) =>
    Math.abs(efforts.indexOf(effort) - wanted) < Math.abs(efforts.indexOf(best) - wanted)
      ? effort
      : best,
  );
}
