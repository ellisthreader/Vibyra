...
function AddressFacts({ address }: { address: string }) {
  ...
  return (
    <>
      {ip ? (
        <Fact label={local ? 'Public IP' : 'IP address'} value={ip} long={ip.includes(':')} />
      ) : local && asking ? (
        <Fact label="Public IP" value="Locating…" quiet />
      ) : null}
      <Fact
        label="Location"
        quiet={asking}
...
function Fact({
  ...
  const { colors } = useTheme();
  return (
    <View style={[s.fact, long && s.factLong, { borderBottomColor: colors.border }]}>
      <Text style={[s.factLabel, { color: colors.muted }]}>{label}</Text>
      <Text
        numberOfLines={long ? 2 : 1}
        selectable
        style={[s.factValue, { color: quiet ? colors.muted : colors.text }]}
      >
        {value}
      </Text>
    </View>
