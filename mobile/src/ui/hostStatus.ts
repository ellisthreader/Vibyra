...
export interface ConnectionWords {
  label: string; tone: 'success' | 'accent' | 'muted' | 'error'; working: boolean; figure: Figure;
...
export function describeConnection(workspace: Pick<WorkspaceModel, 'status' | 'reconnecting' | 'error' | 'demo'>): ConnectionWords {
  if (workspace.demo) return { label: 'Sample computer', tone: 'muted', working: false, figure: 'live', problem: null };
  if (workspace.status === 'connected') return { label: 'Connected', tone: 'success', working: false, figure: 'live', problem: null };
  if (workspace.status === 'pairing') {
    return { label: 'Waiting for approval on your computer', tone: 'accent', working: true, figure: 'waking', problem: null };
  }
  if (workspace.status === 'connecting' || workspace.reconnecting) {
    return { label: 'Reconnecting…', tone: 'accent', working: true, figure: 'waking', problem: null };
  }
  if (workspace.status === 'error' && workspace.error) {
    return { label: 'Couldn’t reach your computer', tone: 'error', working: false, figure: 'asleep', problem: workspace.error };
  }
