...
const rankOf = (vendor: string) => {
  const at = RANKED.indexOf(vendor);
  return at < 0 ? RANKED.length : at;
};
...
const tierOf = (model: VibesModel) => {
  const at = TIERS.indexOf(model.tier ?? '');
