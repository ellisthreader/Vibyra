...
import { deleteDrafts, readDraft, writeDraft } from './draftStorage';
...
/** Clear one computer or account; an in-flight read must never restore its old draft. */
export async function clearDrafts(prefix: string) {
  const scopes = new Set([...knownScopes, ...drafts.keys(), ...revisions.keys()]);
...
export function useDraft(scope: string, persist = false) {
  ...
  useEffect(() => {
    ...
    const save = (force = false) => {
      ...
      if (persist)
        void writeDraft(scope, drafts.get(scope) ?? '')
          .then(() => {
            if (active) setError(null);
          })
          .catch(() => {
            if (active) setError('Your draft is kept here, but could not be saved on this phone.');
          });
    };
    const unsubscribe = persist ? subscribe(save) : () => {};
    if (persist && drafts.has(scope)) save(true);
    if (persist && !drafts.has(scope))
      void readDraft(scope)
        .then((saved) => {
          if (active && revisions.get(scope) === revision && !drafts.has(scope) && saved)
            setDraftForScope(scope, saved);
        })
        .catch(() => {
          if (active) setError('Your saved draft could not be restored.');
        });
    return () => {
      active = false;
      unsubscribe();
