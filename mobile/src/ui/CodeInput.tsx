...
export function CodeInput({
  ...
  useEffect(() => {
    if (value.length < LENGTH) {
      if (value.length === 0) sent.current = null;
      return;
    }
  ...
  return (
    <Pressable accessibilityRole="none" onPress={() => field.current?.focus()} style={s.row}>
      {digits.map((digit, at) => (
        <View
          key={at}
          style={[
...
const s = StyleSheet.create({
  ...
  digit: { fontSize: 24, fontWeight: '600', letterSpacing: -0.4, fontVariant: ['tabular-nums'] },
