...
export function describeSession(session: Session, workspace: Pick<WorkspaceModel,
  'status' | 'reconnecting' | 'demo' | 'control' | 'actions'>): SessionStatus {
  ...
  if (workspace.status !== 'connected') {
    return workspace.reconnecting
      ...
      : { hidden: false, text: 'Connection paused', tone: 'muted', action: workspace.actions.reconnect ? 'reconnect' : undefined };
  }
  if (session.status === 'interrupted') return { hidden: false, text: 'Interrupted · start a new chat to continue', tone: 'muted' };
  const refusal = typingRefusal(session);
  if (refusal) return { hidden: false, text: refusal, tone: 'warning' };
  if (session.status === 'exited') {
    return { hidden: false, text: `Ended${session.exitCode === undefined ? '' : ` · exit ${session.exitCode}`}`, tone: 'muted' };
  }
  if (workspace.control === 'claiming') return { hidden: false, text: 'Taking control…', tone: 'muted' };
  if (typableSession(session) && workspace.control !== 'ready') {
    return { hidden: false, text: 'Viewing live', tone: 'muted', action: workspace.actions.claimControl ? 'takeControl' : undefined };
  }
  return quiet;
}
...
export const typingRefusal = (session: Pick<Session, 'canInput' | 'readOnly'>) =>
  session.canInput === false ? 'Typing from your phone is off. Turn it on in Vibyra on your Mac: Settings › iPhone connection.'
    : !typableSession(session) ? 'Update Vibyra on your Mac to type from your phone' : null;
