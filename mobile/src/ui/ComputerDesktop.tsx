import { styles as s } from './ComputerDesktopStyles';
    ...
    <View style={s.desktop}>
      <View style={[s.chrome, edge, { backgroundColor: colors.rail }]}>
        <View style={s.brand}>
          {mac &&
            lights.map((color) => (
              <View key={color} style={[s.light, { backgroundColor: color }]} />
            ))}
          <BrandMark size={9} />
          <Text
            allowFontScaling={false}
            numberOfLines={1}
            style={[s.wordmark, { color: colors.text }]}
          >
            Vibyra
          </Text>
        </View>
        <View style={s.context}>
          ...
          {!mac &&
            (['remove', 'square-outline', 'close'] as const).map((name) => (
      ...
      <View style={s.body}>
        <View style={[s.rail, edge, { backgroundColor: colors.rail }]}>
          ...
          <View style={s.section}>
            <Bar width={16} color={colors.muted} opacity={0.35} />
          </View>
      ...
      {logo === 'logo-windows' && (
        <View style={[s.taskbar, edge, { backgroundColor: colors.surface }]}>
...
const ran: Output[] = [
  { mark: 'accent', width: 0.8 },
  { width: 0.52 },
  { width: 0.66, passed: true },
  { width: 0 },
  { mark: 'muted', width: 0.6 },
  { width: 0.74 },
  { width: 0.4 },
];
const working: Output[] = [
  { mark: 'accent', width: 0.72 },
  { width: 0.46 },
  { width: 0.58 },
  { width: 0 },
  { mark: 'accent', width: 0.64 },
  { width: 0.5 },
  { mark: 'success', width: 0.38 },
];
...
function Pane({ number, output, caret }: { number: string; output: Output[]; caret?: Caret }) {
  const { colors } = useTheme();
  return (
    <View style={[s.pane, { backgroundColor: colors.workspace }]}>
      <View style={[s.head, { backgroundColor: colors.rail, borderColor: colors.border }]}>
        <View style={[s.badge, { backgroundColor: colors.accentSoft }]}>
          <Text allowFontScaling={false} style={[s.number, { color: colors.accent }]}>
            {number}
          </Text>
        </View>
      ...
      <View style={s.output}>
        {output.map((row, index) => (
          <View key={index} style={[s.line, !row.mark && s.indent]}>
            {row.mark && <View style={[s.mark, { backgroundColor: colors[row.mark] }]} />}
            <View style={s.grow}>
              <Bar
                width={`${row.width * 100}%`}
                color={row.passed ? colors.success : colors.muted}
                opacity={row.passed ? 0.8 : row.mark ? 0.6 : 0.35}
              />
            </View>
          </View>
        ))}
      ...
      <View style={[s.prompt, { backgroundColor: colors.elevated }]}>
        ...
        {caret === undefined ? (
          <Bar width={30} color={colors.muted} opacity={0.3} />
        ) : (
          <Animated.View style={[s.caret, { backgroundColor: colors.accent, opacity: caret }]} />
        )}
      </View>
      {caret !== undefined && (
        <View style={[StyleSheet.absoluteFill, s.ring, { borderColor: colors.accent }]} />
      )}
    </View>
  );
}
...
function Row({ width, selected, dot }: { width: number; selected?: boolean; dot?: string }) {
  ...
  return (
    <View style={[s.row, selected && { backgroundColor: colors.elevated }]}>
      <View style={[s.glyph, { borderColor: ink, opacity: selected ? 0.8 : 0.5 }]} />
      <View style={s.grow}>
        <Bar width={`${width * 100}%`} color={ink} opacity={selected ? 0.72 : 0.45} />
      </View>
