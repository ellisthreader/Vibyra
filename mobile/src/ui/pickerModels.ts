...
export const PICKER_MODEL_IDS = new Set([
  ...
  'minimax/minimax-m3',
  'mistralai/mistral-medium-3-5', 'mistralai/mistral-small-2603', 'mistralai/devstral-2512',
]);
...
export function pickerModels(models: VibesModel[]): VibesModel[] {
  return models.filter(model => {
    ...
    return model.available && PICKER_MODEL_IDS.has(model.id) && Boolean(brand.path || brand.paths?.length);
  });
}
