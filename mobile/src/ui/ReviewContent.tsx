 ...
 */
export function ReviewContent({ project, workspace, mode, active, revision = 0 }: {
  project: Project; workspace: WorkspaceModel; mode: ReviewMode; active: boolean; revision?: number;
}) {
  ...
  useEffect(() => { setPath(''); setFile(false); }, [project.id, mode]);
  useEffect(() => {
    if (!active) return;
    let current = true;
  ...
  const parent = () => { setPath(path.split('/').slice(0, -1).join('/')); setFile(false); };
  return <View style={s.fill}>
    {mode === 'files' && <View style={[s.breadcrumb, { borderBottomColor: colors.border }]}>
      {path ? <IconButton icon="arrow-back" label="Parent folder" onPress={parent} /> : <Icon name="folder-outline" size={18} color={colors.muted} />}
      <Text numberOfLines={2} selectable style={[s.path, { color: colors.muted }]}>{path || project.path}</Text>
    </View>}
    ...
    {busy ? <View style={s.loading}><ActivityIndicator color={colors.accent} /><Hint>{workspace.demo ? 'Opening example…' : 'Reading from your computer…'}</Hint></View> :
      error ? <View style={s.notice}><Hint error>{error}</Hint></View> : mode === 'files' && !file ?
        <ScrollView contentContainerStyle={s.files}>{entries.length === 0 ? <EmptyState icon="folder-open-outline"
          ...
          entries.map(entry => <Pressable key={entry.path} accessibilityRole="button" onPress={() => {
            setPath(entry.path); setFile(entry.kind !== 'directory');
          ...
          </Pressable>)}</ScrollView> : content ? <ScrollView style={[s.fill, { backgroundColor: colors.workspace }]}>
...
const s = StyleSheet.create({
  fill: { flex: 1 },
  breadcrumb: { minHeight: 50, flexDirection: 'row', alignItems: 'center', gap: 10,
