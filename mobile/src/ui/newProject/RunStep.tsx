...
export function RunStep({
  ...
  useEffect(() => {
    if (expanded) tail.current?.scrollToEnd({ animated: false });
  }, [expanded, log]);
  ...
  const footer = running
    ? { secondary: { title: 'Cancel', onPress: onCancel } }
    : phase === 'stalled'
      ? {
          secondary: { title: 'Close', onPress: onClose },
          primary: { title: 'Open it in a terminal', onPress: onOpenTerminal },
        }
      : phase === 'failed'
        ? {
            secondary: { title: 'Close', onPress: onClose },
            ...
            primary:
              log.length > 0
                ? { title: 'Open the folder anyway', onPress: onOpenFolder }
    ...
    <>
      <ScrollView style={s.scroll} contentContainerStyle={s.content}>
        ...
        <Text
          ...
          style={[s.status, { color: phase === 'failed' ? colors.error : colors.text }]}
        >
          {status}
        </Text>
        {running && log.length > 0 && (
          <Text numberOfLines={1} style={[s.tail, { color: colors.muted }]}>
            {log[log.length - 1]}
          </Text>
        )}
        {steps.length > 0 && (
          <View style={s.steps}>
            {steps.map((step, at) => {
              ...
              return (
                <View key={`${step}-${at}`} style={s.step}>
                  <View
                    style={[
                      s.bullet,
                      {
                        borderColor: finished || active ? colors.accent : colors.border,
        ...
        {expanded && (
          <ScrollView
            ref={tail}
            style={[s.log, { backgroundColor: colors.elevated }]}
            contentContainerStyle={s.logContent}
          >
            <Text selectable style={[s.logText, { color: colors.text }]}>
              {log.join('\n')}
            </Text>
          </ScrollView>
        )}
