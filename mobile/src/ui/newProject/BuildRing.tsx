...
export function BuildRing({ phase, index, total, label }: {
  ...
  const tone = done ? colors.success : phase === 'stalled' ? colors.warning
    : phase === 'failed' ? colors.error : colors.accent;
  ...
  useEffect(() => {
    Animated.timing(fill, { toValue: fraction, duration: reduced ? 0 : 620,
      easing: Easing.out(Easing.cubic), useNativeDriver: false }).start();
  }, [fraction, reduced, fill]);
  useEffect(() => {
    if (!running || reduced) { spin.setValue(0); breathe.setValue(0); return; }
    const turn = Animated.loop(Animated.timing(spin, { toValue: 1, duration: 1500,
  ...
  return <View accessibilityRole="progressbar" accessibilityValue={{ min: 0, max: total, now: done ? total : index }}
    accessibilityLabel={label} style={s.wrap}>
    {running && !reduced && <Animated.View style={[s.sweep, { opacity: glow, transform: [{ rotate }] }]}>
      <Svg width={SIZE} height={SIZE}>
        ...
        <Circle cx={SIZE / 2} cy={SIZE / 2} r={R} stroke="url(#sweep)" strokeWidth={STROKE}
          fill="none" strokeLinecap="round" strokeDasharray={`${CIRCUMFERENCE * 0.24} ${CIRCUMFERENCE}`} />
      </Svg>
    </Animated.View>}
    <Svg width={SIZE} height={SIZE} style={s.ring}>
      <Circle cx={SIZE / 2} cy={SIZE / 2} r={R} stroke={colors.border} strokeWidth={STROKE} fill="none" />
      <ArcCircle cx={SIZE / 2} cy={SIZE / 2} r={R} stroke={tone} strokeWidth={STROKE} fill="none"
