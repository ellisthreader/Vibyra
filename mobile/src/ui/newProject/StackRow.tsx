...
export function StackRow({ entry, missing, kindLabel, recommended, selected, onPick }: {
  ...
  return <Pressable accessibilityRole="checkbox" accessibilityLabel={entry.name} accessibilityHint={entry.blurb}
    accessibilityState={{ disabled: blocked, checked: selected }} aria-disabled={blocked} aria-checked={selected}
    disabled={blocked} onPress={onPick}
    ...
    {blocked
      ...
      : <View style={s.tick}>{selected && <Icon name="checkmark" size={19} color={colors.accent} />}</View>}
  </Pressable>;
}
const s = StyleSheet.create({
  row: { minHeight: 64, paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', gap: 14 },
