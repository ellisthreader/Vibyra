    ...
    <>
      ...
      >
        {stacks.length > 0 && (
          <View style={s.stacks}>
            {stacks.map((entry) => (
              <View key={entry.id} style={[s.chip, { backgroundColor: colors.elevated }]}>
                <StackMark templateId={entry.id} kind={entry.kinds[0]!} size={17} />
                <Text numberOfLines={1} style={[s.chipText, { color: colors.muted }]}>
                  {entry.name}
                </Text>
              </View>
            ))}
        ...
        {editingParent ? (
          <TextInput
            value={parent}
            onChangeText={onParent}
            autoFocus
            autoCapitalize="none"
            autoCorrect={false}
            spellCheck={false}
            accessibilityLabel="Folder to put the project in"
        ...
        {destination.error ? (
          <Text accessibilityLiveRegion="polite" style={[s.error, { color: colors.error }]}>
            {destination.error}
          </Text>
        ) : null}
      </ScrollView>
