...
export function useProjectCreate(
  ...
  const fail = useCallback(
    (error: unknown) =>
      dispatch({
        ...
        patch: {
          phase: 'failed',
          error: error instanceof Error ? error.message : 'The project could not be built.',
        },
      }),
    [dispatch],
  );
  ...
  const finish = useCallback(
    async (event: Extract<ScaffoldEvent, { type: 'done' }>) => {
      const { workspace: live, state: now, visible: shown, onDone: done } = latest.current;
      if (!event.ok) {
        dispatch({
          type: 'run',
          patch: {
            phase: event.stalled ? 'stalled' : 'failed',
            error: event.message ?? 'The project could not be built.',
          },
        });
        return;
      }
      ...
      const project =
        event.project ??
        latest.current.workspace.projects.find((item) => item.path === destination.path) ??
        null;
  ...
  useEffect(() => {
    ...
    return scaffold.follow((event) => {
      ...
      if (event.type === 'step')
        dispatch({
          type: 'run',
          patch: { progress: { index: event.index, total: event.total, label: event.label } },
        });
  ...
  const start = useCallback(async () => {
    ...
    if (destination.error) {
      dispatch({ type: 'run', patch: { phase: 'failed', error: destination.error } });
      return;
    }
    if (!scaffold) {
      dispatch({
        type: 'run',
        patch: { phase: 'failed', error: 'Connect a computer that can start projects.' },
      });
      return;
    }
    dispatch({ type: 'run', patch: { phase: 'running', error: null, progress: null } });
    try {
      await scaffold.start(now.runId, request);
    } catch (error) {
      fail(error);
    }
  ...
  const cancel = useCallback(() => {
    void scaffold?.cancel(latest.current.state.runId).catch(fail);
  }, [scaffold, fail]);
  ...
  const adoptAsIs = useCallback(
    async (withTerminal: boolean) => {
