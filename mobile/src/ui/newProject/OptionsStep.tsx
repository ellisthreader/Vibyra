...
export function OptionsStep({
  ...
  host: string;
  onChange: (patch: Partial<TemplateOptions>) => void;
  onStart: () => void;
}) {
  ...
  return (
    <>
        ...
        <Group>
          {installs ? (
            <SwitchRow
              ...
              detail="Slower now, but the project runs the moment it opens."
              value={options.install}
              onChange={(install) => onChange({ install })}
            />
          ) : null}
          <SwitchRow
            title="Start a git repository"
            detail="Skipped if the template made one."
            value={options.git}
            onChange={(git) => onChange({ git })}
          />
          <SwitchRow
            title="Open a terminal when it is done"
            detail="In the new project's folder, on your computer."
            value={options.openTerminal}
            onChange={(openTerminal) => onChange({ openTerminal })}
          />
        </Group>
        {commands.length > 0 ? (
          <>
            <Pressable
              ...
              accessibilityState={{ expanded: showCommands }}
            ...
            >
              <Text style={[s.toggleText, { color: colors.accent }]}>
                {showCommands
                  ? 'Hide the commands'
                  : `Show the ${commands.length === 1 ? 'command' : `${commands.length} commands`} ${host} will run`}
              </Text>
            </Pressable>
            {showCommands && (
              <View style={[s.commands, { backgroundColor: colors.elevated }]}>
                {commands.map((command, index) => (
                  <Text key={index} selectable style={[s.command, { color: colors.text }]}>
                    {command}
                  </Text>
                ))}
              </View>
      ...
      <WizardFooter
        primary={{ title: 'Start building', label: `Start building on ${host}`, onPress: onStart }}
      />
    </>
  );
}
