 ...
 * as answering it.
 */
export function WizardFooter({ primary, secondary, quiet = [] }: {
  primary?: FooterAction; secondary?: FooterAction; quiet?: FooterAction[];
}) {
  ...
  return <View style={[s.strip, { borderTopColor: colors.border, paddingBottom: Math.max(bottom, 12) + 4 }]}>
    ...
    {quiet.length > 0 && <View style={s.quietRow}>
      {quiet.map(action => <Pressable key={action.title} accessibilityRole="button" accessibilityLabel={action.label ?? action.title}
        disabled={action.disabled} onPress={action.onPress} hitSlop={8}
        style={({ pressed }) => [s.quiet, { opacity: action.disabled ? 0.4 : pressed ? 0.55 : 1 }]}>
...
const s = StyleSheet.create({
  strip: { paddingHorizontal: 20, paddingTop: 12, gap: 6, borderTopWidth: StyleSheet.hairlineWidth },
  buttons: { flexDirection: 'row', gap: 10 },
  grow: { flex: 1 },
  quietRow: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 22, flexWrap: 'wrap' },
