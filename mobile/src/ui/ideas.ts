...
export const ideasProject: Project = { id: IDEAS_PROJECT_ID, name: 'Ideas', path: 'On your phone' };
...
export function knownProjects(workspace: Pick<WorkspaceModel, 'status' | 'projects' | 'remembered'>): Project[] {
  return workspace.status === 'connected' ? workspace.projects : workspace.remembered?.projects ?? [];
}

/** Which project a phone chat lives in. */
export function chatProjectId(chat: Pick<VibesChat, 'host_id' | 'project_id'>,
  workspace: Pick<WorkspaceModel, 'status' | 'projects' | 'remembered' | 'host'>): string {
  if (!chat.project_id || !chat.host_id || chat.host_id !== workspace.host?.id) return IDEAS_PROJECT_ID;
  return knownProjects(workspace).some(project => project.id === chat.project_id) ? chat.project_id : IDEAS_PROJECT_ID;
}

/** The chats in one project, in the order the store keeps them (newest first). */
export const chatsInProject = (chats: VibesChat[], projectId: string,
