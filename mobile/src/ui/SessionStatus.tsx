...
export function SessionStatus({
  ...
  const action =
    model.action === 'reconnect'
      ? { label: 'Reconnect', name: 'Reconnect to this computer', press: onReconnect }
      : model.action === 'takeControl'
        ? { label: 'Take control', name: 'Take control of this terminal', press: onTakeControl }
        : null;
  return (
    <View style={[s.strip, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
      ...
      <Text
        ...
        style={[s.text, { color: warning ? colors.warning : colors.muted }]}
      >
        {model.text}
      </Text>
      {action?.press && (
        <Pressable
          ...
          style={({ pressed }) => [
            s.action,
            { backgroundColor: colors.action, opacity: pressed ? 0.7 : 1 },
          ]}
        >
          {busy ? (
...
const s = StyleSheet.create({
  ...
  text: { flex: 1, fontSize: 13, lineHeight: 18, letterSpacing: -0.05 },
  action: {
    minHeight: 30,
  ...
  actionText: { fontSize: 13, fontWeight: '600', letterSpacing: -0.1 },
  pill: {
    position: 'absolute',
    right: 14,
    bottom: 12,
