    ...
    <>
      ...
      >
        <Text style={[s.lead, { color: colors.muted }]}>
          Connect an account, then mention it in a chat.
        </Text>
        {!live && error && (
          <View style={s.notice}>
        ...
        <View style={s.note}>
          ...
          <Text style={[s.noteText, { color: colors.muted }]}>
            Connections are encrypted. Disconnect any time.
          </Text>
        </View>
      </ScrollView>
      ...
      <IntegrationSheet
        integration={openedDevice ? null : opened}
        visible={open !== null && !openedDevice}
        ...
        signedIn={signedIn}
      />
      {workspace && vault && onConnectComputer && (
        <DeviceIntegrationSheet
          integration={openedDevice}
          visible={openedDevice !== null}
          onClose={() => setOpen(null)}
          workspace={workspace}
          vault={vault}
...
function Group({
  ...
  return (
    <View style={s.group}>
      {label && (
        <Text accessibilityRole="header" style={[s.groupLabel, { color: colors.muted }]}>
          {label}
        </Text>
      )}
...
const s = StyleSheet.create({
  ...
  lead: { ...font.subhead, fontSize: 15, marginLeft: 4 },
  notice: { marginTop: 16 },
  group: { marginTop: 22 },
  groupLabel: { ...font.section, marginBottom: 8, marginLeft: 16 },
  ...
  noteText: { ...font.footnote, fontSize: 12 },
});
