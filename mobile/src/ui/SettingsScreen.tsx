...
export function SettingsScreen({ workspace }: { workspace: WorkspaceModel }) {
  ...
  return <ScrollView contentContainerStyle={s.content}>
    ...
    {(workspace.actions.enterDemo || workspace.demo) && <>
      ...
      <View style={[s.group, { borderColor: colors.border }]}>
        <SettingRow title={workspace.demo ? 'Leave sample workspace' : 'Open sample workspace'} icon="layers-outline"
...
function SettingRow({ title, icon, value, expanded, onPress }: {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={title}
    ...
    {value && <Text style={[s.value, { color: colors.muted }]}>{value}</Text>}
...
const s = StyleSheet.create({
  ...
  rowTitle: { flex: 1, fontSize: 15, lineHeight: 22 }, value: { fontSize: 13 }, separator: { height: StyleSheet.hairlineWidth, marginLeft: 49 },
  ...
  forgetText: { fontSize: 14 }, error: { paddingTop: 12 }, footer: { marginTop: 45, textAlign: 'center', fontSize: 14, letterSpacing: -0.4 },
});
