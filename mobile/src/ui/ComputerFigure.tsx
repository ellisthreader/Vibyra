...
export function ComputerFigure({ state, logo }: { state: Figure; logo: IconName }) {
  ...
  const shell = { backgroundColor: dark ? colors.elevated : colors.surface, borderColor: colors.border,
    shadowColor: '#080B12', shadowOpacity: dark ? 0.5 : 0.16, shadowRadius: 20, shadowOffset: { width: 0, height: 12 }, elevation: 8 };
  ...
  const caret = reduced ? 1 : blink.interpolate({ inputRange: [0, 0.48, 0.52, 1], outputRange: [1, 1, 0.08, 0.08] });
  return <View pointerEvents="none" aria-hidden accessibilityElementsHidden importantForAccessibility="no-hide-descendants"
    ...
    <View style={[s.lid, shell]}>
      ...
      <View style={[s.display, screen]}>
        {state === 'live' ? <ComputerDesktop logo={logo} caret={caret} /> : <View style={s.boot}>
          <View style={{ opacity: state === 'waking' ? 0.85 : 0.3 }}><Icon name={logo} size={38} color={colors.text} /></View>
...
function useBlink(active: boolean) {
  ...
  useEffect(() => {
    value.setValue(0);
    if (!active) return;
    const loop = Animated.loop(Animated.timing(value, { toValue: 1, duration: 1100, easing: Easing.linear,
...
const s = StyleSheet.create({
  ...
  lid: { marginHorizontal: 16, height: 170, borderWidth: 1, padding: 10, borderTopLeftRadius: 18, borderTopRightRadius: 18,
  ...
  deck: { height: 14, borderWidth: 1, borderTopWidth: 0, borderBottomLeftRadius: 9, borderBottomRightRadius: 9,
