...
export function DrawerHome({
  ...
  const folders = knownProjects(workspace).filter(
    (project) => !query || project.name.toLowerCase().includes(query),
  );
  const showIdeas = !query || 'chats'.includes(query);
  ...
  return (
    <>
      {query !== '' && !showIdeas && folders.length === 0 && (
        <RailNote>{`No projects match “${query}”.`}</RailNote>
      )}
      {showIdeas && (
        <View style={s.lead}>
          <RailGroup>
            <RailRow
              mark={<Tile icon="chatbubbles-outline" />}
              label="Chats"
              detail={ideas}
              accessibilityLabel={`Chats, ${ideas}`}
              selected={inProject && currentProjectId === IDEAS_PROJECT_ID}
              onPress={() => onEnter(IDEAS_PROJECT_ID)}
            />
          </RailGroup>
        </View>
      )}
        ...
        <>
          ...
          <RailGroup>
            {folders.map((project) => {
              ...
              const where =
                project.branch ?? project.path.split('/').filter(Boolean).pop() ?? project.path;
              return (
                <RailRow
                  key={project.id}
                  mark={
                    <Tile
                      icon={project.kind === 'vault' ? 'book-outline' : 'folder-outline'}
                      faded={away}
                    />
                  }
                  label={project.name}
                  ...
                  detail={
                    sessions.length
                      ? `${where} · ${sessions.length} ${sessions.length === 1 ? 'terminal' : 'terminals'}`
                      : where
                  }
                  selected={inProject && !away && currentProjectId === project.id}
                  accessibilityLabel={`${project.name}, ${terminalWords(sessions)}`}
                  accessibilityHint={
                    away ? 'Shows what is known about this project' : 'Opens this project'
                  }
                  onPress={() => (away ? onOptions(project) : onEnter(project.id))}
                  onLongPress={() => onOptions(project)}
                />
              );
            })}
            {folders.length === 0 && (
              <RailNote>
                {workspace.viewOnly
                  ? `Open a project in Vibyra on ${host} to see it here.`
                  : `Add a folder in Vibyra Host on ${host} to see it here.`}
              </RailNote>
            )}
          </RailGroup>
        </>
          ...
          <RailGroup>
            {places.map((place) => (
              <RailRow
                ...
                icon={place.icon}
                label={place.label}
                selected={destination === place.id}
                onPress={() => onNavigate(place.id)}
              />
            ))}
          </RailGroup>
        </View>
    ...
    <View
      style={[
        s.tile,
        {
          ...
          backgroundColor: accent ? colors.accentSoft : colors.elevated,
          opacity: faded ? 0.6 : 1,
        },
      ]}
    >
