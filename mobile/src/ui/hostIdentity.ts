...
const families: { icon: IconName; label: string; match: RegExp; wire: RegExp }[] = [
  {
    icon: 'logo-apple',
    label: 'macOS',
    match: /mac|darwin|apple|os ?x/i,
    wire: /^(macos|mac ?os ?x?|darwin|apple)$/i,
  },
  { icon: 'logo-windows', label: 'Windows', match: /win/i, wire: /^win(dows)?$/i },
  {
    icon: 'logo-tux',
    label: 'Linux',
    match: /linux|tux|ubuntu|debian|fedora|arch/i,
    wire: /^linux$/i,
  },
];
...
export function describePlatform(platform?: string): HostPlatform {
  ...
  for (const family of families) {
    ...
    return {
      icon: family.icon,
      label: [family.wire.test(head) ? family.label : head, ...rest].join(' · '),
    };
  }
...
export function guessPlatform(name: string, host?: string): string | undefined {
  ...
  if (/^(desktop|laptop)-[a-z0-9]{5,}$/i.test(name.trim()) || /\bwindows\b/i.test(name))
    return 'windows';
  if (/\b(linux|ubuntu|debian|fedora|arch ?linux|nixos|raspberry ?pi|raspberrypi)\b/i.test(name))
    return 'linux';
...
export function locationLookup(address: string): string | null {
  ...
  const host = bare(address);
  return /^\d{1,3}(\.\d{1,3}){3}$/.test(host) || (/^[\da-f:]+$/.test(host) && host.includes(':'))
    ? host
    : null;
}
