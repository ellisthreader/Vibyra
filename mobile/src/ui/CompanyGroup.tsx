import { styles as s } from './CompanyGroupStyles';
  ...
  return (
    ...
    >
      ...
      <View style={locked && s.dim}>
        <BrandLogo vendor={vendorOf(model.id)} size={30} />
      </View>
      <View style={s.text}>
        <View style={s.nameRow}>
          <Text numberOfLines={1} style={[s.name, { color: locked ? colors.muted : colors.text }]}>
            {model.name}
          </Text>
          {fresh && (
      ...
      {locked ? (
        <View style={[s.lock, { backgroundColor: colors.elevated }]}>
          <Icon name="lock-closed" size={12} color={colors.muted} />
        </View>
      ) : (
...
export function CompanyGroup({
  ...
  return (
    <View style={[s.group, { borderBottomColor: colors.border, backgroundColor: 'transparent' }]}>
      <Pressable
      ...
      >
        ...
        <View style={s.text}>
          ...
          <Text
            numberOfLines={1}
            style={[s.detail, { color: chosen ? colors.accent : colors.muted }]}
          >
            {selectedModel
              ? selectedModel.name
              : company.models.length === 1
                ? '1 model'
