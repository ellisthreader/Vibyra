...
/**
 * A sheet that rises over whatever is on screen and stops short of the top, so the
 * place it was opened from stays in view, dimmed. It is drawn in the parent's own
 * tree rather than in a `Modal`: presenting a Modal while the drawer's Modal is still
 ...
 */
export function BottomSheet({ visible, title, label = title, onClose, onBack, onClosed, children, testID = 'bottom-sheet' }: {
  ...
  return <View style={StyleSheet.absoluteFill} pointerEvents={visible ? 'auto' : 'none'}>
    ...
    <Animated.View testID={testID} accessibilityViewIsModal role={native ? undefined : 'dialog'} aria-modal aria-label={label}
      ...
      <View {...panHandlers} style={s.handleZone}>
        ...
        <View style={s.header}>
          <View style={s.side}>{onBack && <Pressable accessibilityRole="button" accessibilityLabel="Back" onPress={onBack}
            style={({ pressed }) => [s.back, { opacity: pressed ? 0.55 : 1 }]}>
...
const s = StyleSheet.create({
  ...
  closeTarget: { width: 44, height: 44, alignItems: 'flex-end', justifyContent: 'center' },
