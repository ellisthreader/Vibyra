...
export function ScannerSheet(props: {
  visible: boolean;
  onClose: () => void;
...
function Scanner({ onScan }: { onScan: (value: string) => void }) {
  ...
  if (!permission?.granted)
    return (
      ...
      >
        <Button
          title={permission?.canAskAgain === false ? 'Open camera settings' : 'Allow camera'}
          onPress={() => {
  ...
  return (
    <View style={s.body}>
      <View style={s.camera}>
        <CameraView
          ...
          onBarcodeScanned={(event) => {
            ...
            if (event.data.startsWith('vibyra://pair')) {
      ...
      {invalid && (
        <Hint error>This is not a Vibyra pairing code. Scan the code shown by Vibyra Host.</Hint>
      )}
    </View>
  );
}
const s = StyleSheet.create({
  body: { flex: 1, padding: 22, gap: 20 },
  camera: {
    flex: 1,
    borderRadius: 26,
    overflow: 'hidden',
