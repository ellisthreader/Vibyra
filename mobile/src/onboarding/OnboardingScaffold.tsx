...
export function OnboardingScaffold({
  ...
  return (
    ...
    >
      {backdrop && (
        <View pointerEvents="none" aria-hidden style={s.backdrop}>
          {backdrop}
        </View>
      )}
      <View ref={top.frame} onLayout={top.onLayout} style={s.frame}>
        <View style={s.header}>
          ...
          {header ? (
            <View style={s.headerTitle}>{header}</View>
          ) : (
            <View
              accessible
              ...
              {Array.from({ length: steps }, (_, index) => (
                <View
                  ...
                  style={[
                    s.dot,
                    {
                      backgroundColor: index + 1 === step ? colors.accent : colors.border,
                      width: index + 1 === step ? 18 : 6,
                    },
                  ]}
                />
              ))}
...
const s = StyleSheet.create({
  ...
  body: { flex: 1 },
  header: { minHeight: 56, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8 },
  spacer: { width: 44 },
  headerTitle: { flex: 1, alignItems: 'center' },
  dots: { flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 6 },
  dot: { height: 6, borderRadius: 3 },
  content: { flexGrow: 1, paddingHorizontal: GUTTER, paddingTop: 8, paddingBottom: 24, gap: 18 },
  footer: { paddingHorizontal: GUTTER, paddingTop: 8, paddingBottom: 12, gap: 4 },
  link: { minHeight: 44, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 12 },
  linkText: { fontSize: 15, fontWeight: '600', letterSpacing: -0.2, textAlign: 'center' },
  titles: { gap: 8 },
  title: { ...font.title, fontSize: 29, lineHeight: 34, letterSpacing: -0.9 },
  detail: { ...font.subhead, fontSize: 15, lineHeight: 22 },
});
