 ...
 */
export function AccountCodeStep({ email, busy, error, onSubmit, onBack }: {
  email: string; busy: boolean; error: string | null; onSubmit: (code: string) => void; onBack: () => void;
}) {
  const { colors } = useTheme();
  const [code, setCode] = useState('');
  const [recovery, setRecovery] = useState(false);
  useEffect(() => { if (error) setCode(''); }, [error]);
  const swap = () => { setRecovery(value => !value); setCode(''); };
  return <View style={s.form}>
    <View style={s.head}>
      ...
      <Text style={[s.line, { color: colors.muted }]}>
        {recovery ? `One of the recovery codes you saved for ${email}.`
          : `Open your authenticator app and enter the six-digit code for ${email}.`}</Text>
    </View>
    {recovery ? <TextInput accessibilityLabel="Recovery code" value={code} onChangeText={setCode}
      placeholder="abcde-12345" placeholderTextColor={colors.muted} autoCapitalize="none" autoCorrect={false}
      autoFocus editable={!busy} returnKeyType="done" onSubmitEditing={() => onSubmit(code)}
      style={[s.recovery, { color: colors.text, backgroundColor: colors.surface, borderColor: colors.border }]} />
    ...
    <Button title="Log in" busy={busy} disabled={busy || !code.trim()} onPress={() => onSubmit(code)} />
    <View style={s.links}>
      <Pressable accessibilityRole="button" disabled={busy}
        accessibilityLabel={recovery ? 'Use your authenticator app instead' : 'Use a recovery code instead'}
        onPress={swap} style={s.link}>
        <Text style={[s.linkText, { color: colors.accent }]}>
          {recovery ? 'Use your authenticator app' : 'Use a recovery code'}</Text>
      </Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel="Back" disabled={busy} onPress={onBack} style={s.link}>
...
const s = StyleSheet.create({
  ...
  line: { fontSize: 14, lineHeight: 20, textAlign: 'center', maxWidth: 320 },
  recovery: { minHeight: 52, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 16,
    fontSize: 17, textAlign: 'center', letterSpacing: 1 },
