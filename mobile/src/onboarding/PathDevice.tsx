...
export function PathDevice({ phone = false, lit }: { phone?: boolean; lit: Animated.Value }) {
  ...
  const clamp = (inputRange: number[], outputRange: number[]) =>
    lit.interpolate({ inputRange, outputRange, extrapolate: 'clamp' });
  const shell = {
  ...
  const widths = phone ? [32, 22, 27] : [52, 72, 36];
  const screen = (
    <View
      style={[
        s.screen,
        {
          borderRadius: phone ? 10 : 5,
    ...
    >
      <Animated.View
        style={[
          ...
          { backgroundColor: colors.accent, opacity: clamp([0, 1], [0, dark ? 0.16 : 0.1]) },
        ]}
      />
      <View style={s.bars}>
        {widths.map((width, index) => {
          ...
          return (
            <View key={index} style={[s.bar, { width }]}>
              <Animated.View
                style={[
                  StyleSheet.absoluteFill,
                  s.bar,
                  { backgroundColor: colors.muted, opacity: clamp([0, 1], [0.3, 0]) },
                ]}
              />
              <Animated.View
                style={[
                  ...
                  s.bar,
                  {
                    backgroundColor: index === 1 ? colors.muted : colors.accent,
  ...
  return (
    <Animated.View
      ...
      style={[
        s.stage,
        {
          opacity: clamp([0, 1], [0.72, 1]),
          transform: [
            { translateY: clamp([0, 1], [0, -6]) },
            { scale: clamp([0, 1], [0.97, 1.03]) },
          ],
        },
      ]}
    >
      {phone ? (
        <View style={[s.phone, shell]}>
          <View style={[s.gloss, { left: 14, right: 14 }, gloss]} />
          {screen}
        </View>
      ) : (
        <View style={s.laptop}>
          <View style={[s.lid, shell]}>
            <View style={[s.gloss, { left: 12, right: 12 }, gloss]} />
            {screen}
          </View>
...
const s = StyleSheet.create({
  stage: { height: 128, alignItems: 'center', justifyContent: 'flex-end' },
  ...
  phone: { width: 60, height: 114, borderRadius: 17, borderWidth: 1, padding: 4 },
  screen: { flex: 1, overflow: 'hidden', justifyContent: 'center' },
  bars: { gap: 7, paddingHorizontal: 11 },
  bar: { height: 5, borderRadius: 2.5 },
});
