  ...
  return <View style={s.block}>
    ...
    <View style={s.who}>
      <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}
        style={[s.name, !name && s.nameLong, { color: colors.text }]}>{name || account.email}</Text>
      {Boolean(name) && <Text numberOfLines={1} style={[s.email, { color: colors.muted }]}>{account.email}</Text>}
    </View>
    <View style={[s.pill, { backgroundColor: colors.elevated }]}>
      {provider.icon
        ? <Icon name={provider.icon} size={14} color={colors.muted} />
        : <Image source={require('../../assets/google-g.png')} style={s.google} accessible={false} aria-hidden />}
...
const s = StyleSheet.create({
  ...
  badge: { position: 'absolute', right: 0, bottom: 2, width: 26, height: 26, borderRadius: 13, borderWidth: 2.5,
  ...
  pillText: { fontSize: 12.5, fontWeight: '500' }, google: { width: 13, height: 13 },
});
