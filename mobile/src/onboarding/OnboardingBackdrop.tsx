...
// Four cobalt and sky glows drifting on different cycles with a gentle brightness pulse (Graphite + Cobalt: no violet). `centre` is the y the
...
export function OnboardingBackdrop({ centre, soft = false }: { centre: number; soft?: boolean }) {
  ...
  const halo = useDrift({ period: 9000, dx: 34, dy: -22, grow: 1.16, opacity: 0.85 * k });
  const bloom = useDrift({ period: 12500, dx: 58, dy: 44, grow: 1.22, opacity: 0.4 * k });
  const wash = useDrift({ period: 10500, dx: -52, dy: -38, grow: 1.2, opacity: 0.32 * k });
  const ember = useDrift({ period: 14000, dx: -44, dy: 34, grow: 1.18, opacity: 0.36 * k });
  return (
    <>
      <Animated.Image source={cobalt} style={[s.bloom, { top: centre - 250 }, bloom]} />
