...
export type ProfileActionName = 'updateProfile' | 'setAvatar' | 'removeAvatar' | 'loadAccountDevices' | 'removeAccountDevice'
  | 'signOutEverywhere' | 'sendPasswordReset' | 'resendVerification' | 'deleteAccount' | 'openBillingPortal';
 ...
 */
export function makeProfileActions(store: WorkspaceStore): Pick<WorkspaceActions, ProfileActionName> {
  const signedIn = () => {
  ...
  return {
    updateProfile: async changes => {
      ...
      if (changes.email !== undefined && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email ?? '')) throw new Error('Enter a valid email address.');
      await keep(token, api('updateProfile')(token, { ...(name !== undefined ? { name } : {}), ...(email !== undefined ? { email } : {}) }));
    },
    setAvatar: async uri => { const { token } = signedIn(); await keep(token, api('uploadAvatar')(token, uri)); },
    removeAvatar: async () => { const { token } = signedIn(); await keep(token, api('removeAvatar')(token)); },
    ...
    deleteAccount: async (proof: AccountDeletion, signal?: AbortSignal) => {
      ...
      if ('password' in proof) {
        ...
        if (!store.deps.account.providerDeletion) throw new Error('Deleting this account isn’t available on this phone yet.');
        const deleted = await store.deps.account.providerDeletion(proof.provider, token, signal ?? new AbortController().signal);
