...
export interface ProfileApi {
  ...
  startDeletion(token: string, provider: AccountProvider): Promise<{ flowId: string; authUrl: string }>;
...
export function createProfileApi({ baseUrl, deviceName, fetch: fetchImpl = fetch, read = fetch }: {
  baseUrl: string; deviceName: string; fetch?: Fetch; read?: Fetch;
}): ProfileApi {
  const root = baseUrl.replace(/\/+$/, '');
  const call = async (method: string, path: string, { json, form, token }: { json?: object; form?: FormData; token?: string } = {}) => {
    let response: Response;
    let payload: Record<string, unknown> | null;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), form ? 60000 : 20000);
    try {
      response = await fetchImpl(`${root}${path}`, { method, signal: controller.signal, body: form ?? (json ? JSON.stringify(json) : undefined),
    ...
    if (!response.ok || !payload || payload.ok !== true) {
      const message = typeof payload?.error === 'string' && payload.error ? payload.error
        : response.status >= 500 ? 'Vibyra is having trouble right now. Try again in a moment.'
          : response.status === 404 || response.status === 405 ? 'This isn’t available yet. Try again after the next update.' : unreachable;
  ...
  const device = (value: unknown): AccountDevice | null => {
    ...
    return { id: raw.id, name: typeof raw.deviceName === 'string' && raw.deviceName ? raw.deviceName : 'Vibyra device',
      location: typeof raw.location === 'string' ? raw.location : '', current: raw.current === true,
      lastActive: typeof raw.updatedAt === 'string' ? raw.updatedAt : null };
  };
  return {
    updateProfile: async (token, changes) => parseAccount((await call('POST', '/api/account/profile', { json: changes, token })).user),
    uploadAvatar: async (token, uri) => {
      const form = new FormData();
      if (onDisk.test(uri)) form.append('photo', { uri, name: 'avatar.jpg', type: 'image/jpeg' } as unknown as Blob);
      else {
        let blob: Blob;
        try { blob = await (await read(uri)).blob(); } catch { throw new AccountError('That photo could not be read. Choose another.', 0); }
        form.append('photo', blob, 'avatar.jpg');
      }
      return parseAccount((await call('POST', '/api/account/avatar', { form, token })).user);
    },
    removeAvatar: async token => parseAccount((await call('DELETE', '/api/account/avatar', { token })).user),
    devices: async token => {
      const list = (await call('GET', '/api/account/sessions', { token })).devices;
      return Array.isArray(list) ? list.map(device).filter((item): item is AccountDevice => item !== null) : [];
    },
    revokeDevice: async (token, id) =>
      ({ currentRevoked: (await call('DELETE', `/api/account/devices/${encodeURIComponent(id)}`, { token })).currentRevoked === true }),
    revokeAllSessions: async token => { await call('DELETE', '/api/account/sessions', { token }); },
    forgotPassword: async email => message(await call('POST', '/api/auth/password/forgot', { json: { email } }),
      'If that email belongs to a Vibyra password account, a reset link has been sent.'),
    resendVerification: async email => message(await call('POST', '/api/auth/email/resend', { json: { email } }),
      'If that email still needs verification, a new link has been sent.'),
    deleteWithPassword: async (token, password) => { await call('DELETE', '/api/account', { json: { password }, token }); },
    deleteSignedIn: async token => { await call('DELETE', '/api/account', { token }); },
    ...
    startDeletion: async (token, provider) => {
      const data = await call('POST', `/api/auth/desktop/${provider}/start`, { json: { deviceName, purpose: 'deletion' }, token });
    ...
    deletionStatus: async (provider, flowId) => {
      const data = await call('GET', `/api/auth/desktop/${provider}/status/${encodeURIComponent(flowId)}`);
    ...
    billingPortal: async token => {
      ...
      if (typeof data.url !== 'string' || !data.url.startsWith('https://')) throw new AccountError('Billing couldn’t be opened. Please try again.', 0);
      return data.url;
    },
  };
}
