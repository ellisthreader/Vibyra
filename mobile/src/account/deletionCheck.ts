...
export async function finishedAnyway(api: Pick<ProfileApi, 'deletionStatus'>, provider: AccountProvider,
  flowId: string | null, signal: AbortSignal) {
  if (!flowId || signal.aborted) return false;
  try { return await api.deletionStatus(provider, flowId) === 'deleted'; } catch { return false; }
}
