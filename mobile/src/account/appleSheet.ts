...
export const appleSheet = Platform.OS === 'ios' && requireOptionalNativeModule('ExpoAppleAuthentication') !== null;
export const appleSheetMissing = 'Open the Vibyra app to continue with Apple. This preview does not support Apple sign-in.';
...
export function appleSheetError(error: unknown): Error | null {
  ...
  if (code === 'ERR_REQUEST_UNKNOWN')
    return new Error('Apple couldn’t sign you in. Check your iPhone is signed in to your Apple Account in Settings, then try again.');
  return error instanceof Error ? error : new Error('Apple couldn’t sign you in. Please try again.');
}
