...
export interface TwoFactorPrompt { challengeId: string; expiresIn: number }
...
export interface TwoFactorApi {
  ...
  confirmTwoFactor(token: string, code: string): Promise<{ recoveryCodes: string[]; user: Account }>;
...
type Call = (method: string, path: string, body?: object, token?: string) => Promise<Record<string, unknown>>;

const text = (value: unknown, whose: string): string => {
  if (typeof value !== 'string' || !value) throw new AccountError(`Vibyra returned an unexpected ${whose}. Try again.`, 0);
  return value;
};
...
export function twoFactorPrompt(payload: Record<string, unknown>): TwoFactorPrompt | null {
  ...
  return { challengeId: asked.challengeId, expiresIn: typeof asked.expiresIn === 'number' ? asked.expiresIn : 300 };
}
...
export function createTwoFactorApi(call: Call, deviceName: string): TwoFactorApi {
  const session = (payload: Record<string, unknown>): AccountSession =>
    ({ token: text(payload.token, 'session'), user: parseAccount(payload.user) });
  return {
    twoFactorState: async token => {
      const data = await call('GET', '/api/account/2fa', undefined, token);
      return { enabled: data.enabled === true, available: data.available !== false,
    ...
    startTwoFactor: async token => {
      ...
      if (!uri.startsWith('otpauth://totp/')) throw new AccountError('Vibyra returned an unexpected setup link.', 0);
      return { secret: text(data.secret, 'setup key'), uri, account: typeof data.account === 'string' ? data.account : '' };
    },
    ...
    newRecoveryCodes: async (token, code) => codes((await call('POST', '/api/account/2fa/recovery', { code }, token)).recoveryCodes),
    disableTwoFactor: async (token, code) => parseAccount((await call('DELETE', '/api/account/2fa', { code }, token)).user),
