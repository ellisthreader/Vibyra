...
export const terminalState = (disabled: boolean, colors: Colors, dark: boolean, fontSize?: number, grid?: TerminalGrid | null) => ({
  target: TERMINAL_TARGET, type: 'state', disabled, fontSize, ...(grid ? { grid } : {}),
...
export const terminalOutput = (data: string, reset: boolean, grid?: TerminalGrid | null) => ({
  target: TERMINAL_TARGET, type: 'output', data, reset, ...(reset && grid ? { grid } : {}),
});
