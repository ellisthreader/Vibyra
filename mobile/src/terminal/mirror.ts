...
export const clampZoom = (size: number) =>
  !(size > 0)
    ? DEFAULT_FONT_SIZE
...
export const parseGrid = (grid?: Partial<Grid> | null): Grid | null =>
  ...
  grid.cols! > 0 &&
  grid.rows! > 0
    ? { cols: Math.min(grid.cols!, 500), rows: Math.min(grid.rows!, 200) }
    : null;
...
export class MirrorStage {
  ...
  readonly zooming: ZoomTarget = {
    current: () => this.zoom ?? this.fitted(),
    clamp: clampZoom,
    apply: (size, at) => this.setZoom(size, at),
    reset: (at) =>
      this.setZoom(
        this.zoom === null || Math.abs(this.zoom - this.fitted()) < 0.5 ? DEFAULT_FONT_SIZE : null,
        at,
      ),
  };
  ...
  layout(follow: boolean) {
    ...
    if (!grid) return;
    if (terminal.options.fontSize !== DEFAULT_FONT_SIZE)
  ...
  setZoom(next: number | null, at: Point) {
    ...
    const px = (view.scrollLeft + x) / before,
      py = (view.scrollTop + y) / before;
    this.zoom = next;
    this.layout(false);
  ...
  cursorVisible() {
    ...
    return (
      y >= this.view.scrollTop - 1 && y + height <= this.view.scrollTop + this.view.clientHeight + 1
    );
  }
  ...
  private fitted() {
    ...
    return clampZoom(
      Math.floor(((DEFAULT_FONT_SIZE * this.view.clientWidth) / pixels.width) * 4) / 4,
    );
  }
  private scale() {
    return (this.zoom ?? this.fitted()) / DEFAULT_FONT_SIZE;
  }
}
