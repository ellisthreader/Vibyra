...
export class PreviewFeed {
  ...
  constructor(
    private peek: (sessionId: string) => Promise<Snapshot>,
  ...
  receive(event: TerminalEvent) {
    ...
    if (event.type === 'size') {
      if (event.cols && event.rows) {
        entry.cols = event.cols;
        entry.rows = event.rows;
        this.mark(entry);
      }
      return;
    }
  ...
  preview(id: string) {
    return this.entries.get(id)?.preview;
  }
  close() {
  ...
  private load(id: string) {
    ...
    this.peek(id)
      ...
      .catch(() => {
        if (this.entries.get(id) !== entry) return;
        entry.preview ??= { lines: [], cols: entry.cols ?? 80 };
        this.changed();
      });
  }
  ...
  private flush() {
    ...
    for (const entry of this.entries.values()) {
      ...
      entry.preview = {
        lines: previewLines(entry.ledger.output, cols, entry.rows, this.count),
        cols,
      };
    }
    this.changed();
