...
export class Grid {
  ...
  put(ch: string, width: number, style: Style) {
    if (width === 0) { this.join(ch); return; }
    if (this.pending || (width === 2 && this.x === this.cols - 1)) { this.x = 0; this.pending = false; this.feed(); }
    const line = this.lines[this.y]!;
    line[this.x] = { ch, style };
    if (width === 2) line[this.x + 1] = TAIL;
    if (this.x + width >= this.cols) { this.x = this.cols - 1; this.pending = true; } else this.x += width;
  }
  ...
  feed() { if (this.y === this.bottom) this.scroll(1); else if (this.y < this.rows - 1) this.y++; }
  reverse() { if (this.y === this.top) this.scroll(-1); else if (this.y > 0) this.y--; }
  ...
  scroll(n: number, from = this.top) {
    ...
    for (let i = 0; i < count; i++) {
      if (n > 0) { this.lines.splice(from, 1); this.lines.splice(this.bottom, 0, []); }
      else { this.lines.splice(this.bottom, 1); this.lines.splice(from, 0, []); }
    }
  }
  region(top: number, bottom: number) {
    if (top >= bottom || bottom > this.rows) return;
    this.top = top - 1; this.bottom = bottom - 1; this.moveTo(0, 0);
  }
  ...
  eraseDisplay(mode: number) {
    if (mode === 0) { this.eraseLine(0); for (let y = this.y + 1; y < this.rows; y++) this.lines[y] = []; }
    else if (mode === 1) { this.eraseLine(1); for (let y = 0; y < this.y; y++) this.lines[y] = []; }
  ...
  deleteChars(n: number) { this.lines[this.y]!.splice(this.x, n); }
  insertLines(n: number) { this.scroll(-n, this.y); this.x = 0; }
  deleteLines(n: number) { this.scroll(n, this.y); this.x = 0; }
}
