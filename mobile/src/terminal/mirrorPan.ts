...
export function mirrorPan(view: HTMLElement, active: () => boolean) {
  ...
  view.addEventListener('touchstart', event => {
    ...
    start = active() && event.touches.length === 1
      ? { x: touch.clientX, y: touch.clientY, left: view.scrollLeft, top: view.scrollTop } : null;
  }, { passive: true, capture: true });
  view.addEventListener('touchmove', event => {
    if (!start || event.touches.length !== 1 || document.getSelection()?.toString()) { start = null; return; }
    const dx = start.x - event.touches[0].clientX, dy = start.y - event.touches[0].clientY;
  ...
  view.addEventListener('touchend', () => { start = null; }, { passive: true });
