...
export function IntegrationsProvider({
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const next = await api.catalogue();
      if (request === version.current) adopt(next);
    } catch (e) {
      if (request !== version.current) return;
      setError(e instanceof Error ? e.message : 'Integrations are temporarily unavailable.');
      setLive(false);
    }
  }, [api, adopt]);
  useEffect(() => {
  ...
  const act = useCallback(
    async (
      id: string,
      run: (signal: AbortSignal) => Promise<IntegrationCatalogue>,
      signal?: AbortSignal,
    ) => {
      ...
      try {
        ...
        if (request === version.current && !cancelled)
    ...
    () => ({
      ...
      authorize: (id, signal) =>
        act(
          id,
          async (active) =>
            (await (api!.authorize ? api!.authorize(id) : authorizeInBrowser(api!, id, active)))
              .catalogue,
          signal,
        ),
