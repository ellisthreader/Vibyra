        ...
        <Animated.View
          style={[
            s.knot,
            {
              ...
              opacity: knot,
              transform: [
                { scale: knot.interpolate({ inputRange: [0, 1], outputRange: [0.4, 1] }) },
              ],
            },
          ]}
        >
          <Icon
            name={connected ? 'checkmark-circle' : 'lock-closed'}
            size={connected ? 20 : 16}
            color={tint}
          />
        </Animated.View>
      </View>
      <View style={s.mark}>
        <Glyph brand={brand} size={40} />
      </View>
    </View>
  );
}
...
function Glyph({ brand, size }: { brand: Brand; size: number }) {
    ...
    return (
      <Svg width={size} height={size} viewBox="0 0 24 24">
        {brand.paths.map((part) => (
          <Path key={part.fill + part.d.length} d={part.d} fill={part.fill} />
        ))}
      </Svg>
    );
  if (brand.path)
  ...
  return (
    <Text style={[s.initial, { fontSize: size * 0.8, color: colors.text }]}>
      {brand.name.charAt(0)}
    </Text>
  );
}
