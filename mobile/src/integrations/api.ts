...
export function createIntegrationsApi(baseUrl: string, token: (prepare?: boolean) => string | null | Promise<string | null>, fetcher: typeof fetch = fetch): IntegrationsApi {
  // The catalogue is a menu: it answers before sign-in, the same way models do.
  const call = async (path: string, body?: unknown, anonymous = false, retried = false): Promise<any> => {
    const identity = await token(!anonymous);
    if (!identity && !anonymous) throw new IntegrationsError('Your guest session could not be started. Please try again.', 401);
    ...
    try {
      ...
      if ((r.status === 403 && data.error === 'Create your free account to use this.')
        || (r.status === 401 && /^Sign in to Vibyra to connect /i.test(data.error ?? data.message ?? ''))) {
        throw new IntegrationsError('Connecting is temporarily unavailable. Please try again later.', r.status);
      }
      // Legacy OAuth setup failures describe deployment internals, not a user action.
      const message = data.error ?? data.message;
      if (!r.ok && /\/start$/.test(path) && typeof message === 'string'
  ...
  return {
    ...
    flow: async flowId => {
      ...
      return { status: data.status, error: data.error, catalogue: validate(data.catalogue) } as IntegrationFlowState;
    },
  };
}
