...
class VibesAttachmentsTest extends TestCase
{
    ...
    private const SEES = 'openai/gpt-6-astra';
    ...
    public function test_the_configured_persistent_disk_carries_the_file_through_the_queued_reply(): void
    {
        Storage::fake('vibes-attachments');
        config(['vibes.attachments_disk' => 'vibes-attachments']);
        $file = $this->upload(UploadedFile::fake()->createWithContent('persistent.txt', 'Survives deployment'));
        $path = DB::table('vibes_attachments')->where('id', $file['id'])->value('path');
        Storage::disk('vibes-attachments')->assertExists($path);
        Storage::disk('local')->assertMissing($path);
        ['body' => $body] = $this->send($this->quote($this->chat(), self::BLIND, [$file['id']])->assertOk()->json());
        $this->assertStringContainsString('Survives deployment', json_encode($body['messages']));
    }

    public function test_a_failed_storage_write_never_returns_an_attachment_record(): void
    {
        $disk = \Mockery::mock(\Illuminate\Contracts\Filesystem\Filesystem::class);
        $disk->shouldReceive('put')->once()->andReturnFalse();
        Storage::shouldReceive('disk')->with('local')->andReturn($disk);
        $this->postJson('/api/vibes/attachments', ['file' => UploadedFile::fake()->createWithContent('note.txt', 'hello')])
            ->assertStatus(503);
        $this->assertDatabaseCount('vibes_attachments', 0);
    }
}
